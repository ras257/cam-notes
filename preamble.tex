...
\usepackage{listofitems} % Provides tools for working with lists
...
\usepackage{optidef} % Used for typesetting optimisation problems
...
\usetikzlibrary{arrows.meta, calc, shapes.geometric, cd, decorations.pathreplacing, decorations.markings, fpu, hobby, patterns, automata, positioning} % TikZ packages
\usepackage{tikz-3dplot} % Used for simple 3D plots
\usepackage{pgfplots} % Used for more advanced/3D plots
\usepackage{siunitx} % Used for SI units
\usepackage{mathrsfs} % Used for script calligraphic font
\usepackage{bm} % Used for bold Greek characters
\usepackage{esint} % Used for alternative integrals
...
\newcounter{law}\counterwithin{law}{chapter}
...
\newenvironment{law}[1][] {
  ...
  \par\smallskip
  \noindent
  \textbf{Law \thelaw}\ifstrempty{#1}{}{ (#1)}:
  \begin{shaded*}
  \ignorespaces
}{
  \end{shaded*}
...
\newcommand{\induction}[6]{
  ...
  \quietinduction{#1}{#2}{#3}{#4}{#5}{#6}
  Thus proven by mathematical induction.
}

\newcommand{\quietinduction}[6]{
  \par\noindent
  \textbf{Base case} (#1):\nopagebreak[4]
...
\renewcommand{\vec}[1]{\mathbf{\bm{#1}}} % vector (bold notation)
\newcommand{\uvec}[1]{\hat{\vec{#1}}} % unit vector
\newcommand{\avec}[1]{\overrightarrow{#1}} % vector from A to B
...
\newcommand{\comp}{\complement} % Set complement
