...
\end{tikzpicture}
\end{center}
If the vector $\vec{x}$ is not originally on the plane, taking $\vec{a} \times \vec{x}$ first projects $\vec{x}$ onto the plane and then carries out the same operation as above.
...
\]
The perpendicular distance from the origin to the plane is $\frac{|k|}{|\vec{n}|}$.
...
\begin{example}[Finding the Intersection Between a Line and a Plane]
  ...
  \begin{itemize}
    \item The line and plane are parallel and never intersect.
...
\end{enumerate}
\subsection{Applications of Suffix Notation and Summation Convention}
...
Finally, we can use it to provide a proof for the Cauchy-Schwarz inequality (\cref{cauchySchwarz}) that works in $\R^{n}$.
\begin{proof}
  \begin{align*}
    (\vec{x} \cdot \vec{x})(\vec{y} \cdot \vec{y}) - (\vec{x} \cdot \vec{y})^2 &= x_i x_i y_j y_j - x_i y_i x_j y_j \\
    ...
    &\geq 0
  \end{align*}
  Thus $|\vec{x}|^2|\vec{y}|^2 \geq (x \cdot y)^2$ with equality if and only if:
  \[
    x_i y_j = x_j y_i \iff \frac{x_i}{y_i} = \frac{x_j}{y_j} = \lambda
  \]
  for all $i, j$. Equivalently, $x_i = \lambda y_i\ \forall i \iff \vec{x} = \lambda \vec{y}$.
\end{proof}
...
\]
\begin{remark}[Note]
  When we write vectors in $\R^{n}$ as column vectors, if $\vec{x}, \vec{y} \in \R^{n}$, $\vec{x}^{\trans}$ and $\vec{y}^{\trans}$ will be row vectors and their inner product can be represented as:
  \[
...
For $\vec{v}, \vec{w} \in V$, denote their inner product by $\vec{v} \cdot \vec{w}$ or $(\vec{v}, \vec{w})$.
This has the following properties:
\begin{enumerate}
