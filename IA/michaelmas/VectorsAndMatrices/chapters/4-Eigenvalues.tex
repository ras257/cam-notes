...
\begin{proof}[General $n \times n$ Matrix]
  ...
  \]
  Since every entry of $\widetilde{M}$ is a cofactor of $M$, it will be a degree $n- 1$ polynomial in $t$.
  Therefore we can define the matrices $B_r$ such that:
  \[
...
\begin{example}
  ...
  \begin{proofcases}
    \begin{case}{Ellipse}
      ...
      \]
      If we set $\mathcal{F}(\vec{x}) = 1$ this defines an ellipse with respect to the principle axes:
      \begin{center}
    ...
    \begin{case}{Hyperbola}
      ...
      \]
      If we set $\mathcal{F}(\vec{x}) = 1$ this defines a hyperbola with respect to the principal axes:
      \begin{center}
      \begin{tikzpicture}[>=stealth]
        \draw[->, gray!90] (0, -3) -- (0, 3) node[above] {$x_2$};
        \draw[->, gray!90] (-3, 0) -- (3, 0) node[right] {$x_1$};
