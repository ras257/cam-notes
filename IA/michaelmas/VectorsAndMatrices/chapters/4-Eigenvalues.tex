...
\begin{remark}
  ...
  Let $k = \vec{x}^{\trans}A\vec{x}^{\trans}$, then:
  \[
    k = \vec{x}^{\trans}A\vec{x} = (\vec{x}^{\trans}A^{\trans}\vec{x})^{\trans} = -(\vec{x}^{\trans}A\vec{x})^{\trans} = -k^{\trans} = -k \implies k=\vec{x}^{\trans}A\vec{x}^{\trans} = 0  \]
...
Noting that $(A^{-1})^{\trans} = A^{-1}$, we then have:
\[
...
\begin{proofcases}
  \begin{case}{Ellipse}
    ...
    For $a > b$, $b^2 = a^2(1 - e^2)$ and they have foci at $x = \pm ae$
  \end{case}
  \begin{case}{Parabola}
...
\end{itemize}
Consider the expression for the conic given by:
\[
  \frac{x^2}{a^2} + \frac{y^2}{b^2} = k
\]
\begin{definition}[Conic Sections]
  For all \textit{conic sections} or \textit{conics}, we define the following:
  \begin{itemize}
  ...
  \begin{enumerate}
    ...
    \item For $\lambda_1 = \lambda_2 = \lambda$ and $M_\lambda = 2 \neq m_{\lambda} = 1$, let $\vec{v}$ be an eigenvector for $\lambda$ and extend this to a basis for $\C^{2}$ by adjoining a vector $\vec{w}$ that is linearly independent to $\vec{v}$.
      ...
      \[
        ...
        \end{pmatrix}
      \]
      represents $T$ with respect to the basis $\{\vec{u}, \vec{w}\}$.
...
\begin{theorem}[Jordan normal forms in $\C^n$]
  ...
  \[
    ...
    \end{pmatrix}
  \]
  where all other entries are 0, $n_1 + \cdots n_r = n$, and $\lambda_1, \ldots, \lambda_r$ are the eigenvalues of $A$ and $B$.

  Each \textit{Jordan block} $J_p(\lambda)$ has form:
  \[
...
\end{definition}
Recall from \cref{orthoDeterminant} that $R \in O(n) \iff \det R = \pm 1$.
...
\begin{align*}
  ...
  \text{ or }&RH \text{ where }  \in \SO(n),\ H \in O(n) \setminus \SO(n)
\end{align*}
...
\begin{remark}
  ...
  That is, we can either carry out the rotation directly in the active point of view, or transform the axes by the inverse rotation and find the coordinates of $\vec{x}$ with respect to this new basis in the passive point of view.
\end{remark}
...
We generalise the notion of orthonormality to this new ``inner product'' as above so that the standard basis vector are orthogonal.
