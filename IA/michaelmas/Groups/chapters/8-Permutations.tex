...
\begin{theorem}[Transpositions Generate]
  \label{transpositionsGenerate}
  The set of transpositions generates $S_n$ for any finite $n \geq 2$.
\end{theorem}
\begin{proof}
  \induction
  {$n = 2$}{
...
\begin{lemma}
  ...
  If $\tau_1, \ldots, \tau_k$ are all adjacent transpositions and $\sigma = \tau_1 \cdots \tau_k$, then $k \equiv \text{\#(inversions of $\sigma$)} \pmod{2}$
\end{lemma}
\begin{proof}
  \induction
  {$k = 0$}{
...
\begin{proof}
  ...
  \begin{proofdirection}{Assume $\sigma_1$ and $\sigma_2$ are conjugate}
    ...
    \[
      \sigma_2 = \cycle{b^{1}_{1} {\cdots} b^{1}_{l_1}} \cdots \cycle{b^{k}_{l} {\cdots} b^{k}_{l_k}}
    \]
    where $b^{i}_{j} = \tau(a^{i}_{j})$ for all $1 \leq i \leq k$ and $1 \leq j \leq l_i$.
...
\begin{example}[Conjugacy Classes of $S_3$]
  ...
  There are 3 options for the first element, 2 for the second element and then 1 for the third element so $|\ccl_{S_3}\cycle{1 2 3}| = 2 \cdot 1 = 2$
\end{example}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item The hypothesis means that:
      ...
      Now consider some odd permutation $\tau \in S_n$.
      ...
      It follows that each element of $\ccl_{S_n}(\gamma)$ must be in either $\ccl_{A_n}(\gamma)$ (if it is $\gamma$ conjugated by an even element) or $\ccl_{A_n}(\tau\gamma\tau^{-1})$ (if it is $\gamma$ conjugated by an odd element).
      Therefore, $\ccl_{S_n}(\gamma)$ is split into disjoint conjugacy classes $\ccl_{A_n}(\gamma)$ and $\ccl_{A_n}(\tau \gamma \tau^{-1})$ in $A_n$.
  \end{enumerate}
\end{proof}
\begin{remark}
  In the case where the conjugacy class splits, we know that $|\ccl_{S_n}(\gamma)| = 2 |\ccl_{A_n}(\gamma)|$.
...
\begin{example}[Conjugacy in $A_5$]
  ...
  \begin{itemize}
    \item $\cycle{4 5} \in C_{S_5}\cycle{1 2 3}$, that is, $\cycle{4 5}$ commutes with $\cycle{1 2 3}$, so by \cref{conjugacyAn} $\ccl_{S_5}\cycle{1 2 3}$ \textbf{does not split}.
...
\begin{proof}
  Suppose $H \lhd A_5$.
  By Sheet 3 Q5 (or the definition), $H$ is a union of conjugacy classes in $A_5$.
  ...
  Therefore, the only normal subgroups are $1$ and $A_5$ so $A_5$ is simple.
\end{proof}
\end{document}
