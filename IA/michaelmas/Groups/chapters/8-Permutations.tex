...
\begin{proof}
  ...
  {$k$}{
    ...
    \]
    where $\tau$ is an adjacent transposition (as if not, it could be rewritten as a product of and odd number of adjacent transpositions without changing the parity of $k$), so we can write $\tau = \cycle{l {l + 1}}$ for some $l$, and $\sigma'$ is the product of $k - 1$ transpositions.
