...
\begin{example}
  ...
  \begin{itemize}
    ...
    \item Now we check where 3 goes.
      ...
      We have already written 1 so we close the cycle.
    \item Now we check where 2 goes.
      In the first cycle $2 \mapsto 1$.
      In the second cycle $1 \mapsto 2$.
      So $2$ is unchanged and so does not need to be included.
  \end{itemize}
...
\begin{theorem}[Disjoint Cycles]
  Every $\sigma \in S_n$, can be written as a product of disjoint cycles. \label{disjointCyclesUnique}
...
\begin{proof}
  ...
  We see that:
  \[
    \sigma = \cycle{i_1 \sigma(i_1) {\cdots} \sigma^{n_1 - 1}(i_1)} \cdots \cycle{i_k \sigma(i_k) {\cdots} \sigma^{n_k - 1}(i_k)}
  \]
...
\begin{remark}
  If $\sigma$ is a $k$-cycle, then $\ord(\sigma) = k$.
  More generally, if $\sigma$ is a $(k_1, \ldots, k_l)$-cycle then:
  \[
    \ord(\sigma) = \lcm\{k_1, \ldots, k_l\}
  \]
...
\begin{theorem}[Transpositions Generate]
  The set of transpositions generates $S_n$ for any finite $n \geq 2$. \label{transpositionsGenerate}
\end{theorem}
\begin{proof}
  \induction
  {$n = 2$}{
...
\begin{lemma}[Products of Adjacent Transpositions]
  Any transposition $\cycle{i j}$ can be written as a product of an \textbf{odd} number of adjacent transpositions. \label{adjacentLemma}
\end{lemma}
\begin{proof}
  WLOG, assume $j > i$ as we can just reorder the transposition.
  We now carry out induction on $j$ for any fixed $i$.
  \quietinduction
  {$j = i + 1$}{
  ...
  {$j = i + k + 1$}{
    ...
    Thus $\cycle{i j}$ can be written a product of an odd number of adjacent transpositions as it has two more adjacent transpositions than $\cycle{i {j - 1}}$.
  }
\end{proof}
...
\begin{definition}
  ...
  That is, an inversion is any pair where the ordering of the two elements is changed by the permutation.
\end{definition}
\begin{lemma}
  If $\tau_1, \ldots, \tau_k$ are all adjacent transpositions and $\sigma = \tau_1 \cdots \tau_k$, then $k \equiv \text{\#(inversions of $\sigma$)} \pmod{2}$. \label{inversionsMod2}
\end{lemma}
\begin{proof}
  \induction
  {$k = 0$}{
  ...
  {$k$}{
    ...
    \]
    Since $\tau$ is an adjacent transposition, we can write $\tau = \cycle{l {l + 1}}$ for some $l$, and $\sigma'$ is the product of $k - 1$ adjacent transpositions.

    Now, consider which pairs $\{i, j\}$ could be an inversion of $\sigma'$ but not of $\sigma$ or vice versa?
    The only such pairs are those where $\sigma$ sends $i$ and $j$ to either $l$ or $l + 1$, that is, $\{i, j\}$ such that $\sigma'(i) = l$ and $\sigma'(j) = l + 1$.
...
\end{proof}
\begin{lemma}
  If $\tau_1, \ldots, \tau_k$ are all transpositions and $\sigma = \tau_1 \cdots \tau_k = e$, then $k$ is even. \label{evenLemma}
\end{lemma}
\begin{proof}
  By \cref{adjacentLemma}, we may assume that all $\tau_i$ are adjacent as any non-adjacent transpositions could be rewritten as a product of an \textbf{odd} number of adjacent transpositions without changing the parity of $k$ (whether $k$ is odd or even).
...
\begin{proof}\par
  ...
  \]
  Note how there is a different number of transpositions on each side as it is possible to decompose the same element into transpositions in multiple different ways.
...
\end{proof}
\begin{remark}
  Since $C_2$ is abelian, $\sign(\alpha \tau \alpha^{-1}) = \sign(\tau)$ so all elements in a conjugacy class of $S_n$ share the same sign.
\end{remark}
...
\begin{remark}
  ...
  This is because the full cycle is even if and only if we are multiplying an even number of odd permutations.
\end{remark}
\begin{example}
...
\begin{theorem}
  Two permutations $\sigma_1, \sigma_2 \in S_n$ have the same cycle type if and only if they are conjugate. \label{conjugateCycles}
\end{theorem}
\begin{proof}
  \begin{proofdirection}{Assume $\sigma_1$ and $\sigma_2$ have the same cycle type}
    ...
    We can write:
    \begin{align*}
      \sigma_1 &= \cycle{a^{1}_{0} {\cdots} a^1_{l_1 - 1}} \cdots \cycle{a^{k}_{0} {\cdots} a^{k}_{l_k - 1}} \\
      \sigma_2 &= \cycle{b^{1}_{0} {\cdots} b^{1}_{l_1 - 1}} \cdots \cycle{b^{k}_{0} {\cdots} b^{k}_{l_k - 1}}
    \end{align*}
    So for any $a^{i}_{j}$, we have:
    \[
  ...
  \begin{proofdirection}{Assume $\sigma_1$ and $\sigma_2$ are conjugate}
    ...
    \]
    where $b^{i}_{j} = \tau(a^{i}_{j})$ for all $1 \leq i \leq k$ and $0 \leq j \leq l_i - 1$.
...
\begin{example}[Conjugacy Classes of $S_3$]
  ...
  This involves picking 2 numbers from 3 numbers (since the order of the elements within a transposition is irrelevant) so there is exactly $\binom{3}{2} = 3$ ways of doing this so $|\ccl_{S_3}\cycle{1 2}| = 3$.

  To find $|\ccl_{S_3}\cycle{1 2 3}|$, consider constructing a 3-cycle.
  Since we are in $S_3$, the cycle must contain all three elements.
  We can always choose 1 to be the first entry as the cycle is unaffected when we cycle its entries.
  This means that we then just have a choice between two orderings of 2 and 3, so $|\ccl_{S_3}\cycle{1 2 3}| = 2$.
\end{example}
...
\begin{example}[Counting a Centraliser]
  ...
  \begin{enumerate}
    ...
    \item We know that changing the order of the two transpositions does not affect the cycle type.
      We can do this using another permutation and conjugation.
      Let $\sigma = \cycle{1 2}\cycle{3 4}$ and $\tau = \cycle{1 3}\cycle{2 4}$, we then have:
      \[
        \tau \sigma \tau^{-1} = \cycle{1 3}\cycle{2 4}\cycle{1 2}\cycle{3 4}\cycle{1 3}\cycle{2 4} = \cycle{1 2}\cycle{3 4} = \sigma
      \]
      So $\tau \sigma \tau^{-1} = \sigma \implies \tau \sigma = \sigma \tau$ so they commute.
      This is a prime example of how conjugation acts like a ``change of basis''.
    \item $\cycle{1 4}\cycle{2 3}$ commutes for the same reason as above.
    \item $\tau = \cycle{1 4 2 3}$ commutes using the same change of basis argument as:
      \[
        \tau \sigma \tau^{-1} = \cycle{4 3}\cycle{1 2} = \sigma \implies \tau \sigma = \sigma \tau
      \]
      We could also notice that it squares to $\cycle{1 2}\cycle{3 4}$ as if we have $\sigma^2 = \rho$, then $\sigma \rho = \sigma \sigma^2 = \sigma^2 \sigma = \rho \sigma$ so it commutes.
    \item $\cycle{1 3 2 4}$ commutes as above.
  \end{enumerate}
  We have listed 8, so we know have found the entire centraliser.

  In fact, it is often easier to think of this as the stabiliser of the conjugation action.
  As discussed, we know conjugation directly swaps around entries in the resulting cycle, for example:
  \[
    \cycle{1 2 3 4} \xrightarrow{\text{ conjugate by } \cycle{1 3}} \cycle{3 2 1 4}
  \]
  This means that each element of the stabiliser should correspond to a different permutation of entries of $\tau$ that does not affect $\tau$.

  We can quite easily list all the different equivalent forms of $\tau$, noting that they will all come from reordering cycles and cycling elements within each cycle (see \cref{disjointCyclesUnique}), and then form the suitable element to conjugate with:
  \begin{center}
  \begin{tabular}{c|c}
  Equivalent form of $\sigma$ &  Conjugating element $\tau$\\
  \hline
  $\cycle{1 2}\cycle{3 4}$ & $e$ \\
  $\cycle{1 2}\cycle{4 3}$ & $\cycle{3 4}$ \\
  $\cycle{2 1}\cycle{3 4}$ & $\cycle{1 2}$ \\
  $\cycle{2 1}\cycle{4 3}$ & $\cycle{1 2}\cycle{3 4}$ \\
  $\cycle{3 4}\cycle{1 2}$ & $\cycle{1 3}\cycle{2 4}$ \\
  $\cycle{3 4}\cycle{2 1}$ & $\cycle{1 3 2 4}$ \\
  $\cycle{4 3}\cycle{1 2}$ & $\cycle{1 4 2 3}$ \\
  $\cycle{4 3}\cycle{1 2}$ & $\cycle{1 4}\cycle{2 3}$
  \end{tabular}
  \end{center}
\end{example}
\begin{example}[Conjugacy Classes of $S_4$]
  ...
  $\cycle{1 2 3}$ & $2 \binom{4}{3} = 8$ & 3 & $\binom{4}{3}$ to pick, smallest element can always go first so two orderings by swapping last two \\
  $\cycle{1 2 3 4}$ & $3! = 6$ & 4 & Cycle can always start with 1, then $3!$ for remaining items \\
  \end{tabular}
  \end{center}
  The sum of the sizes of the conjugacy classes is 24, the order of the group, which is expected as conjugacy classes partition.
\end{example}
...
\begin{lemma}[Conjuacy Classes in $A_n$]
  Let $\gamma \in A_n \lhd S_n$. Either: \label{conjugacyAn}
  \begin{enumerate}
    \item If some odd element of $S_n$ (which will not be in $A_n$) commutes with $\gamma$, then:
      \[
        \ccl_{A_n}(\gamma) = \ccl_{S_n}(\gamma)
      \]
    \item If every element of $S_n$ that commutes with $\gamma$ is even, that is, $C_{S_n}(\gamma) \leq A_n$, then $\ccl_{S_n}(\gamma)$ splits into two equally sized and disjoint subsets as follows:
      \[
        \ccl_{S_n}(\gamma) = \ccl_{A_n}(\gamma) \cup \ccl_{A_n}(\tau \gamma \tau^{-1})
      \]
      where $\tau$ is any odd element of $S_n$ (e.g. a transposition).
  \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item The hypothesis means that:
      ...
      Now consider any odd permutation $\tau \in S_n$.
      ...
      But then:
      \[
        \gamma (\tau^{-1} \alpha) = (\tau^{-1} \alpha) \gamma
      \]
      so $\tau^{-1} \alpha \in C_{S_n}(\gamma) = C_{A_n}(\gamma)$.
      But $\tau^{-1} \alpha$ is odd, so we have arrived at a contradiction.
      Hence $\tau \gamma \tau^{-1} \notin \ccl_{A_n}(\gamma)$.

      We know that $|\ccl_{A_n}(\gamma)| = \frac{1}{2}|\ccl_{S_n}(\gamma)|$ and since conjugacy classes are disjoint, $\ccl_{A_n}(\gamma) \cap \ccl_{A_n}(\tau \gamma \tau^{-1}) = \emptyset$.
      We can form a bijection between them by conjugating by $\tau$ and so they must both have the same size.
      Thus $|\ccl_{A_n}(\gamma)| = |\ccl_{A_n}(\tau \gamma \tau^{-1})| = \frac{1}{2}|\ccl_{S_n}(\gamma)|$.
      Therefore, $\ccl_{S_n}(\gamma)$ is split into disjoint conjugacy classes $\ccl_{A_n}(\gamma)$ and $\ccl_{A_n}(\tau \gamma \tau^{-1})$ of equal size in $A_n$.
  \end{enumerate}
\end{proof}
\begin{remark}
  \begin{itemize}
    \item In the case where the conjugacy class splits, we know that $|\ccl_{S_n}(\gamma)| = 2 |\ccl_{A_n}(\gamma)|$.
      This means that if $|\ccl_{S_n}(\gamma)|$ is odd, then the conjugacy class cannot split.
    \item When the conjugacy class splits, $\ccl_{A_n}(\gamma)$ is all the elements of $\ccl_{S_n}(\gamma)$ that are $\gamma$ commuted with an event element of $S_n$ and $\ccl_{A_n}(\tau \gamma \tau^{-1})$ is all the elements that are $\gamma$ commuted with an odd element of $S_n$.
  \end{itemize}
\end{remark}
This makes it possible to determine conjugacy classes in $A_n$.
\subsection{Examples for Small Alternating Groups}
\begin{example}[Conjuacy Classes in $A_4$]
  The even elements of $S_4$ are $e$, $(2, 2)$-cycles, and 3-cycles.

  Since there is an odd number of $(2, 2)$-cycles, we cannot split the conjugacy class in half, so $\ccl_{S_4}(\cycle{1 2}\cycle{3 4})$ remains intact in $A_4$.
  ...
  \]
  All of the elements in this subgroup are even, so by \cref{conjugacyAn}, $\ccl_{S_4}\cycle{1 2 3}$ splits.
  ...
  Finally, lets look at conjugacy in $S_5$ and $A_5$.
  \begin{example}[Conjugacy in $S_5$]
    \begin{center}
    \renewcommand\arraystretch{1.5}
    \begin{tabular}{c|c|c|>{\small}m{5cm}}
    Typical Element -- $\gamma$ & $|\ccl_{S_5}(\gamma)|$ & $|C_{S_5}(\gamma)|$ & Comment\\
    \hline
    $e$ & 1 & 120 & $5! = 120$ \\
    $\cycle{1 2}$ & $\binom{5}{2} = 10$ & 12 & Choose 2 from 5, order irrelevant \\
    $\cycle{1 2 3}$ & $2 \binom{5}{3} = 20$ &6 & $\binom{5}{3}$ to pick, smallest element can always go first so two orderings by swapping last two \\
    $\cycle{1 2}\cycle{3 4}$ & $\frac{1}{2}\binom{5}{2}\binom{3}{2} = 15$ & 8 & $\binom{5}{2}$ for first and $\binom{3}{2}$ for second, then half as we have double counted as transpositions commute \\
    $\cycle{1 2 3}\cycle{4 5}$ & $2\binom{5}{3} = 20$ & 6 & Select a 3-cycle as above, then the remaining two elements are the transposition\\
    $\cycle{1 2 3 4}$ & $\binom{5}{4}\cdot3! = 30$ & 4 & $\binom{5}{4}$ to pick and then $3!$ ways to arrange as we can always start with the smallest element. \\
    $\cycle{1 2 3 4 5}$ & $4! = 24$ & 5 & Can always start with 1, $4!$ for rest\\
    \end{tabular}
    \end{center}
    The sum of the sizes of the conjugacy classes is $120 = |S_5|$, as  expected.
  \end{example}
\end{example}
\begin{example}[Conjugacy in $A_5$]
  \label{conjugacyA5}
  The even elements types in $S_5$ are $e$, $(2, 2)$-cycles, $3$-cycles, and $5$-cycles so we only have these elements in $A_5$.

  \begin{itemize}
    \item $\cycle{4 5} \in C_{S_5}\cycle{1 2 3}$, that is, $\cycle{4 5}$ is odd and commutes with $\cycle{1 2 3}$, so by \cref{conjugacyAn} $\ccl_{S_5}\cycle{1 2 3}$ \textbf{does not split}.
    \item $|\ccl_{S_5}(\cycle{1 2}\cycle{3 4})| = 15$ which is an odd number so $\ccl_{S_5}(\cycle{1 2}\cycle{3 4})$ \textbf{does not split}.
    \item $|C_{S_5}\cycle{1 2 3 4 5}| = 5$ so $C_{S_5}\cycle{1 2 3 4 5} = \langle \cycle{1 2 3 4 5} \rangle \leq A_5$ so $\ccl_{S_5}\cycle{1 2 3 4 5}$ \textbf{does split} by \cref{conjugacyAn}.
  \end{itemize}
  ...
  \renewcommand\arraystretch{1.5}
  \begin{tabular}{c|c|>{\small}m{7cm}}
  Typical Element -- $\gamma$ & $|\ccl_{A_5}(\gamma)|$ & Comment\\
  \hline
  $e$ & 1 & -- \\
  $\cycle{1 2 3}$ & 20 & Does not split \\
  $\cycle{1 2}\cycle{3 4}$ & 15 & Does not split \\
  $\cycle{1 2 3 4 5}$ & 12 & Has order 24 in $S_5$ and splits so has order 12 \\
  $\cycle{2 1 3 4 5}$ & 12 & Other half of $\ccl_{S_5}\cycle{1 2 3 4 5}$, that is, $\ccl_{A_5}(\tau \cycle{1 2 3 4 5}\tau^{-1})$ for odd $\tau \in S_5$.

  In this case, we used $\tau = \cycle{1 2}$. \\
  \end{tabular}
  \end{center}
  The sum of the sizes of the conjugacy classes is $60 = \frac{120}{2} = |A_5|$.
\end{example}
...
\begin{proof}
  Suppose $H \lhd A_5$.
  By Sheet 3 Q5, $H$ is a union of conjugacy classes in $A_5$.
  Since conjugacy classes are disjoint, the possible orders are then:
  \begin{center}
  \begin{tabular}{l|c}
  Order & Divides 60? \\
  \hline
  ...
  $1 + 12 = 13$ & \cross \\
  $1 + 12 + 12 = 25$ & \cross\\
  $1 + 20 + 15 + 12 + 12 = 60$ & \tick\\
  \end{tabular}
  \end{center}
  By Lagrange's Theorem (\cref{lagrangesTheorem}), the only groups that could be normal subgroups are $1$ and $A_5$ which we know are normal subgroups.
  Therefore, the only normal subgroups are $1$ and $A_5$, thus $A_5$ is simple.
\end{proof}
\end{document}
