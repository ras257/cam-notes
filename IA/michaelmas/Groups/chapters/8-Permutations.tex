...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item The hypothesis means that:
      ...
      Now consider transposition $\tau \in S_n$, which is an odd permutation.
...
\begin{example}[Conjuacy Classes in $A_4$]
  ...
  \begin{example}[Conjugacy in $S_5$]
    ...
    $\cycle{1 2 3}$ & $2 \binom{5}{3} = 20$ &6 & $\binom{5}{3}$ to pick, then two orderings by reversing items \\
    ...
    $\cycle{1 2 3 4}$ & $\binom{5}{4}\cdot3! = 30$ & 4 & $\binom{5}{4}$ to pick and then $3!$ ways to arrange as we can always start with a specific element. \\
