...
\begin{definition}[Reflections]
  Any $v \in \R^{n} \setminus \{\vec{0}\}$ defines an \textit{orthogonal plane}:
  \[
    \vec{v}^{\perp} = P_{\vec{v}} = \{x \in \R^{n}: \vec{x} \cdot \vec{v} = 0\}
  \]
...
\begin{proof}
  ...
  {$n$}{
    ...
    If we restrict $S_{\vec{v}}A$ to $P_{\vec{e}_n}$, it is still an orthogonal transformation, so, by the induction hypothesis, there exists $\vec{v}_1, \ldots, \vec{v}_{n - 1} \in \R^{n - 1}$ such that:
    \[
      S_{\vec{v}}A = S_{\vec{v}_1} \cdots S_{\vec{v}_{n - 1}} \text{ on }\R^{n - 1}
    \]
    Since both sides also fix $\vec{e}_n$, they must also agree on $\R^{n}$.
    Therefore $S_{\vec{v}}A = S_{\vec{v}_1} \cdots S_{\vec{v}_n}$ and so $A = S_{\vec{v}} S_{\vec{v}_1} \cdots S_{\vec{v}_n}$.
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item If $A \in \SO(2)$, then $\det A = 1$ so $k$ is even.
      ...
      \]
      We see that $\vec{v}$ is parallel to $\vec{x} - S_{\vec{v}}\vec{x}$ and $\vec{u}$ is parallel to $\vec{x} - S_{\vec{u}}\vec{x}$.
      Note that $\vec{x} - S_{\vec{v}}\vec{x}$ and $\vec{x} - S_{\vec{v}}\vec{x}$ cannot both be 0 as the only point on both the planes $P_{\vec{u}}$ and $P_{\vec{v}}$ is $\vec{0}$.
      However, $\vec{x} - S_{\vec{u}}\vec{x} = \vec{x} - S_{\vec{v}}\vec{x}$ so $\vec{u}$ and $\vec{v}$ are parallel to the same non-zero vector and therefore $\vec{u}$ and $\vec{v}$ are parallel, which is a contradiction.
      Hence $A$ only fixes $\vec{0}$ as claimed and so is a rotation.
  \end{enumerate}
\end{proof}
\begin{remark}
  Let $A \in \SO(2)$.
...
\begin{proof}
  ...
  Suppose some $\vec{x}$ not on $\ell$ is fixed by $A$, that is:
  \[
    S_{\vec{u}}S_{\vec{v}}\vec{x} = \vec{x} \iff S_{\vec{u}}\vec{x} = S_{\vec{v}}\vec{x}
  \]
  Arguing as in the previous lemma, $\vec{v}$ is parallel to $\vec{x} - S_{\vec{v}}\vec{x}$ and $\vec{u}$ is parallel to $\vec{x} - S_{\vec{u}}\vec{x}$.
  Note that $\vec{x} - S_{\vec{v}}\vec{x}$ and $\vec{x} - S_{\vec{v}}\vec{x}$ cannot both be $\vec{0}$ as the only points on both the planes $P_{\vec{u}}$ and $P_{\vec{v}}$ is the line $\ell$.
  However, $\vec{x} - S_{\vec{u}}\vec{x} = \vec{x} - S_{\vec{v}}\vec{x}$ so $\vec{u}$ and $\vec{v}$ are both parallel to the same non-zero vector and therefore $\vec{u}$ and $\vec{v}$ are parallel, which is a contradiction.
