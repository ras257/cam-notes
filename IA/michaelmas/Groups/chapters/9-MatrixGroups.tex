...
\begin{proof}
  \begin{proofdirection}{Suppose a matrix $B$ represents $\alpha$ in some basis}
    ...
    So $B \in \GL_n(\R)A$.
    Thus, the set of all matrices representing $\alpha$ is a subset of the orbit.
  \end{proofdirection}
  \begin{proofdirection}{Suppose a matrix $B$ is in the orbit of $A$}
    ...
    Thus the orbit is a subset of the set of matrices representing $\alpha$.
  \end{proofdirection}
\end{proof}
...
\begin{proof}
  ...
  \[
    b = c = 0 \text{ and } a = d
  \]
  We also require that $ad - bc = ad = a^2 \neq 0 \iff a \in \C \setminus \{0\}$ so $\ker \Phi$ is exactly the matrices in $\C_\times$.
  Since $\ker \Phi \lhd \GL_2(\C)$, we have $\C_\times \lhd \GL_2(\C)$.
  Finally, by the First Isomorphism Theorem (\cref{isomorphismTheorem}), we have $M \cong \GL_2 /\C_\times$.
\end{proof}
...
\end{lemma}
\begin{proof}
  We can show that these two characterisations are equivalent as follows:
  \begin{proofdirection}{Suppose $A \in \GL_n(\R)$ preserves the dot product}
    For any $\vec{v} \in \R^{n}$ we have:
    \begin{align*}
  ...
  \end{proofdirection}
  \begin{proofdirection}{Suppose $A \in \GL_n(\R)$ preserves norms}
    For any $\vec{x}, \vec{y} \in \R^{n}$ we have:
    \begin{align*}
      2(A \vec{x}) \cdot (A \vec{y}) &= \norm{A\vec{x}}^2 + \norm{A\vec{y}}^2 - \norm{A\vec{x} - A\vec{y}}^2 \text{ (by polarisation identity)}\\
                                     &= \norm{A\vec{x}}^2 + \norm{A\vec{y}}^2 - \norm{A(\vec{x} - \vec{y})}^2 \\
                                     &= \norm{\vec{x}}^2 + \norm{\vec{y}}^2 - \norm{\vec{x} - \vec{y}}^2 \text{ (as $\norm{A\vec{u}} = \norm{\vec{u}}\ \forall \vec{u} \in \R^{n}$)}\\
...
\begin{definition}[Reflections]
  ...
  \[
    P_{\vec{v}} = \{\vec{x} \in \R^{n}: \vec{x} \cdot \vec{v} = 0\}
  \]
...
\begin{theorem}[Reflections Generate $O(n)$]
  Every $A \in O(n)$ is a product of \textbf{at most} $n$ reflections. \label{reflectionsGenerate}
\end{theorem}
\begin{proof}
  \induction
  {$n = 1$}{
  ...
  {$n$}{
    ...
    \]
    So $S_{\vec{v}}A$ is an orthogonal transformation that fixes $\vec{e}_n$.
    Since orthogonal transformations preserve the dot product, so the plane $P_{\vec{e}_n}$ orthogonal to $\vec{e}_n$ must also be preserved (i.e. every vector on the plane is sent to a vector on the plane).
...
\begin{remark}
  ...
  We can then write $a = \cos \theta$ and $b = \sin \theta$ for some $\theta \in \R$ so:
  \[
    A = \begin{pmatrix}
    \cos \theta & \sin \theta \\
    -\sin \theta & \cos \theta \\
