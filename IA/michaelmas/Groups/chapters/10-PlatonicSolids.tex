...
\begin{proof}
  ...
  We therefore have a \textbf{surjective} homomorphism:
  \[
    \theta: S_n \to C_2 = \{\pm 1\}
  \]
  with $\ker \theta = H$.
  Since it is surjective, we must have some element that is mapped to $-1$.
...
\begin{proof}
  ...
  To do this, we first need to find a rotational symmetry that swaps only two diagonals.
  \begin{center}
...
\begin{itemize}
  ...
  \item $\ord(-I) = 2$ so $\langle -I \rangle \cong C_2$.
\end{itemize}
...
Therefore, if we restrict the codomain of $\theta$ to its image, we get an isomorphism between $G_0$ and $A_5$ so $G_0 \cong A_5$.
\subsection{Full Symmetry Group}
