...
\begin{itemize}
  \item The ``do-nothing'' symmetry. (The \textit{identity} symmetry)
...
We can also invert symmetries, for example, to invert a reflection you can just repeat the same reflection as they are ``self-inverse''.
\begin{remark}[Important Features]
  \begin{itemize}
    \item Symmetries can be \textbf{composed} to obtain another symmetry (\textbf{closure})
...
\begin{proof}[\textbf{iv}]
  ...
  Working from $a \cdot e' = a$, we have:
  \begin{align*}
    a &= a \cdot e'\\
...
\begin{remark}[Notation]
  ...
  Therefore if $a \cdot b = e$ we may write $b = a^{-1}$ to mean the unique inverse of $a$ in $G$.
\end{remark}
...
\begin{definition}[Abelian Group]
  ...
  \]
  then $G$ is an \textit{abelian group}.
\end{definition}
...
\begin{enumerate}
  ...
  \item $(\Q, \times, 1)$ is \textbf{not} a group as $0$ has no inverse.
    However if we just remove 0, $(\Q \setminus \{0\}, \times, 1)$ is an abelian group!

    Similarly, $(\R \setminus \{0\}, \times, 1)$ and $(\C \setminus \{0\}, \times, 1)$ are abelian groups.
\end{enumerate}
\subsubsection{Finite Groups}
\begin{enumerate} \label{finiteGroupExamples}
  \item For any $n \in \N$, if we let $C_n = \{z \in \C : z^{n} = 1\}$ be the $n$-th complex roots of unity, then $(C_n, \times, 1)$ is an abelian group.
  \item If we let $\Z_n = \{0, 1, 2, \ldots, n - 1\}$ and let $a +_n b$ be the remainder when $a + b$ is divided by $n$ (i.e. the value of $a + b$ under mod $n$), then $(\Z_n, +_n, 0)$ is an abelian group.
...
\begin{definition}[Bijection]
  Let $X, Y$ be sets, a \textit{bijection} is a map $f: X \to Y$ that has an inverse $g: Y \to X$ such that $f \circ g = 1=\id_Y$ (i.e. $(f \circ g)(y) = y$ for all $y \in Y$) and $g \circ f = \id_X$ (i.e. $(g \circ f)(x) = x$ for all $x \in X$).
\end{definition}
\begin{remark}[Notation]
  We write ``$\circ$'' to mean function composition, i.e. $(f \circ g)(y) = f(g(y))$.
\end{remark}
...
\begin{lemma}
  ...
  That is, if we have maps of sets:
  \[
...
\begin{proof}
  \begin{enumerate}
    \item \textbf{Closure -} Automatic as composition of two bijections must also be a bijection.
      Note that $(f \circ g)^{-1} = g^{-1} \circ f^{-1}$ and \textbf{not} $f^{-1} \circ g^{-1}$.
...
\begin{remark}[Notation]
  ...
  If we want to emphasise that $e$ is the identity element of the group $G$ specifically, we will write $e_G$.
...
\end{definition}
\begin{remark}
  If $G$ is a group and $H \leq G$, then $H$ is a group as the associativity axiom comes along ``for free'' since it is a property of the binary operation.
\end{remark}
\begin{example}
  There are two easy examples of subgroups:
  \begin{enumerate}
    \item Every group $G$ is a subgroup of itself
    \item For any group $G$, $1_G = \{e_G\}$ is the \textit{trivial subgroup} of $G$.
  \end{enumerate}
\end{example}
...
\begin{example}
  ...
  \]
  Firstly, observe that $n\Z \subseteq \Z\ \forall n \in \N_0$.
  We can then check the three subgroup axioms:
  \begin{enumerate}
    \item \textbf{Identity -} $0 \in n\Z$.
    \item \textbf{Closure -} Let $x = nk$, $y = nl$ then $x + y = n(k + l) \in n\Z$
...
\begin{proof}
  \begin{proofcases}
    ...
    \begin{case}{Otherwise}
      ...
      Using the division algorithm, we obtain that $x = nq + r$ for some $q \in \Z$ and $0 < r < n$.
...
\section{Isometries}
Let $\C$ be the complex plane, equipped with with the usual notion of Euclidean distance:
\[
...
\begin{proposition}[Isometry Groups in $\C$]
  ...
  In particular $\Isom(X)$ is a group. \label{isomProof}
\end{proposition}
\begin{proof}
...
To do this, we first need some geometric lemmas, the first of which is a basic fact from Euclidean geometry.
...
\begin{proof}
  ...
  So both $x_2 - x_1$ and $x_3 - x_1$ are perpendicular to $f(y) - y$.
  Since $f(y) \neq y$, $f(y) - y$ is non-zero and so $x_1, x_2, x_3$ must be collinear which contradicts the hypothesis.
  Therefore we must have $f(x) = x$ for all $x \in X$.
\end{proof}
\begin{remark}
...
\begin{enumerate}
 ...
 \item Reflection in the real axis, $s(z) = \overline{z}$.
\end{enumerate}
...
\begin{proof}
  ...
  Therefore, since $f$ is a bijection, there are two cases:
  \begin{proofcases}
  ...
  We now must check that all of these elements are distinct one another.
  There are four cases.
  Considering $0 \leq k, l < n$
  \begin{proofcases}
    \begin{case}{$r^{k} = r^{l}$}
      So $e^{\frac{2 \pi i k}{n}} \cdot 1 = r^{k}(1) = r^{l}(1) = e^{\frac{2\pi i l}{n}} \cdot 1$.
      Thus $r \equiv k \pmod{n}$ but $0 \leq r, k < n$ so $k = l$ so they must be the same element.
    \end{case}
    \begin{case}{$r^{k} = s$}
    ...
    \begin{case}{$r^{k}s = r^{l}s$}
      We can right multiply by $s^{-1}$ to obtain $r^{k} = r^{l}$ and thus $k = l$ as in case 1 so they are the same element.
    \end{case}
  \end{proofcases}
  So we have shown all elements are distinct from each other and thus $|D_{2n}| = 2n$.
\end{proof}
\begin{remark}
  This proof demonstrates a useful property that follows directly from the three point lemma (\cref{threePoint}).

  If two isometries agree on 3 non-collinear points $x_1, x_2, x_3$, then they must be the same isometry:
  \begin{align*}
    f(x_i) = g(x_i)\ \forall i \in \{1, 2, 3\} &\implies (f \circ g^{-1})(x_i) = x_i\ \forall i \in \{1, 2, 3\} \\
                                               &\implies f \circ g^{-1} = \id_{X} \\
                                               &\implies f = g
  \end{align*}
\end{remark}
...
\end{lemma}
\begin{proof}
  As described in the above remark, it suffices to check that the two expressions do the same thing to three non-collinear points.
  We again pick the convenient points $x = 1, y = e^{\frac{2\pi i}{n}}, z = e^{-\frac{2\pi i}{n}}$:
  \begin{align*}
    sr(x) &= s\left(e^{\frac{2\pi i}{n}}\right) = \overline{\left(e^{\frac{2\pi i }{n}}\right)} = e^{-\frac{2\pi i}{n}} = r^{-1}(x) = r^{-1}s(x) \\
    sr(y) &= s\left(e^{\frac{4\pi i}{n}}\right) = \overline{\left(e^{\frac{4\pi i }{n}}\right)} = e^{-\frac{4\pi i}{n}} = r^{-1}(z) = r^{-1}s(y) \\
