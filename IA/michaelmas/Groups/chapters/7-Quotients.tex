...
\end{example}
\begin{lemma}
  For a subgroup $H \leq G$, $H \lhd G$ if and only if: \label{quotientCosets}
  \[
    gH = Hg
  \]
  for all $g \in G$.
...
\begin{theorem}[First Isomorphism Theorem]
  If $\phi: G \to H$ is a homomorphism, then: \label{isomorphismTheorem}
  \[
...
\begin{proof}
  ...
  Define $\Psi: G / \ker \phi \to \im \phi$, $g \ker \phi \mapsto \phi(g)$.
  ...
  \begin{align*}
    \Psi((g_1 \ker \phi)(g_2 \ker \phi)) &= \Psi(g_1 g_2 \ker \phi) \\
                                         &= \phi(g_1g_2) \\
                                         &= \phi(g_1)\phi(g_2) \\
  ...
  \textbf{Injectivity -} If $g \ker \phi \in \ker \Psi$ then $\Psi(g \ker \phi) = e$.
  Then:
  \[
    \Psi(g \ker \phi) = \phi(g) = e
  \]
  So $g \in \ker \phi$.
  Therefore,
  \[
    g \ker \phi = \ker \phi
  \]
  That is, $g \ker \phi \in \ker \Psi \implies g \ker \phi = \ker \phi$, the trivial coset.
  Therefore, $\ker \Psi = \{\ker \phi\}$ so $\ker \Psi$ is trivial and thus $\Psi$ is injective.
  ...
  Therefore, $\Psi$ is a well defined bijective homomorphism so $G / \ker \phi \cong \im \phi$
\end{proof}
\begin{example}
  \begin{enumerate}
  ...
  \item Similarly $\phi: \R \to \C$, $t \mapsto e^{2\pi i t}$ is a homomorphism with:
    ...
    \[
      \R / \Z \cong U(1)
    \]
    Remember that $\R / \Z$ are the cosets of $\Z$ in $\R$ so this has the same cyclic property as $U(1)$.
  \end{enumerate}
\end{example}
\begin{corollary}
  If $|G| < \infty$ and $\phi: G \to H$ is a homomorphism, then $|G| = |\im \phi||\ker \phi|$. \label{firstIsomorphismCorollary}
\end{corollary}
\begin{proof}
...
\begin{definition}[Simple Group]
  A group $G$ is \textit{simple} if the only normal subgroups are $1$ and $G$ itself.
\end{definition}
\begin{remark}
  A group being simple is a similar notion of a number being prime.
  It cannot be ``broken down'' into smaller groups.
\end{remark}
Every homomorphism of a simple group $G$, $\phi: G \to H$ is either trivial (so that $\im \phi = 1$ and $\ker \phi = G$) or injective (so that $\ker \phi = 1$).
\begin{example}
  $C_p$ is simple whenever $p$ is prime as its only subgroups are $1$ and itself.
\end{example}
\begin{remark}[Challenge]
  Find and understand examples of non-abelian simple groups.
\end{remark}
\end{document}
