...
For example $\Sym(\{\text{Deck of Cards}\})$ and $\Sym(\{\text{52 people}\})$ have the same structure despite a deck of cards and 52 people being different things.
\begin{definition}[Homomorphims]
  ...
  \[
    \phi(g \cdot g') = \phi(g) * \phi(g')\ \forall g, g' \in G
  \]
  where $\cdot$ is the binary operation on $G$ and $*$ is the binary operation on $H$.
\end{definition}
\begin{remark}[Notation]
  Usually differentiating the two binary operations is unnecessary if it is obvious which group's elements we are working with and the operation is fairly ``normal''.
\end{remark}
\begin{example} \label{homExamples}
  \begin{enumerate}
    \item For any two groups $G$ and $H$ the map $\phi: G \to H$, $g \mapsto e_H$ is the \textit{trivial homomorphism}.
    \item If $H \leq G$ then the map $i: H \to G$, $h \mapsto h$ is the \textit{inclusion homomorphism}.
    \item Recall from \cref{finiteGroupExamples} the group $C_n = \{z \in \C : z^{n} = 1\}$.
          ...
          \end{align*}
    \item Since $\det(AB) = \det(A)\det(B)$, the determinant function, $\det: \GL_2(\R) \to \R \setminus \{0\}$, $A \mapsto \det(A)$ is a homomorphism.
...
\begin{definition}[Isomorphism]
  If a homomorphism $\phi: G \to H$ is also a \textbf{bijection} then it is an \textit{isomorphism}.
  We then write $G \cong H$.
\end{definition}
...
\begin{example}
  \begin{enumerate}
    \item Recall $C_n = \{z \in \C : z^{n} = 1\}$ and $\Z_n = \{0, 1, \ldots, n - 1\}$ with operation $+_n$.
      ...
      To check that $\phi$ is homomorphism:
      \begin{align*}
        \phi(k +_n l) &= e^{\frac{2 \pi i}{n}(k +_n l)} \text{ by definition of $\phi$}\\
                      &= e^{\frac{2 \pi i}{n}(np)} e^{\frac{2 \pi i}{n}(k +_n l)} \text{ as $e^{2\pi i p} = 1$} \\
      ...
      Note that we needed to utilise that $k + l = np + (k +_n l)$ as $+_n$ is the binary operation for $\Z_n$, not just regular addition.
    \item Consider the two groups $(\R, +, 0)$ and $(\R_{> 0}, \times, 1)$ with the map:
      \[
        \exp: (\R, +, 0) \to (\R_{> 0}, \times, 1),\ x \mapsto e^{x}
      \]
      $\exp$ is a homomorphism as:
      \[
        \exp(x + y) = e^{x + y} = e^{x}e^{y} = \exp(x)\exp(y)
      \]
      Since $\exp^{-1} = \log$, $\exp$ is also a bijection, thus $\exp$ is an isomorphism.
...
\end{proof}
\begin{remark}
  The above proof follows a similar strategy to that of the inverses of isometry groups (\cref{isomProof}).
\end{remark}
...
\begin{proof}[\textbf{iii}]
  ...
  \begin{itemize}
    ...
    \item \textbf{Symmetric -} If $G \cong H$ then there is a isomorphism $\phi: G \to H$.
      Then from \textbf{i} we have that $\phi^{-1}: H \to G$ is also an isomorphism.
      So $H \cong G$.
...
We saw earlier in \cref{homExamples} that every subgroup leads to an inclusion homomorphism.
The converse is also true, that is, homomorphisms lead to subgroups.
...
The groups $C_n \cong \Z_n$ that we described earlier have a special property:
\begin{definition}[Cylic Group]
  A group $G$ is \textit{cyclic} if there exists an element $g \in G$ such that:
  \[
    G = \{g^{k} : k \in \Z\}
  \]
  or equivalently $G = \langle g \rangle$ where $g$ is called a \textit{generator} of $G$.
\end{definition}
\begin{example}
  \begin{enumerate}
...
\begin{proof}
  ...
  \begin{proofcases}
    \begin{case}{$n = \infty,\ S = \emptyset$}
      ...
      We know that $\phi$ is surjective since $G$ is cyclic so by definition $g^{k}$ for $k \in \Z$ are all the elements of $G$.
    ...
    \begin{case}{$n < \infty,\ S \neq \emptyset$}
      ...
      So either $k \in S$ or $k = 0$.
      Since $k \in \Z_n$, $k < n$.
      But $n$ was minimal in $S$ so $k \notin S$.
      Thus $k = 0$, so $\ker \phi = \{0\}$ and $\phi$ is injective.
...
\begin{definition}[Element Order]
  ...
  \]
  Note that $\langle g \rangle$ is cyclic so $\langle g \rangle \cong C_n$ for some $n \in \N \cup \{\infty\}$.
  We then say that $n$ is the \textit{order} of $g$ and is denoted by $\ord(g)$ or $|g|$.
\end{definition}
...
\]
So the relation holds for $-l < 0$ also.
Thus, if $xy = yx^{-1}$ and $y^2 = e$, $yx^{l} = x^{-l}y$ for any $l \in \Z$.
\begin{remark}[Memorisation]
  This is almost like we can ``push'' the $x$ across the $y$ provided we swap the sign of the power.
\end{remark}
\begin{lemma}
  Let $G$ be a group and $a, b \in G$, such that: \label{dihedralLemma}
  \begin{itemize}
...
\end{lemma}
\begin{proof}
  There are two different forms of elements in $D_{2n}$, $r^{k}$ and $r^{k}s$, so there is a total of four cases to check:
  \begin{proofcases}
    \begin{case}{$r^{k} \text{ and } r^{l}$}
      \vspace{-2em}
      \begin{align*}
    ...
    \begin{case}{$r^{k}s \text{ and } r^{l}$}
      ...
      \end{align*}
    \end{case}
    \begin{case}{$r^{k}s \text{ and } r^{l}s$}
      \vspace{-2em}
      \begin{align*}
...
\end{proof}
\begin{proposition}
  If $G$ has generating set $\{a, b\}$ such that: \label{dihedralIsomorphic}
  \begin{itemize}
    \item $a^{n} = e$
    \item $b^{2} = e$
...
\begin{proof}
  ...
  $a, b \in \im \phi$ so $\langle \{a, b\} \rangle \leq \im \phi$, therefore:
  \[
    G = \langle \{a, b\} \rangle \leq \im \phi \leq G \implies G = \im \phi
  \]
  so $\phi$ is surjective.

  We also have that $|D_{2n}| = 2n = |G|$.
  So $\phi$ is a surjection between two finite sets of equal size so must also be bijective.

  Therefore $\phi$ is an isomorphism, thus $G \cong D_{2n}$.
\end{proof}
\begin{remark}
  The above proof highlights the property that if the generators of the group are in the image of a homomorphism, then the homomorphism must be a surjection.
\end{remark}
\end{document}
