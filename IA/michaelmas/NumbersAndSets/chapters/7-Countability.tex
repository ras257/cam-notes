...
\begin{proof}
  ...
  Therefore for some $a \in A_0$, that terminates in an even number of steps, we have $f(a) = b$ (If we take $a = f^{-1}(b)$ when we see that this terminates after an even number of steps as the ancestor sequence for $b$ has an odd number of steps, so $a \in A_0$).
  Thus we have a bijection between $A_0$ and $B_1$ given by $f$.

  By symmetry, $g$ bijects $B_0$ with $A_1$.

  Finally, both $f$ and $g$ bijects $A_{\infty}$ with $B_{\infty}$ by a similar argument as these are only the points whose ancestor sequence never terminates.
