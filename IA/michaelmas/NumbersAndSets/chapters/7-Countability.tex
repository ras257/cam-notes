...
We want to talk about the sizes of infinite sets, for example, $\N$ ``looks smaller'' than $\Z$ which seems ``a lot smaller'' than $\Q$, which seems ``even smaller than'' $\R$.
\begin{definition}[Countable Set]
  We say that a set $X$ is \textit{countable} if $X$ is finite or there is a bijection $X \to \N$. \label{countableDef}

  Sets countable sets that are not finite are often referred to as \textit{countably infinite}.
\end{definition}
That is, $X$ is countable if and only if we can enumerate the elements of $X$ using $\N$.
This means that we can list its elements as:
\[
  x_1, x_2, x_3, \ldots
\]
...
\end{example}
\begin{lemma}
  Any subset of $\N$ is countable. \label{countableSubset}
\end{lemma}
\begin{proof}
  If $S \subseteq \N$ is non empty, by the Well-Ordering Principle (\cref{WOP}), there is a least element $s_1 \in S$.
  \begin{itemize}
    \item If $S \setminus \{s_1\} \neq \emptyset$, again, there is a least element $s_2 \in S \setminus \{s_1\}$.
    \item If $S \setminus \{s_1, s_2\} \neq \emptyset$, again, there is a least element $s_3 \in S \setminus \{s_1, s_2\}$.
    \item and so on.
  \end{itemize}
...
\end{proof}
\begin{theorem}
  The following statements are equivalent: \label{countabilityEquivalence}
  \begin{enumerate}
    \item $X$ is a countable set.
    \item There exists an injection from $X \to \N$.
    \item $X = \emptyset$ or there exists a surjection $\N \to X$.
  \end{enumerate}
\end{theorem}
\begin{proof}\par
  ...
  \begin{indentenvironment}
    If there is an injection $f: X \to \N$, then $f$ is a bijection between $X \to S = f(X)$.
    ...
    \[
      X \xrightarrow{f} f(X) = S \xrightarrow{g} \N
    \]
    is a bijection from $X \to \N$ and thus $X$ is countable.
  \end{indentenvironment}
  (\textbf{i} $\implies$ \textbf{iii})
  \begin{indentenvironment}
    If $X$ is finite, then either $X = \emptyset$ or there is a bijection (and therefore a surjection) from $\N \to X$, by definition of the size of finite sets.
    (Note that we need the case of $X = \emptyset$ as we cannot have a function $\N \to \emptyset$ as each element of $\N$ must be mapped to something).
...
\end{proof}
\begin{corollary}
  Any subset of a countable set is countable. \label{subsetOfCountableSet}
\end{corollary}
\begin{proof}
  Since $X$ is countable, by the above, there is an injection $X \to \N$.
  If $Y \subseteq X$ and $X$, we can just restrict the domain to $Y$ without affecting injectivity and so $Y$ is countable.
\end{proof}
We may now view ``countable'' as saying that a set is ``at most as big as $\N$''.
\subsection{Unions and Products of Countable Sets}
\begin{theorem}
  $\N \times \N$ is countable. \label{NxNCountable}
\end{theorem}
...
\end{proof}
\begin{remark}
  We can iterate this using induction to show that the set:
  \[
...
\end{remark}
\begin{theorem}
  A countable union of countable sets is countable. \label{countableUnion}
\end{theorem}
\begin{proof}
  ...
  Define $f: \bigcup_{n \in \N} A_n \to \N \times \N,\ x \mapsto (i, j)$ where $x = a^{(i)}_{j}$ for the \textbf{least} $i$ such that $x \in A_i$ as $x$ could be in multiple sets which we can pick by WOP.
  This is well defined as we always pick the minimal $i$ and is injective as each $x$ is associated with a unique $(i, j)$.
  We know from \cref{NxNCountable} that $\N \times \N$ is countable and so are done as we have injected into a countable set.
\end{proof}
...
\end{example}
\begin{theorem}
  The set $\mathbb{A}$ of algebraic numbers is countable. \label{ACountable}
\end{theorem}
\begin{proof}
...
\begin{proof}[Cantor -- 1891]
  ...
  \[
    ...
    \end{cases}
  \]
  This $r$ has only one decimal expansion (as it does not terminate in all 0s or 9s) and is not in the enumerated list, as $r \neq r_n\ \forall n \in \N$ as we have constructed it to disagree with $r_n$ on the $n$-th decimal digit.
...
\begin{remark}
  ...
  We can also put $(0, 1)$ in bijection with any interval $(a, b)$ with $a < b$ to conclude that all such intervals are uncountable.
\end{remark}
\begin{corollary}
...
\begin{proof}[1 -- Diagonal Arugment]
  ...
  Similarly to in \cref{RUncountable}, we wish to construct a set that disagrees with each of our sets on at least one element.
  If we define:
  \[
    S = \{n \in \N: n \notin S_n\}
  \]
  then $S$ is not on our list since $S \neq S_n\ \forall n \in \N$ as, by construction, they must disagree on the element $n$.
...
\end{proof}
\begin{remark}
  Note that we are not requiring $f$ to be surjective, as the subset $\{n \in \N: n > 5\}$ cannot be the image of any $x$ it would need to end in an infinite string of $1$s.
\end{remark}
The strategy from the first proof of this theorem can also be used to show the following:
\begin{theorem}
...
\begin{proof}[2 -- Union of Countable Sets]
  ...
  By \cref{archimedesCorollary}, for any length $\ell_i \in \R$ of $A_i$, we can find some $n \in \N$ such that $\frac{1}{n} < \ell_i$.
...
\begin{enumerate}
  ...
  \item If the set is in or ``near'' $\R$, then it is useful to try injecting into $\Q$ instead of $\N$ -- \cref{intervalsExample}
\end{enumerate}
...
\end{tabular}
\end{center}
For these interpretations to make sense, we need to check that ``$A$ is at most as big as $B$'' if and only if ``$B$ is at least as big as $A$'' so that our intuition about inequalities for sizes of finite sets still holds even with this new institution for relations between sets.
...
\begin{proof}
  \begin{proofdirection}{Suppose we have an injection $f: A \to B$}
    ...
    \[
     ...
     \end{cases}
    \]
    The first case of this map satisfies the criterion for surjectivity, however we need the second case to send elements of $B$ that were not mapped to by $f$ from $A$ so that $g$ is a well-defined function.
  \end{proofdirection}
  \begin{proofdirection}{Suppose we have a surjection $g: B \to A$}
    ...
    \]
    Such a $b \in B$ will always exist because of the surjectivity of $g$.
    It follows that $f$ is injective as $g$ is a well-defined map so must send each $b \in B$ to \textbf{exactly one} $a \in A$.
  \end{proofdirection}
\end{proof}
...
\begin{theorem}[Schr\"oder Bernstein Theorem]
  If $f: A \to B$ and $g: B \to A$ are injections, then there exists a bijection $h: A \to B$. \label{schroderBerstein}
\end{theorem}
\begin{proof}
  ...
  Finally, by a similar argument, both $f$ or $g$ provide a bijection between $A_{\infty}$ and $B_{\infty}$ as these are the points whose ancestor sequence never terminates.
