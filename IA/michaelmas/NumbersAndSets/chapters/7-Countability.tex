...
\begin{proof}[2 -- Union of Countable Sets]
  ...
  Similarly, the set $S_2 = \{i \in I: A_i \text{ has length}\geq 1/2\}$ is countable as it injects into the set of half integers which is countable by \cref{countableSubset} as it is a subset of $\Z$.
...
\begin{enumerate}
  ...
  \item Construct an injection into $\N$ -- \cref{countabilityEquivalence}
...
\begin{theorem}[Schr\"oder Berstein Theorem]
...
\begin{proof}
  ...
  Intuitively, $A_0$ is the element of $A$ such that their ancestor sequence ends in $A$ and there are similar intuitions for $A_0, B_0$ and $B_1$.
  Since each element is in exactly one of these three sets, they partition $A$ and $B$ respectively.

  Every $b \in B_1$ terminates in an odd number of steps so has a preimage under $f$ that terminates in an even number of steps and so is in $A_0$.
  Similarly, every $a \in A_0$ terminates in an even number of steps so has a preimage under $g$ that terminates in an odd number of steps and so in $B_1$.
  Thus we have a bijection between $A_0$ and $B_0$ given by $f$.

  By symmetry, $g$ bijects $B_0$ with $A_1$.

  Finally, both $f$ and $g$ bijects $A_{\infty}$ with $B_{\infty}$ as these are only the points whose ancestor sequence never terminates.
...
\begin{remark}[Remarks]
  \begin{enumerate}
    \item It would be nice to be able to say that for any sets $A$ and $B$, either $A$ injects into $B$, or $B$ injects into $A$.
      ...
      This is in fact true but it more difficult to prove, see Logic and Set Theory.
