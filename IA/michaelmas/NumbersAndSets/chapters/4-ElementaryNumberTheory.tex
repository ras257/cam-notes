...
\begin{definition}[Highest Common Factor]
  ...
  \end{enumerate}
  We denote this $c = \hcf(a, b) = \gcd(a, b)$ or sometimes $(a, b)$.
\end{definition}
\begin{example}
...
\begin{proposition}[Division Algorithm]
  For any $n, k \in \N$, we can write $n = qk + r$ for some $q, r \in \Z$ and $0 \leq r \leq k - 1$. \label{divisionAlgorithm}
\end{proposition}
...
\begin{proof}
  \induction
  {$n = 1$}{
    If $k = 1$, then $q = 1$ and $r = 0$.
    Otherwise if $k > 1$, then $q = 0$ and $r = 1$
  }
  {$n = p - 1$}{
...
\end{proof}
\begin{lemma}
  The quotient and remainder obtained for each $n$ and $k$ are unique.
\end{lemma}
\begin{proof}
...
\begin{definition}[Coprime]
  We say that $a$ and $b$ are \textit{coprime} or \textit{relatively prime} if $\hcf(a, b) = 1$.
\end{definition}
...
\begin{proof}[Using Euclid's Algorithm -- Constructive]
  ...
  But at step $n-1$ we see that $r_{n-1}$ is also expressible as $r_{n-1} = xr_{n-2} + yr_{n-3}$ for some other $x, y \in \Z$.
  We can combine these to get:
  \[
    r_{n} = xr_{n-2} + yr_{n-3} \text{ for some }x, y \in \Z
  \]
  We can continue this inductively.
  Therefore for $\forall i \in \{1, \ldots, n - 1\}$:
  \[
    r_n = xr_i + yr_{i-1} \text{ for some }x, y \in \Z
  \]
  So using steps 1 and 2, we have that $r_n = xa + yb$ for some $x, y \in \Z$.
\end{proof}
\begin{remark}
...
\begin{proof}[Existence Proof]
  ...
  \begin{enumerate}
    \item Suppose $h \nmid a$. Then by the division algorithm we can write $a = qh + r$ for $q, r \in \Z$, with $0 < r < h$.
      Hence $r = a - qh = a - q(xa + yb) = a(1-qx) - qyb$ and so $r$ is also a positive linear combination of $a$ and $b$.
      However, we also have that $0 < r < h$ which contradicts the definition of $h$.
      So $h \mid a$ and, by a similar argument, $h \mid b$ also.
...
\end{proof}
\begin{remark}
  This existence proof tells us that such coefficients exist but does not give us a way to find them.
\end{remark}
...
\begin{proof}
  ...
  \begin{proofdirection}{Assume there is a solution}
    ...
    Since $h \mid a$ and $h \mid b$, $h \mid c$.
  \end{proofdirection}
...
\begin{theorem}[Fundemental Theorem of Arithmetic]
  Every natural number $n \geq 2$ is expressible as a product of primes, unique up to reordering. \label{uniquePrimeFactorisation}
\end{theorem}
\begin{proof}
  The existence of such a factorisation follows from \cref{prodPrimes}\par
  \quietinduction
  {$n = 2$}{
    Clearly true as 2 itself is the smallest prime so there cannot be any other prime factorisations.
  }
  {$n < k$}{
    So all $n < k$ have a unique (up to reordering) prime factorisation.
  }
  {$n = k$}{
    Suppose $k = p_1 p_2 \cdots p_m = q_1 q_2 \cdots q_l$ where $p_i$ and $q_i$ are all prime.
    ...
    Thus $m=l$ and after reordering $p_2 = q_2, \ldots,\ p_k = q_k$.
    So $k$ has a unique prime factorisation.
  }
\end{proof}
\begin{remark}
  \nonexaminable
  There are arithmetical systems permitting addition and multiplication in which prime factorisations are not unique.

  For example, consider $\Z\left[\sqrt{-3}\right]$, this is all numbers of the form $x + y\sqrt{3}$ where $x, y \in \Z$.
...
\begin{enumerate}
  \item If we know a numbers prime factorisation, we can use this to find all factors of this number.
    ...
    \[
      p^{b_1}_{1} p^{b_2}_{2} \cdots p^{b_k}_{k} \text{ with } 0\leq b_i \leq a_i\ \forall i \in \{1, \ldots, k\}
    \]
  ...
  \item The common multiples of $2^{2} \cdot 3^{7} \cdot 5 \cdot 11^{3}$ and $2^{4} \cdot 3^{2} \cdot 11 \cdot 13$ are all numbers of the form $2^{a}3^{b}5^{c}11^{d}13^{e} \cdot N$ where $a \geq 4,\ b \geq 7,\ c \geq 1,\ d\geq 3,\ e\geq 1$ and $N$ is any other integer.
    ...
    \]
  \item Note that since $\min\{a_i, b_i\} + \max\{a_i, b_i\} = a_i + b_i$, we have:
    \[
...
\begin{proof}[Erd\H{o}s, 1930]
  ...
  \[
    p^{a_1}_{1} p^{a_2}_{2} \cdots p^{a_k}_{k}
  \]
  We can rewrite this as:
  \[
    m^2 p^{b_1}_{1} p^{b_2}_{2} \cdots p^{b_k}_{k} \text{ where } a_i = \begin{cases}
    0 & \text{ if $b_i$ even} \\
    1 & \text{ if $b_i$ odd}
    \end{cases}
  \]
  Let $N \in \N$.
  Given an $n \leq N$ of the above form, we must have $m \leq \sqrt{N}$ and so there are at most $\sqrt{N}$ possible values for $m$.
  We also have $2^{k}$ choices for the exponents $b_i$, and thus there is at most $2^{k}\sqrt{N}$ numbers $n$ in the above form with $n \leq N$.

  However, now if we pick $N > 2^{k}\sqrt{N}$, that is $N > 4^{k}$, then there must be a number $n \leq N$ that this not of the above form.
  So this number must have a prime factor that is not in $p_1, \ldots, p_k$.
\end{proof}
\begin{remark}
  Euclid's proof tells us that the $k$-th prime is less than $2^{2^{k}}$, however Erd\H{o}s' proof tells us that the $k$-th prime is less than $4^{k}$ (since the prime factorisation for $n$ can only contain primes less than or equal to $n$).
  In fact, the $k$-th prime occurs $\sim k \log k$.
\end{remark}
\end{document}
