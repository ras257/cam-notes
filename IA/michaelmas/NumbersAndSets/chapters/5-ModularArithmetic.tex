...
\begin{example}
  \begin{enumerate}
    ...
    \item Solve $10x \equiv 12 \pmod{34}$.
      ...
      \[
        \frac{10}{2}x \equiv \frac{12}{2} \pmod{34/2} \iff 5x \equiv 1 \pmod{17}
      \]
...
\begin{theorem}[Fermat-Euler Theorem]
  If $a \in \Z, m \in \N$ such that $\hcf(a, m) = 1$, then $a^{\varphi(m)} \equiv 1 \pmod{m}$.
  \label{fermatEuler}
\end{theorem}
\begin{proof}
...
\begin{proof}
  ...
  Note that the units modulo $p$ come in pairs whose product is 1, together with some elements that are self-inverse, that is $x$ such that $x^2 \equiv 1 \pmod{p}$.
...
\begin{proposition}
  ...
  Then $-1$ is a square modulo $p$ if and only if $p \equiv 1 \pmod{4}$.
\end{proposition}
\begin{proof}
  ...
  \begin{proofdirection}{Assume $p \equiv 1 \pmod{4}$}
    ...
    \end{align*}
    Since $p = 4k + 1$ for some $k \in \Z$, then:
    \[
...
\begin{remark}
  ...
  For example, when $p = 29$, $\frac{p - 1}{2} = 14$ so $x = 14!$ is a solution.
\end{remark}

\section{Public Key Cryptography}
\subsection{RSA Scheme}
Suppose person $A$ wants to send a message to person $B$ in an \textit{encrypted form}.
That is, person $B$ can decrypt the messages easily but a third-party cannot.
...
\end{enumerate}
In order to decrypt in this way, you need to know $n, e, \varphi(n)$.
...
Fermat's criterion is almost the converse of this:
...
\end{proposition}
Psuedoprimes are quite rare for large $p$ so this gives a very efficient way of filtering the size of the search space when finding large primes.
\end{document}
