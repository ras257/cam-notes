...
\begin{remark}
  \begin{enumerate}
    \item Note that for an increasing sequence to converge, we only required that it is \textit{bounded above} and similarly for decreasing sequences, we only required that it is \textit{bounded below}.
...
A \textit{series} is an infinite sum of terms of a sequence. Its value can be thought of as the limit of the partial sum $s_k$ as $k \to \infty$.
...
\begin{theorem}[Liouville -- 1844]
  \label{LNumber}
  The number, \textit{Liouville's Constant}, defined by:
  \[
...
\begin{proof}
  \induction
  {$d = 1$}{
    A first degree polynomial is of the form $ax + b = 0$, which has exactly one real root, namely, $x = -\frac{b}{a}$.
  }
  {$d = k - 1$}{}
  {$d = k$}{
...
\begin{proof}[\cref{LNumber}]
  Write the partial sums of $L$ as:
  \[
...
where the second inequality is due to the rapid decay of $\frac{1}{10^{k!}}$.
  ...
  \[
    L_n = \frac{s}{10^{n!}}
  \]
  for some $s \in \N$, by bringing all of the fractions in the partial sum to a common denominator.
  So:
  \[
    p(L_n) = \frac{t}{10^{d \cdot n!}}
  \]
  for some $t \in \Z$ by bring all the terms in $p(L_n)$ to a common denominator.
...
Now let $i = (0, 1)$. Then $i^2 = (-1, 0)$, corresponding to $-1 \in \R$, so $i^2 + 1 = 0$ which is what we wanted to construct.
...
\begin{remark}[Remarks]
  \begin{enumerate}
    \item $\C$ obeys all the usual rules of arithmetic, that is, it satisfies axioms \textbf{i}, \textbf{ii}, and \textbf{iii} in \cref{realDef}.
      ...
      If $z \in \C$, $z \neq 0$, then there exists $w \in \C$ such that $zw = 1$.
