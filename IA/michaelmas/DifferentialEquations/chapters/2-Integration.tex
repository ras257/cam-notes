...
\begin{theorem}[Mean Value Theorem]
  For continuous $f(x)$, the area under the \textbf{curve} from $x_n$ to $x_{n + 1}$ is:
  \[
...
\end{theorem}
\begin{proof}
  This is the integral form of the mean value theorem which will be proven in Analysis I.
\end{proof}
...
\begin{remark}[Notation]
  ...
  \[
    \int f(x) \d{x} \text{ or } \int_{}^{x} f(t) \d{t}
  \]
...
\begin{align*}
  (uv)' &= u'v + uv' \\
  \implies \int uv' \d{x} &= uv - \int u'v \d{x}
\end{align*}
\begin{example}
...
\begin{example}
  ...
  \[
    I = \int \log x \d{x}
  \]
  Let $u = \log x$, $v' = 1$ so $u' = 1/x$, $v = x$ and so:
  \begin{align*}
    I &= x \log x - \int 1 \d{x} \\
      &= x \log x - x + C
  \end{align*}
