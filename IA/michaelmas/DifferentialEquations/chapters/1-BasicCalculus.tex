...
\begin{example}[Newton's Second Law]
  ...
  \]
  This differential equation relates the rate of change of position $x$, the \textit{dependant variable}, with time $t$, the \textit{independent variable}.
\end{example}
...
\begin{remark}[Notation]
  We sometimes also use the notation $f'(x)$ or $\dot{f}(t)$.
  The dot notation is usually used to denote differentiation with respect to \textit{time}.
\end{remark}
...
\begin{example}
  ...
  If we try and find its derivative at 0:
  \[
    \at{\deriv{f}{x}}{0} = \lim_{h \to 0} \frac{|h|}{h}
  \]
  The left and right-handed limits are different ($-1$ and $1$) so $f$ is \textbf{not} differentiable at $x = 0$ but it is everywhere else.
\end{example}
...
\begin{definition}[Big-$O$ Notation]
  \begin{proofcases}
    \begin{case}{$x_0$ finite}
      $f(x)$ is $O(g(x))$ as $x \to x_0$ if $\exists \delta > 0 \text{ and } M > 0$ such that $\forall x$ with $0 < |x - x_0| < \delta$, $|f(x)| \leq M |g(x)|$.
    \end{case}
    \begin{case}{$x_0$ infinite}
      $f(x)$ is $O(g(x))$ as $x \to \infty$ if $\exists x_1 \text{ and } M > 0 \text{ such that } \forall x > x_1$, $|f(x)| \leq M|g(x)|$.
    \end{case}
  \end{proofcases}
\end{definition}
...
\begin{example}
  ...
  \begin{align*}
    |2x^3 - 4x + 12| &\leq 2|x^3| + |-4x| + 12 \\
                     &\leq 2|x^3| + 4|x^3| + 12|x^3| \\
                     &\leq 18|x^3|
  \end{align*}
  So the definition is satisfied with $x_0 = 1 \text{ and } M = 18$.
\end{example}
\begin{definition}[Little-$o$ Notation]
  ...
  \]
  The equivalence of this will apparent after rigorously studying limits in Analysis I.
\end{definition}
\begin{remark}[Notation]
  When handwritten little-$o$ is often denoted $\underline{o}(g(x))$ to differentiate it from big-$O$.
\end{remark}

$f(x) = o(g(x))$ loosely means that $f(x)$ is \textbf{much smaller} than $g(x)$ as $x \to x_0$ as we can bound $f(x)/g(x)$ by any arbitrary $\varepsilon > 0$ provided $x$ is sufficiently close to $x_0$.
\begin{example}
...
\begin{remark}[Notes]
  \begin{itemize}
    \item $f(x) = o(g(x))$ is a stronger statement that $f(x) = O(g(x))$.
      This is because little-o means that we are bounded by any multiple whereas big-O means were are bounded by some multiple.
      ...
      In Analysis I, we will see that this is a result of the fact that if $\lim_{x \to x_0} f(x)/g(x) = 0$, then $|f(x)/g(x)|$ must be bounded.
    \item Constants don't matter, that is, if $f(x) = O(g(x))$ then $af(x) = O(g(x))$ and $f(x) = O(ag(x))$ for any non-zero constant $a$ and similarly for little-$o$.
  \end{itemize}
\end{remark}
...
\begin{theorem}[Chain Rule]
 If $f(x) = F(g(x))$, then:
 \[
   \deriv{f}{x} = F'(g(x))\deriv{g}{x} = \deriv{F}{g} \deriv{g}{x}
 \]
\end{theorem}
\begin{proof}
  \begin{align*}
...
\begin{theorem}[Taylor's Theorem]
  For $f(x)$ $n$ times differentiable at $x_0$ we have: \label{taylorsThm}
  \[
    f(x_0 + h) = f(x_0) + hf'(x_0) + \frac{h^2}{2!}f''(x_0) + \cdots + \frac{h^{n}}{n!} f^{(n)} (x_0) + E_n(h)
  \]
  Where $E_n(h)$ is an ``error/remainder'' with $E_n(h) = o(h^{n})$ as $h \to 0$.

  Further, if $f^{(n+1)}(x)$ exists and is continuous $\forall x \in (x_0, x_0 + h)$ then $E_n(h) = O(h^{n+1})$ as $h \to 0$.
\end{theorem}
\begin{remark}
  $E_n = O(h^{n + 1})$ is  a stronger statement than $o(h^{n})$ as $h \to 0$.
  For example $h^{n + \frac{1}{2}}$ is $o(h^{n})$ but it is not $O(h^{n + 1})$.
\end{remark}
...
\begin{theorem}[Mean-value/Lagrange form of the remainder]
If $f^{(n+1)}(x)$ exists and is continuous $\forall x \in (x_0, x_0 + h)$ then:
\[
  E_n(h) = f^{(n+1)}(x_n) \frac{h^{n+1}}{(n+1)!} = f^{(n + 1)}(x_n)\frac{(x - x_0)^{n + 1}}{(n + 1)!}
\]
for some $x_0 < x_n < x_0 + h$.
\end{theorem}
\begin{proof}
  See Analysis I.
\end{proof}
...
\begin{example}
  ...
  \[
    E_n(h) = \frac{h^{n + 1}}{(n + 1)!}\exp(x_n)\quad 0 \leq x_n \leq h
  \]
...
\end{theorem}
\begin{proof}[Specific case with continious derivatives]
  ...
  \end{align*}
\end{proof}
L'H\^opital's rule can be generalised to higher order derivatives if we still have an indeterminate form after applying it once.
That is, if $f'(x_0) = g'(x_0) = 0$ then:
\[
  \lim_{x \to x_0} \frac{f(x)}{g(x)} = \lim_{x \to x_0} \frac{f''(x)}{g''(x)}
\]
\begin{example}
  ...
  $f''(x) = -3\sin x + 9\sin3x$, $g''(x) = 4\sin2x$ but $f''(0) = g''(0) = 0$ so we again need to try another higher order derivative.
