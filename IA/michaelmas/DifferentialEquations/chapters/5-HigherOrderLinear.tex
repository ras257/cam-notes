...
We will focus on 2nd order here, but many of these methods are also applicable to higher order differential equations.
...
\end{equation}
with $a, b, c$ constants where $\mathcal{D}y$ is a linear differential operator, defined as:
\[
...
\end{definition}
We can exploit this property to solve DEs of the form in \cref{general2ndConst}:
...
\begin{definition}[Linear Dependence]
  A set of $n$ functions $\{f_i(x)\}$ is \textit{linearly dependant} if: \label{linearDependence}
  \[
    \sum_{i=1}^{n} C_i f_i(x) = 0\ \forall x
  \]
  for $n$ constants, $C_i$, \textbf{not all} of which are $0$.
  If such constants do not exist (i.e. all of them are 0), then the functions are said to be \textit{linearly independent}.
\end{definition}
\begin{remark}
  This is the same idea as linear dependence for vectors apart from it needs to be true for all values of $x$.
\end{remark}
...
Recall that $e^{\lambda x}$ is an eigenfunction of the operator $\deriv{}{x}$:
\[
  \deriv{}{x} e^{\lambda x} = \lambda e^{\lambda x}
\]
We see that $e^{\lambda x}$ is also an eigenfunction of $\mathcal{D}$ since:
\begin{align*}
...
We then obtain the \textit{characteristic equation}:
\[
  a \lambda^2 + b\lambda + c = 0
\]
Since this is a 2nd degree polynomial it must have two roots $\lambda_1$ and $\lambda_2$ in $\C$.
\begin{proofcases}
  \begin{case}{$\lambda_1 \neq \lambda_2$}
    ...
    \]
    So the most general complimentary function is a linear combination of these two:
    \[
  ...
  \begin{case}{$\lambda = \lambda_2$ -- Degenerate Case}
    This means that we have only one linearly independent complimentary function of the form $e^{\lambda_1 x}$.
...
\begin{example}[Complex, non-degenerate roots]
  Consider the differential equation: \label{complexNonDegenerate}
  \[
    \deriv[2]{y}{x} + 4y = 0
  \]
  The characteristic equation is:
...
\begin{example}[Degenerate roots and ``detuning'']
  Consider the differential equation: \label{detuningExample}
  \[
  ...
  \textbf{Detuning -} We remove the degeneracy by considering a slightly modified (\textit{detuned}) equation.
  \[
...
\[
  ...
  \end{pmatrix}
\]
That is, given such a $\vec{Y}(x_0)$ at any fixed $x_0$, we can determine the Taylor series for the solution and use this to determine the taylor series for $y$ about $x_0$.
\begin{remark}
  \begin{itemize}
...
\begin{example}
  ...
  \end{tikzpicture}
  \end{center}
  $\vec{Y}_1$ and $\vec{Y}_2$ are linearly independent vectors for all $x$ as for an arbitrary fixed $x$, $\lambda_1 \vec{Y}_1 + \lambda_2 \vec{Y}_2 = \vec{0} \implies \lambda_1 = \lambda_2 = 0$, so since $x$ was arbitrary, they are linearly independent for all $x$.
...
\]
Since this holds for all $x$ we can differentiate it $k$ times.
Therefore:
\[
  \sum_{i=1}^{n} c_i y^{(k)}_i(x) = 0\ \forall x
\]
This sum the $k$-th entry in the sum of all the solution vectors and so we have:
\[
  \sum_{i=1}^{n} c_i \vec{Y}_i(x) = 0\ \forall x
\]
So if $\{y_i\}$ is a set of linearly dependant functions, then the set of solution vectors $\{\vec{Y}_i(x)\}$ is linearly dependant for all $x$.
...
\end{definition}
\begin{remark}[Note]
  In this case with solution vectors, the Wronskian is the determinant of the fundamental matrix, that is:
  \[
    W(x) =
    \det(\Psi(x)) =
    \begin{vmatrix}
...
Therefore we have:
\[
  \{y_i(x)\} \text{ is linearly \textbf{dependant}} \implies \{\vec{Y}_i(x)\} \text{ is linearly \textbf{dependant}}\ \forall x \implies W(x) = 0\ \forall x
\]
Taking the contrapositive, it follows that if $W(x) \neq 0$ for some $x$, then the set of functions $\{y_i(x)\}$ is linearly \textbf{independent}.
...
\begin{example}
  ...
  \]
  Since $W(0) = 2 \neq 0$, $y_1$ and $y_2$ are linearly independent.
\end{example}
...
\end{proof}
\begin{remark}
  The geometric interpretation of this is that the two solution vectors are always collinear or never collinear as $x$ varies.
\end{remark}
...
\[
  \frac{y_1 y_2' - y_2 y_1'}{y^2_1} = \frac{W(x_0)}{y^{2}_{1}}\exp\left[-\int_{x_0}^{x} p(u) \d{u}\right]
\]
...
\begin{remark}[Nomenclature]
  \begin{itemize}
    \item The name \textit{equidimensional} originates from the fact that the quantities on the left hand side have consistent physical dimensions.
    \item Equations of this form are also sometimes referred to as \textit{Euler-Cauchy} equations.
  \end{itemize}
\end{remark}
\subsection{Scaling Property}
If it is homogeneous, then we have a scaling property of solutions.
\begin{proposition}
  If $g(x)$ is a solution of \cref{generalEquidimensional} with $f(x) = 0$ (i.e. it is homogeneous), then so is $y = g(\alpha x)$ where $\alpha$ is a constant.
\end{proposition}
\begin{proof}
  Setting $y(x) = g(\alpha x)$, we have:
  \begin{align*}
    \deriv{y}{x} = \deriv{}{x}(g(\alpha x)) &= \alpha g'(\alpha x) \\
  ...
  \begin{align*}
    ax^2 \deriv[2]{y}{x} + bx \deriv{y}{x} + cy &= a (\alpha x)^2 g''(\alpha x) + b(\alpha x)g'(\alpha x) + cg(\alpha x) \\
                                                &= au^2 g''(u) + bu g'(u) + cg(u) \text{\quad($u = \alpha x$)}\\
                                                &=0 \text{ as $g(u)$ is a solution}
  \end{align*}
  so $y(x) = g(\alpha x)$ is also a solution.
\end{proof}
...
\[
  ak^2 + k(b - a) + c = 0
\]
In general this will have two roots $k_1$ and $k_2$.
If $k_1 \neq k_2$, then the general complimentary function is:
\[
  y_c = Ax^{k_1} + Bx^{k_2}
\]
with $A, B$ constants.
\subsection{Solving By Substitution} \label{equidimensionalSub}
If we substitute $z = \ln x$, we obtain:
\[
...
\begin{example}
  ...
  \]
  The most general complimentary function is:
  \[
...
In the non-resonant case, if $f(x) \propto x^{m}$, then we should try a particular integral of the form $Cx^{m}$ for $m \neq k_1$ and $m \neq k_2$.
In the resonant case, i.e. $f(x) \propto x^{k_1} \text{ or }x^{k_2}$, then the particular integral is:
\[
  y_p \propto (\ln x) x^{k_1}
\]
This follows from our earlier transformation by $z = \ln x$ in \cref{equidimensionalSub}.
From we know that for the degenerate with constant coefficients the particular integral is $y_p = ze^{k_1 z}$.
This then becomes $y_p = (\ln x)x^{k_1}$ after substituting back in with $x$.
\begin{remark}[Note]
  If the homogeneous equation is also degenerate (i.e. $k_1 = k_2$), we may need to try:
  \[
    y_p = (\ln x)^2 x^{k_1}
  \]
\end{remark}
...
\begin{example}
  ...
  \end{align*}
  Since $\sin2x$ is a complimentary function, it can be removed, thus $y_p = -\frac{1}{4}x\cos 2x$ which is $x \times (\text{Non-resonant P.I.})$, as expected.
\end{example}
...
\begin{proofcases}
  \begin{case}{Light Damping, $\kappa < 1$}
    ...
    \begin{align*}
      ...
      \implies y(t) &= e^{-\kappa \omega_0 t}\left[A\sin\left(t \omega_0\sqrt{1-\kappa^2}\right) + B\cos\left(t \omega_0\sqrt{1 - \kappa^2}\right)\right] \\
    \end{align*}
...
If we add a forcing term, initially the solution will be similar to C.F. + P.I, this is the \textit{transient response}.
...
\begin{example}
  ...
  \]
  Notice how the amplitude of $y_p$ \textbf{does not} depend on time.
...
\end{tikzpicture}
\end{center}
This kind of force could occur when, for example, modelling striking a mass on a spring or a car going over a kerb.
\begin{example}
  ...
  \]
  We see that, if $y$ is finite and continuous, as $\varepsilon \to 0^{+}$, the only term on the left hand side that does not vanish is the first term:
  \[
    \lim_{\varepsilon \to 0^{+}} \left(m \eval{\dot{y}}{T-\varepsilon}{T+\varepsilon}\right) =I
  \]
  So if $I \neq 0$, the velocity $\dot{y}$ is discontinuous at $t = T$.
...
\begin{remark}[Note]
  The continuity of $y$ at $x = 0$ is required otherwise $y'$ would be like $y' \propto \delta(x)$ with $y''$ being even worse behaved and so it would not be possible for it to be a solution to the DE.
...
\begin{example}
  ...
  \[
    y = A \cosh x + B \sinh x
  \]
  Applying $y(0) = 0$ we see that $B = 0$ thus so $y = A \sinh x$ for some constant $A$.

  For $\frac{\pi}{2} < x \leq \pi$, we have the same DE but a different boundary condition, $y(\pi) = 0$.
  In this case, noticing that $\sinh \pi$ and $\cosh \pi$ are not particularly nice to deal with, we can write the general form of $y'' - y = 0$ as:
  \[
    y = C \cosh(x - \pi) + D \sinh(x - \pi)
  \]
  and so $y(\pi) = 0 \implies C = 0$ so $y = D\sinh(x - \pi)$ for some constant $D$.

  For $y$ to be continuous at $\pi / 2$, we need to join the solutions at $x = \frac{\pi}{2}$.
  \[
    A\sinh(\pi/2) = -D\sinh(\pi/2) \implies D = -A
  \]

  Now applying the jump condition:
  \begin{align*}
    \lim_{\varepsilon \to 0^{+}} \eval{y'}{\pi/2 - \varepsilon}{\pi/2 + \varepsilon} &= 3 \\
    -A\cosh(-\pi/2) - A\cosh(\pi/2) &= 3 \implies A = -\frac{3}{2\cosh(\pi/2)}
  \end{align*}
  ...
  From the plot, we can see that $y$ is continuous at $\pi/2$ but $y'$ is not, as expected:
  \begin{center}
...
\[
  \eval{y''}{-\varepsilon}{\varepsilon} + \eval{p(x)y'}{-\varepsilon}{\varepsilon} + \eval{q(x)y}{-\varepsilon}{\varepsilon} = \eval{H(x)}{-\varepsilon}{\varepsilon} = 1
\]
...
\]
If we try $y^{(c)}_n \propto k^{n}$, then we obtain the \textit{characteristic equation}:
\begin{align*}
  ak^{n + 2} + bk^{n + 1} + ck^{n} = 0 \\
  \implies ak^2 + bk + c = 0
\end{align*}
which has roots $k_1$ and $k_2$.
The general complimentary function is then:
\[
  y^{(c)}_n  = \begin{cases}
  ...
  \end{cases}
\]
In the case of $k_1 = k_2$, this is similar to we had $y_c = Ae^{\lambda x} + Bxe^{\lambda x}$ for degenerate second order ODEs.
...
$k^{n}$ & $Ak^{n}$ if $k \neq k_1 \text{ or }k_2$ \\
$k^{n}_{1}$ & $Ank^{n}_{1}$ \\
$n^{p},\ p \in \N_0$ & $An^{p} + Bn^{p - 1} + \cdots + Cn + D$
\end{tabular}
\end{center}
\begin{example}[Fibonacci Sequence]
  ...
  \[
    k^2 - k - 1 = 0 \iff k = \frac{1 \pm \sqrt{5}}{2}
  \]
  ...
  \]
  Since $\phi_1 > 1$, $y_n \sim \frac{\phi^{n + 1}_1}{\sqrt{5}}$ as $n \to \infty$ and so:
  \[
    \lim_{n \to \infty} \frac{y_{n + 1}}{y_n} = \phi_1
  \]
  That is, the ratio between successive terms approaches a constant $\phi_1$.
\end{example}
...
\begin{example}
  ...
  To check if it is a regular singular point:
  \[
    x \cdot \frac{q}{p} = -\frac{2x^2}{1 + \sqrt{x}}
  \]
...
\begin{theorem}[Fuch's Theorem]
  \begin{enumerate}\label{fuchTheorem}
    \item
    ...
    \item
      ...
      \]
      where $\sigma \in \C$ and $a_0 \neq 0$ (as if we had $a_0 = 0$, the sum would effectively start at $n = 1$ so we could re-index and so $\sigma$ would not be unique).
