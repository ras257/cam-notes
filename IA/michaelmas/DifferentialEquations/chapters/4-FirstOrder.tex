...
\]
This means that it acts similarly to exponentiation and so we write $\exp(x) = e^{x}$ where:
\[
  e = \exp(1) = \lim_{k \to \infty} \left(1 + \frac{1}{k}\right)^{k}
\]
The inverse function of the exponential function is the natural logarithm, $\log x$, $\ln x$, or $\log_{e} x$, where $\exp(\log x) = x$.
It follows that $a^{x} = (e^{\ln a})^{x} = e^{x \log a}$ and thus $\deriv{}{x}a^{x} = (\log a) e^{x \log a} = a^{x} \log a$.
...
\begin{remark}[Number of Solutions]
  Any $n$-th order linear ODE has $n$ independent solutions, so we expect first order linear ODEs to have only 1 independent solution.
  We will properly define the notion of independent functions later in \cref{linearDependence}.
\end{remark}
...
\begin{remark}
  For any linear homogeneous ODE, any constant multiple of a solution is also a solution as we can just cancel the factor on both sides.
\end{remark}
...
Solutions of linear homogeneous ODEs with constant coefficients (for any order) are linear combinations of functions of the form $x^{n}e^{\lambda x}$ where $n \in \N_0$ and $\lambda \in \C$.
\begin{example}
  \[
    5\deriv{y}{x} - 3y = 0
  \]
  ...
  \[
    5\lambda - 3 = 0 \implies \lambda = \frac{3}{5} \implies y = Ae^{\frac{3x}{5}}
  \]
  This is called the ``general solution''.
  To specify a unique solution, we need to apply suitable boundary condition(s).
  In general, we need $n$ boundary conditions to get a specify a unique solution for an $n$-th order ODE.

  For example if we had the boundary condition $y(0) = y_0$, then $A = y_0$ and so the solution is $y = y_0 e^{3x/5}$.
\end{example}
...
\begin{example}
  ...
  We can then approximate the equation by a discrete form at some set of points $\{x_n\}$ with $x_n = x_0 + nh$.
  Here, we will take $x_0 = 0$.
  ...
  \end{align*}
  Now take $x_n = x$, i.e. after we have taken $n$ we have gone from $x_0 = 0$ to $x_n = x$.
  We then take $n \to \infty$ (and therefore also $h \to 0$ so that $nh = x$ remains fixed at $x$):
  \[
    y(x) = \lim_{n \to \infty} y_n = \lim_{n \to \infty} y_0 \left(1 + \frac{3x}{5n}\right)^{n} = y_0 \exp\left(\frac{3x}{5}\right)
  \]
  which agrees with solving the continuous case directly.
\end{example}
...
\begin{example}
  ...
  \end{align*}
  This is true for all $x$ so $5na_n - 3a_{n-1}$ must be zero for all $n \geq 1$.
  Thus:
  \begin{align*}
  ...
  \]
  which agrees with the other two methods.
\end{example}
\section{Forced ODEs}
\begin{definition}[Forced]
  ...
  Such ODEs are often also called \textit{non-homogeneous} or \textit{inhomogeneous}.
\end{definition}
\begin{remark}
  Unlike with homogeneous ODEs, $y = 0$ is \textbf{not} always a solution to a forced ODE.
\end{remark}
\subsubsection{Method To Solved Forced ODEs}
\begin{enumerate}
  \item Find \textbf{any} solution of the full forced equation.
    This gives you a particular integral (P.I.), $y_p(x)$.
  \item Find complimentary function (C.F.), $y_c(x)$, that satisfies the corresponding homogeneous equation.
  \item We then have that $y(x) = y_p(x) + y_c(x)$ satisfies the full equation.
    Since we know it will only have one independent solution, we are done.
\end{enumerate}
...
\begin{example}
  Consider the ODE $5y' - 3y = 10$.
  We call 10 the ``constant forcing term''.

  We first guess that $y_p(x)$ is a constant, say $\mu$.
  Substituting in, we see that $\mu = - \frac{10}{3}$ and so $y_p(x) = - \frac{10}{3}$.
  We know already that $y_c(x) = Ae^{\frac{3x}{5}}$ is the solution of the homogeneous equation $5y' - 3y = 0$.
...
\begin{example}[Radioactive Decay]
  ...
  \begin{align*}
    ...
    \label{radioactive}
  \end{align*}
  Try a P.I., $b_p(t) = \beta e^{-k_a t}$, and for now, assume that $k_a \neq k_b$:
  \begin{align*}
    (k_b - k_a)\beta e^{-k_a t} &= k_a a_0e^{-k_a t} \\
    \implies \beta &= \frac{k_a a_0}{k_b - k_a}\quad(\text{ as } k_a \neq k_b)
  \end{align*}
  $b_c(t)$ solves the homogeneous equation $b_c' + k_b b_c = 0$ so $b_c(t) = De^{-k_b t}$.
  ...
  \end{tikzpicture}
  \end{center}
  If $k_a = k_b$, we need to use a different P.I., this will be discussed later in \cref{differentPI}.
\end{example}
...
\]
or the more common ``standard-form'':
\[
  \deriv{y}{x} + p(x)y = f(x)
\]
...
So we require that:
\[
  \frac{\mu'}{\mu} = p \implies \int p \d{x} = \int \frac{\mu'}{\mu} \d{x} = \log \mu \quad\text{(Up to a constant)}
\]
and so:
\[
  \mu(x) = \exp\left[\int^x P(u) \d{u}\right]
\]
which is unique up to an irrelevant constant factor as this multiple would just cancel out on either side of the DE.

Since the left hand side is now the derivative of $\mu y$, we can hypothetically integrate both sides:
\[
...
\begin{example}
  ...
  So the I.F. is:
  \begin{align*}
    \log \mu &= \int_{}^{x} \left(\frac{1}{u} - 1\right) \d{u} \\
            &= \log x - x \\
            &= \log(xe^{-x}) \\
    \implies \mu &= xe^{-x}
  \end{align*}
  ...
  \end{align*}
  If we require that $y$ is finite as $x \to 0$, then we would need to set $C = 1$.
\end{example}
  ...
  \begin{proofcases}
    \begin{case}{$k_b \neq k_a$}
      \begin{align*}
        ...
        \implies  b(t) &= \underbrace{\frac{k_0 a_0}{k_b - k_a}e^{-k_a t}}_{\text{P.I.}} + \underbrace{Ce^{-k_b t}}_{\text{C.F.}}
      \end{align*}
    ...
    \begin{case}{$k_a = k_b$}
      ...
      \begin{align*}
        ...
        \implies b(t) &= \underbrace{k_a a_0 t e^{-k_b t}}_{\text{P.I.}} + \underbrace{Ce^{-k_b t}}_{\text{C.F.}}
      \end{align*}
      So $\text{P.I.} \propto t e^{-k_b t}$ not just $e^{-k_b t}$.
      Had we tried $\beta t e^{-k_b t}$ earlier we would have gotten this result, however using an integrating factor like this removes some of the guesswork.
      \begin{center}
...
\begin{definition}[Exact]
  ...
  \]
\end{definition}
If \cref{nonLinearGeneral} is exact, then $\d{f} = 0$ and so $f(x, y) = \text{constant}$ is a solution.
If it is exact, then by the multivariate chain rule (\cref{multiChainRule}) we have:
\[
  \d{f} = \underbrace{\pderiv{f}{x}}_{P(x, y)} \d{x} + \underbrace{\pderiv{f}{y}}_{Q(x, y)} \d{y}
\]
So we need to solve $\pderiv{f}{x} = P$ and $\pderiv{f}{y} = Q$.
We know that partial derivatives commute so we must have:
\[
...
\[
  \text{ODE is exact} \implies \pderiv{P}{y} = \pderiv{Q}{x}
\]
In fact, the converse of this turns out to be true:
\begin{theorem}
...
\end{theorem}
\begin{proof}
  See IA Vector Calculus section 5.1.3.
\end{proof}
...
\end{definition}
\begin{example}
Here are some examples and non-examples of some path connected and simply connected domains:
\begin{center}
...
\begin{example}
  ...
  \]
  So in a simply connected domain, this ODE is exact.

  We can proceed by either integrating $P$ with respect to $x$ or integrating $Q$ with respect to $y$.
  Here we will use $P$:
  \begin{align*}
    ...
    \implies f(x, y) &= \int  (2x - 3y^2) \d{x} \\
                     &= x^2 - 3xy^2 + h(y)
  \end{align*}
  We obtained a constant of integration that is a function of $y$ as $\pderiv{}{x}h(y) = 0$.
  ...
  \begin{align*}
    \at{\pderiv{f}{y}}{x} &= \pderiv{}{y}(x^2 - 3xy^2 + h(y)) \\
                          &= -6xy + \deriv{h}{y}
  \end{align*}
  Equating the two expressions for $\at{(\partial f)/(\partial y)}{x}$, we have:
  \begin{align*}
...
However, we can analyse the behaviour of solutions without actually needing to solve by utilising graphical methods.
\subsection{Solution Curves}
Consider $\deriv{y}{t} = f(t, y)$.
Each initial condition (i.e. $y(t_0) = y_0$) generates a distinct \textit{solution curve} or \textit{trajectory}.

For example with $y = y_0 e^{x}$, a few solution curves are:
\begin{center}
\begin{tikzpicture}[scale=1.13]
...
\end{center}
\begin{remark}
  Solution curves cannot cross (in the sense of intersecting with different tangents and not just touching) if $\deriv{y}{t} = f(t, y)$ is single valued, as if they did, $\deriv{y}{t}$ would take multiple different values at a single point.
\end{remark}
We can sketch families of solution curves without needing to solve the ODE using the expression for $\deriv{y}{t}$.
\begin{example}
  Consider the ODE:
  \[
    \deriv{y}{t} = t(1 - y^2)
...
It is tangent to solution curves.
\begin{example}
  Consider again $\dot{y} = t(1-y^2)$.
  For $t > 0$, $\dot{y} > 0$ for $|y| < 1$ and $\dot{y} < 0$ for $|y| > 1$.
  We can see this behaviour in the slope field:
  \begin{center}
  \begin{tikzpicture}[scale=1.13]
    \def\f(#1,#2){(#1)*(1-(#2)^2)}
    \def\len{0.25}
...
Isoclines are often useful guides when sketching solution curves.
\begin{example}
...
\begin{definition}[Fixed Point]
  ...
  That is, $f(c, t) = 0 = \deriv{}{t}(c)$ for all $t$.
\end{definition}
\begin{definition}[Stable Fixed Point]
  A fixed point $y = c$ is \textit{stable} if whenever $y$ deviates slightly from $c$, then $y$ \textbf{converges} to $y = c$ as $t \to \infty$.
\end{definition}
\begin{definition}[Unstable Fixed Point]
  A fixed point $y = c$ is \textit{unstable} if whenever $y$ deviates slightly from $c$, then $y$ \textbf{diverges} from $y = c$ as $t \to \infty$.
\end{definition}
\begin{example}
  Again with $\dot{y} = t(1-y^2)$, we have fixed points at $y = \pm 1$.
  However they have very different character.
  Visually, we see that solution curves approach $y = 1$ as $t \to \infty$ so $y = 1$ is a \textit{stable} fixed point but diverge from $y = -1$ as $t \to \infty$ so $y = - 1$ is an \textit{unstable} fixed point.
\end{example}
\subsection{Perturbation Analysis and Stability}
\label{pertubationSec}
Let $y = c$ be a fixed point of $\deriv{y}{t} = f(t, y)$, that is $f(t, c) = 0$ for all $t$.
...
\[
  \deriv{\varepsilon}{t} \approx \varepsilon \pderiv{f}{y}(t, c)
\]
...
As $t \to \infty$, if $\varepsilon \to 0$ then it is a stable fixed point, if instead $|\varepsilon| \to \infty$ then it is an unstable fixed point.
\begin{example}
  ...
  \[
    ...
    \varepsilon(t) = \varepsilon_0 e^{t^2}
  \]
  As $t \to \infty$, $|\varepsilon| \to \infty$ so the solution curve diverges from $y =-1$ and so it is an \textit{unstable} fixed point.

  This analysis is only for small $\varepsilon$, so we only really know what happens to solution curves very close to the fixed point solutions.
  All we are asking is if we perturb slightly away from the constant solution, does the solution come back towards or drift away from the fixed point solution.
\end{example}
\section{Autonomous Systems and Phase Portraits}
\label{autoAndPhaseSec}
\begin{definition}[Autonomous System]
  An \textit{autonomous} system is a special case of an ODE where $t$ does not appear explicitly, i.e. it can be written as:
  \[
    \deriv{y}{t} = f(y)
  \]
\end{definition}
...
We can also see this property directly from the ODE; since the slope of the solution does not depend on the $t$ coordinate, any two initial conditions with the same $y_0$ value will evolve in the same way, regardless of the initial $t_0$ and so will be horizontal translations of each other.

Although it is separable the integral may not have a closed form and so we may still wish to analyse fixed points.
For a fixed point $y = c$, we need $f(c)$.
Again, perturbing by a small $\varepsilon(t)$ about the fixed point with $y(t) = c + \varepsilon(t)$, we have:
\[
  \deriv{\varepsilon}{t} \approx \varepsilon \underbrace{\deriv{f}{y}(c)}_{\text{constant} k} = \varepsilon k \implies
  \varepsilon(t) \approx \varepsilon_0 e^{kt}
\]
If $k = f'(c) < 0 \longrightarrow$ stable fixed point.
If $k = f'(c) > 0 \longrightarrow$ unstable fixed point.
\begin{example}[Chemical Kinetics]
  ...
  \end{tabular}
  \end{center}
  where if we react one molecule of A with one of B, we get one of C and one of D.

  Since one $A$ produces one $C$ and one $B$ produced one $C$ we have:
  \begin{align*}
  ...
  \end{align*}
  which is an autonomous non-linear first-order ODE.
  We require $f(c) = 0$ for a fixed point and so there are fixed points at $c = a_0$ and $c = b_0$.
  These correspond to when all of A has been consumed and when all of B has been consumed respectively.

  Assume that $a_0 < b_0$.
  In this case, the fixed point $c = b_0$ is unphysical as if we start with less $A$ than $B$ then there is no way we can use up all of $B$ before $A$ in this reaction.

  We now take the derivative of $f(c)$ to analyse the stability of the fixed points:
  \[
    ...
    \end{cases}
  \]
  So in the case of $a_0 < b_0$, $c = a_0$ is a \textit{stable fixed point} and $c = b_0$ is \textit{unstable}.
  \begin{center}
  \begin{tikzpicture}[scale=1.2]
  ...
  \end{tikzpicture}
  \end{center}
  From this we can see that if we have more $C$ than $a_0$, $\deriv{c}{t} < 0$ so although it is unphysical, the amount of $C$ decreases, and if we have less than $a_0$, $\deriv{c}{t} > 0$ so more is produced.
  On the other hand, if we have more than $b_0$, more is produced and if we have less than $b_0$ then the amount of $C$ decreases.
  In both cases $c$ moves away from $b_0$.
...
\begin{example}[Population Dynamics -- Logistic Equation]
  ...
  \end{align*}
  for constants $\lambda = \alpha - \beta$ and $Y = \frac{\alpha - \beta}{\gamma}$.
  This is called the differential logistic equation and is an autonomous ODE.

  $f(y) = 0 \implies y = 0 \text{ or } y = Y$ and so the fixed points are $y = 0$ and $y = Y$.
  ...
  \[
    ...
    \end{cases}
  \]
  For $\lambda > 0$ (i.e. birth rate $>$ isolated death rate, $\alpha > \beta$), the fixed point $y = 0$ is unstable and the fixed point $y = Y$ is stable:
  \begin{center}
  \begin{tikzpicture}[scale=1.2]
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  We can in fact solve this ODE.
  If we solve it and plot solution curves, we see that the result agrees with our stability analysis:
  \begin{center}
  \begin{tikzpicture}[scale=1.13]
    \def\yconst{1.5}
    \def\lconst{2.5}
...
\begin{definition}[First Order Discrete Equation]
  \textit{First order} discrete equation is an equation relating $x_n$ and $x_{n + 1}$ of the form:
  \[
    x_{n + 1} = f(x_n)
  \]
  Note that $f(x_n)$ does not explicitly depend on $n$, similarly to in an autonomous system.
\end{definition}
\begin{definition}
  A \textit{fixed point} of a first order discrete equation is a value of $x_n$ such that $x_{n + 1} = x_n$, that is $f(x_{n}) = x_{n}$.
\end{definition}
...
\begin{align*}
  ...
  \implies \varepsilon_{n+1} & =\varepsilon_n \deriv{f}{x}(x_F) + O(\varepsilon^{2}_{n})
\end{align*}
Since $\varepsilon$ is small we can linearise this as:
\[
...
From this relationship, we see that $|f'(x_F)| < 1 \implies \text{$x_F$ stable fixed point}$ and $|f'(x_F)| > 1 \implies \text{$x_F$ unstable fixed point}$.
\begin{example}
  ...
  \end{tikzpicture}
  \end{center}
  Since we are requiring that $x_n \geq 0$, we need to make sure that every $x_n$ is in the range $0 \leq x_{n} \leq 1$ otherwise $f(x_n) = x_{n+1}$ would be negative and so we require $0 \leq r \leq 4$.
  ...
  \[
    ...
    \end{cases}
  \]
  So $x_n = 0$ is stable for $0 < r < 1$ but is unstable for $r > 1$ and $x_n = 1 - \frac{1}{r}$ is stable for $1 < r < 3$ but is unstable for $r > 3$ or $r < 1$.

  We can illustrate the behaviour of discrete equations using \textit{cobweb diagrams}.
  If we take $0 < r < 1$, then we expect to see only one fixed point at 0:
  \begin{center}
  \begin{tikzpicture}[scale=1.2]
  ...
  We also see that this solution is attracted towards 0 which is consistent with 0 being a stable fixed point for $0 < r < 1$.

  If we instead take $1 < r < 2$, then we obtain an unstable fixed point at $0$ and a stable fixed point at $1 - \frac{1}{r}$:
  \begin{center}
  \begin{tikzpicture}[scale=1.2]
    \draw[->] (0, 0) -- (7, 0) node[right] {$x_n$};
    \draw[->] (0, 0) -- (0, 4) node[right] {$x_{n+1}$};
    \node[below] at (6.66, 0) {$1$};
    \node[below] at (0, 0) {$0$};
    \def\func#1{1.6*#1*(1 - 0.15*#1)}

    \draw (0, 0) -- (4, 4) node[right] {$x_{n + 1} = x_n$};

    \draw[thick, smooth, samples=100, domain=0:6.66] plot ({\x}, {\func{\x}});

    \pgfmathsetmacro{\xcurrent}{6}
    ...
    \node[below] at (2.4, 0) {$\scriptstyle \cdots$};
  \end{tikzpicture}
  \end{center}
  We can see how this solution moves away from the unstable fixed point at 0 and towards the stable fixed point at $1 - \frac{1}{r}$.
\end{example}
\end{document}
