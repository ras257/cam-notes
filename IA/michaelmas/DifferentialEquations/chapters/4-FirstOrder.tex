...
\begin{example}
  ...
  \end{align*}
  Now take $x_n = x$ ($n$ steps from $x = 0$ to $x$) as $n \to \infty$ (and $h \to 0$):
  \[
...
Let $y = c$ be a fixed point of $\deriv{y}{t} = f(t, y)$.
...
\[
  \deriv{\varepsilon}{t} \approx \varepsilon \underbrace{\deriv{f}{y}(c)}_{\text{constant}} = \varepsilon k \implies
...
\begin{example}
  ...
  If we instead take $1 < r < 2$, then we get an unstable fixed point at $0$ and a stable fixed point at $1 - \frac{1}{r}$:
  \begin{center}
  \begin{tikzpicture}[scale=1.2]
    \draw[->] (0, 0) -- (7, 0) node[right] {$x_n$};
    \draw[->] (0, 0) -- (0, 4) node[right] {$x_{n+1}$};
