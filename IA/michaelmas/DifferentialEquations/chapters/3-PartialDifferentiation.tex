...
Our goal is to generalise differentiation to functions of more than one independent variables, known as \textit{multivariate functions}.
\begin{example}[Multivariate Functions]
  Examples of when we might need a multivariate function are:
  \begin{itemize}
  ...
  \end{itemize}
\end{example}
To visualise multivariate functions, it is often useful to to draw a contour plot.
We do this by drawing contours where $f(x, y) = \text{constant}$, like a topographic map:
\begin{center}
...
\end{tikzpicture}
\end{center}
Because these functions now depend on multiple independent variables, the slope at the point $A$ depends on which direction we are considering.
...
\begin{definition}[Partial Derivative]
  ...
  \]
  This is the slope of $f$ when moving in the $x$ direction whilst keeping $y$ fixed.
\end{definition}
\begin{example}
...
\begin{remark}[Notation]
  If we omit the ``evaluated at'' bar $\at{}{x}$ then we implicitly assume that all other variables are being held fixed.
\end{remark}
\begin{remark}[Alternative Notation]
  \[
    f_x \equiv \pderiv{f}{x}, f_{xy} \equiv (f_x)_y \equiv \frac{\partial^2 f}{\partial y \partial x}
  \]
...
\label{multivariateChainRule}
Given a path $(x(t), y(t))$ through the $(x, y)$-plane and $f(x, y)$ what is $\deriv{f}{t}$ along this path?
...
\begin{theorem}[Integral Form of the Multivariate Chain Rule]
  \[
    \Delta f = \int  \d{f} = \int \pderiv{f}{x} \d{x} + \int \pderiv{f}{y} \d{y}
...
This is explored further in Vector Calculus.
...
For example, it might be useful to convert from Cartesian $(x, y)$ to plane polar $(r, \theta)$.
...
\begin{example}
  ...
  \begin{equation}
    ...
    \label{implicitEx}
  \end{equation}
  We cannot find $z$ as a function of $x$ and $y$ but we can still evaluate partial derivatives.
...
\begin{remark}[Warning]
  ...
  This is important as on the surface we have $\at{\partial f/\partial x}{y} = 0$.
  However, in general, $\at{\partial f/\partial x}{y, z} \neq 0$ as we cannot vary $x$ whilst keeping $y$ and $z$ fixed and expect to stay on the surface where $\d{f} = 0$.
\end{remark}
If we want to find $\at{(\partial z/\partial x)}{y}$ \textbf{on the surface} we first start by holding $y$ fixed:
\begin{align*}
...
\]
We can carry out a similar process to obtain:
\[
...
\]
From this it follows that for any arbitrary surface with $f(x, y, z) = c$ that \textbf{on the surface}:
\[
...
\[
  \at{\pderiv{x}{z}}{y} = -\frac{\at{(\partial f/\partial z)}{x, y}}{\at{(\partial f/\partial x)}{y, z}} = \frac{1}{\at{(\partial z/\partial x)}{y}}
\]
\end{proposition}
\begin{remark}[Warning]
