  ...
  \begin{case}{$\lambda_1$ and $\lambda_2$ are real and have the same sign}
    ...
    \begin{proofcases}
      ...
      \begin{case}{$\lambda_1, \lambda_2 < 0$ -- Stable Node}
        Same as in the previous case but with the arrows reversed and now the term with $e^{\lambda_2 t}$ dominates as $|\lambda_2| < |\lambda_1|$.
...
\begin{example}[Forced Wave Equation]
  ...
  \begin{align*}
    \deriv{\psi}{t} &= \pderiv{\psi}{t}  + \pderiv{\psi}{x} \deriv{x}{t} \\
                    ...
                    &= e^{-t}
  \end{align*}
  Integrating with respect to $t$, we have $\psi(x, t) = f(x_0) - e^{-t}$.
...
\begin{example}
  ...
  \[
    \eta \equiv \frac{x}{2\sqrt{\kappa t}}
  \]
  ...
  \]
  If we plot this solution over time (lighter plot for larger $t$), we see that it tends towards a uniform temperature of $T = 1$ along the whole rod as $t \to \infty$.
  \begin{center}
