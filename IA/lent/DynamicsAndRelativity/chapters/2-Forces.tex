...
Once there is more than one particle in in the universe, there will be \textit{interactions} between these particles.
...
\end{definition}
Mass is an additional property of point particles.
It could change with time, but we typically assume that it is constant unless otherwise specified.
\begin{law}[Newton's Second Law]
  ...
  \label{newton2}
\end{law}
The force $\vec{F}$ depends on the interaction, however, we usually require that it must only depend on $\vec{x}$ and $\dot{\vec{x}}$.
This means that Newton's 2nd law is a second order differential equation for $\vec{x}(t)$.
Thus, given $\vec{x}(t_0)$ and $\dot{\vec{x}}(t_0)$ of all particles in a system at some time $t_0$, Newton's equations uniquely determine $\vec{x}(t)$ for all future and past times.
\begin{remark}[Remarks]
  \begin{itemize}
    \item Forces \textit{can} depend on higher derivatives (for example, the \textit{Abraham-Lorentz force} which depends on $\dddot{\vec{x}}$) however we do not consider such forces in classical mechanics so that Newtons 2nd Law remains an explicit equation and is also a well posed initial value problem.
    \item Forces can also explicitly depend on time, however, we will not consider such forces within this course.
    \item Newtonian mechanics has been superseded by both quantum mechanics (generally, small things) and relativity (generally, fast things), but nevertheless accurately describes much of the universe.
  \end{itemize}
\end{remark}
...
\begin{remark}
  The above calculation effectively derives the Vector Calculus identity $\nabla V(r) = \frac{V'(r)}{r} \vec{r}$.
  Noting that $\nabla$ is invariant under changes of origin, applying this to the gravitational potential energy yields the result for $\vec{F}$ immediately.
\end{remark}
The force is going in the \textbf{opposite} direction to $\uvec{r}$ so the force acting on the particle of mass $m$ at $\vec{x}$ is attractive towards the particle of mass $M$ at $\vec{x}_0$, as expected.

Sometimes we write $V = m \Phi$ where $\Phi = -\frac{GM}{|\vec{x} - \vec{x}_0|}$ is called the \textit{gravitational potential}.
This is useful as then $\Phi$ does not depend on $m$.
...
Using this approximation, when we take the gradient, the constant drops out:
\begin{equation}
  \nabla \Phi = g\uvec{z} \implies \nabla V = mg\uvec{z} \label{uniformGravitationalPot}
\end{equation}
So near the surface of the Earth, $\vec{F} = -\nabla V = -mg\uvec{z}$, which is a constant downwards force.

This force leads to the simplest nontrivial example of motion due to a force.
Applying Newton's 2nd Law (\cref{newton2}) we have that:
\[
...
This is the standard equation for free fall, as expected.
\begin{remark}[Assumptions]
  \begin{itemize}
    ...
    \item We ignored non-inertial effects as technically the Earth is accelerating and so is not \textit{technically} an inertial frame.
  \end{itemize}
\end{remark}
\subsection{Energy}
\label{energySec}
\begin{proposition}[Conserved Energy for Conservative Forces]
...
\begin{example}[Escape Velocity]
  ...
  We call the minimal velocity the object must have to achieve this the \textit{escape velocity}.
...
\begin{proof}[2]
  ...
  \begin{align*}
    W &= \int_{C} \vec{F} \cdot \d{\vec{x}} \\
      &= - \int_{C} \nabla V \cdot \d{\vec{x}} \text{ as $\vec{F}$ is conservative} \\
      &= V(\vec{x}_1) - V(\vec{x}_2)
  \end{align*}
  which again only depends on the endpoints $\vec{x}_1$ and $\vec{x}_2$.
\end{proof}
\begin{definition}[Power]
  The \textit{power} is defined as: \label{powerDef}
  \[
...
\begin{remark}[Charge]
  ...
  For an electron $q \approx \qty{-1.6e-19}{\coulomb}$ which is the negation of the \textit{elementary charge} $e$.
\end{remark}
...
This is why we require that we are working with electrostatics so that we are able to write $\vec{E} = - \nabla \phi (\vec{x})$ (i.e. the electric force is conservative), but do not impose any restrictions on $\vec{B}$ (since $\vec{x} \cdot \vec{B}$ always conserves energy).
...
There are two cases:
\begin{itemize}
  \item If $Q$ and $q$ both share the same sign, $Qq > 0$ and so the force is repulsive (like charges repel).
...
\begin{enumerate}
  ...
  \item Constant velocity $v_0$ in the $z$ direction (arbitrary constant $v_0$).
    If this is 0, then we obtain just circular motion in the $x$-$y$ plane.
\end{enumerate}
\begin{remark}
...
This is the 1D analogue of $\vec{F} = - \nabla V$ and so the following energy is conserved:
\[
...
\]
This is separable so we can rearrange and integrate to yield:
\begin{equation}
...
\begin{remark}
  \begin{itemize}
    \item Although we have both $t_0$ and $x_0$, there is only one constant here as changes to the integration bound $x_0$ can just be absorbed by $t_0$.
    \item It usually clear from context which sign is appropriate to take for the $\pm$.
  \end{itemize}
\end{remark}
...
However, region 1 of the above graph is unbounded so the particle bounces off the turning point and then flies off towards $x \to -\infty$ so its motion is unbounded.
...
\begin{tikzpicture}[scale=1.3, >=stealth]
  ...
  \begin{scope}[scale=0.5, xshift=9cm, >=stealth]
    \node at (5, 2) {$V''(x_0) > 0$};
    \draw[->] (0, -2) -- (0, 2) node[right] {$V(x)$};
    \draw[->] (0, -1) -- (7, -1) node[right] {$x$};
    \draw[dashed] (0, -0.62) -- (7, -0.62) node[right] {$E$};
...
\begin{proofcases}
  \begin{case}{$V''(x_0) > 0$}
    ...
    \begin{align*}
      ...
      \ddot{x} + \frac{V''(x_0)}{m} x &= \frac{V''(x_0)}{m} x_0 \\
    ...
    \begin{remark}
      During its oscillations it may appear as if the particle is inside a region of the graph where $V > E$, however, arbitrarily displacing the particle from $x_0$ modifies its energy for subsequent motion.
    \end{remark}
  \end{case}
  \begin{case}{$V''(x_0) < 0$}
    ...
    If $\widetilde{A} = 0$ and $\widetilde{B} \neq 0$, then this corresponds to ``rolling'' up the potential with just enough energy to reach the top as $t \to \infty$ since as $t \to \infty$, $x \to x_0$ which is at the maximum.
  \end{case}
  \begin{case}{$V''(x_0) = 0$}
...
\begin{example}[Period of a Pendulum]
  ...
  By definition $F(\tau) = \theta(t)$ and so our goal is to rewrite $F(\tau + \Delta \tau)$ as $\theta(t + \Delta t)$.
  \begin{align*}
...
\begin{remark}
  ...
  \]
  which agrees with our result using dimensional analysis as $\artanh(p)$ must be dimensionless.
\end{remark}
\subsection{Damping}
...
\end{proofcases}
\begin{remark}
  We analysed the problem of a damped oscillator in more detail within IA Differential Equations.
\end{remark}
\end{document}
