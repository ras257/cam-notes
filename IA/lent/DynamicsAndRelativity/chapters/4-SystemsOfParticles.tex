...
Suppose we have $N$ particles indexed by $i \in \{1, \ldots, N\}$ with momenta $\vec{p}_i = m_i \dot{\vec{x}}_i$ and all obeying Newton's law individually, that is, for each particle:
\[
...
\begin{align*}
  \dot{\vec{P}} = \sum_{i = 1}^{N} \dot{\vec{p}}_i &= \sum_{i = 1}^{N} \left(\vec{F}^\text{ext}_i + \sum_{\substack{j = 1 \\ j \neq i}}^N \vec{F}_{i j}\right) \\
...
The system cannot accelerate its centre of mass without an external force.
\begin{remark}
  This property of systems of particles is what allows us to treat the Earth as a point particle as all the internal forces between individual particles cancel each other out when considering the centre of mass of the whole system.
\end{remark}
...
In the case of the force $\vec{F}_{i j}$ between particles $i$ and $j$ comes from a potential that depends on the distance from $\vec{x}_i$ to $\vec{x}_j$, then we have that:
\begin{align*}
...
\begin{align}
  M\vec{R} &= \sum_{i = 1}^{N} m_i \vec{x}_i \nonumber \\
           &= \sum_{i = 1}^{N} m_i\vec{R} + \sum_{i = 1}^{N} m_i\vec{y}_i \nonumber \\
           &= \vec{R}\sum_{i = 1}^{N} m_i + \sum_{i = 1}^{N} m_i\vec{y}_i \nonumber \\
           &= M\vec{R} + \sum_{i = 1}^{N} m_i\vec{y}_i \implies \sum_{i = 1}^{N} m_i\vec{y}_i = \vec{0} \label{relativeSum}
\end{align}
...
\begin{align}
  T &= \sum_{i = 1}^{N} \frac{1}{2} m_i \dot{\vec{x}}_i \cdot \dot{\vec{x}}_i \nonumber \\
    ...
    &= \frac{1}{2} |\dot{\vec{R}}|^2\sum_{i = 1}^{N} m_i + \sum_{i = 1}^{N} \frac{1}{2} m_i |\dot{\vec{y}}_i|^2 + \dot{\vec{R}} \cdot\cancelto{\vec{0}}{\left(\sum_{i = 1}^{N} m_i\dot{\vec{y}}_i\right)} \nonumber \\
...
\end{definition}
\begin{remark}[Note]
  For each pair of particles, there are two associated forces $\vec{F}_{ij}$ and $\vec{F}_{ji}$, however there is only a single potential $V_{i j} = V_{j i}$.
  This is because although particle $i$ interacts with particle $j$ and particle $j$ interacts with particle $i$, there is only one physical interaction occurring, which is why we only take the sum over $i < j$ and not $i \neq j$ to avoid double counting potentials.
\end{remark}
...
\[
  \deriv{T}{t} = \sum_{i = 1}^{N} \frac{1}{2} m_i \deriv{}{t}(\dot{\vec{x}}_i \cdot \dot{\vec{x}}_i) = \sum_{i = 1}^{N} m_i \dot{\vec{x}}_i \cdot \ddot{\vec{x}}_i
\]
...
\begin{align*}
  \deriv{E}{t} &= \sum_{i = 1}^{N} \dot{\vec{x}}_i \cdot (m_i \ddot{\vec{x}}_i - \vec{F}^\text{ext}_i) - \sum_{i \neq j} \vec{F}_{i j} \cdot \dot{\vec{x}}_i \\
               &= \sum_{i = 1}^{N} \dot{\vec{x}}_i \cdot \left[m_i \ddot{\vec{x}}_i - \vec{F}^\text{ext}_i - \sum_{\substack{j = 1 \\ j \neq i}}^n \vec{F}_{i j}\right] \\
...
\]
So in this limit, we have reduced the problem to the one body Kepler problem (\cref{keplerSection}) for the relative position $\vec{r}$.
Since the more massive object is essentially stationary, we are effectively finding the motion of $\vec{x}_2$ about a fixed point.
\begin{remark}
