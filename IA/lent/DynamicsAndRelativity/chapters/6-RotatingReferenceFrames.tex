...
\begin{example}[Formation of Hurricanes]
  ...
  Near the equator $\vec{\omega} \times \vec{v}$, if motion is not equatorial, it is not perpendicular to $\vec{\omega}$ and so is negligible, instead, if motion is along on the equator $\vec{\omega} \times \vec{v}$ can be substantive, however, we see that it pushes particles vertically and is negligible compared to gravity so can still usually be ignored.
