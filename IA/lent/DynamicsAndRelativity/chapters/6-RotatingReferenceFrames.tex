...
\[
  m \left(\deriv[2]{\vec{x}}{t}\right)_{S} = \vec{F}
\]
...
\]
Using the right hand rule twice, we see that $\vec{\omega} \times \vec{x}$ points into the plane of the page and so $\vec{F}_{\text{cent}}$ points away from the axis of rotation:
\begin{center}
\begin{tikzpicture}[>=stealth]
  \draw[->] (0, 0)  -- (0, 3) node[right] {$\vec{\omega}$};
  \draw[->] (0, 0) -- (2, 2) node[left] {$\vec{x}$} node[midway, below right] {$|\vec{x}|$};
  ...
  \draw (0, 0) ++ (0:0.5) arc (0:45:0.5) node[pos=0.8, right] {$\theta$};
...
\]
From IA Vector Calculus, we know that $\nabla |\vec{x}|^2 = 2 \vec{x}$ and $\nabla \vec{\omega} \cdot \vec{x} = \vec{\omega}$ and so:
\begin{align*}
  \nabla V_{\text{cent}} &= -\frac{m}{2} \nabla (|\vec{\omega}|^2 |\vec{x}|^2 - (\vec{\omega} \cdot \vec{x})^2) \\
                         &= -m [\vec{x}|\vec{\omega}^2| - (\vec{\omega} \cdot \vec{x}) \vec{\omega}] \\
                         &= -m[(\vec{\omega} \cdot \vec{\omega}) \vec{x} - (\vec{\omega} \cdot \vec{x}) \vec{\omega}] \\
                         &= -m\vec{\omega} \times (\vec{x} \times \vec{\omega}) \\
                         &= -m (\vec{\omega} \times \vec{x}) \times \vec{\omega}
\end{align*}
...
\begin{example}
  ...
  So:
  \begin{align*}
    \vec{F}_{\text{cent}} &= |\vec{F}_{\text{cent}}| (\cos \theta \uvec{r} - \sin \theta \uvec{\theta}) \\
...
\begin{example}[Formation of Hurricanes]
  ...
  \end{tikzpicture}
  \end{center}
  The trajectory of each air molecule is bent clockwise (by using the right hand rule with $-\vec{\omega}$ going into the plane) which leads to an anti-clockwise swirling motion.
  This means that hurricanes rotate anti-clockwise in the northern hemisphere and clockwise in the southern hemisphere.

  Near the equator if motion is not equatorial, then $\vec{v}$ is not perpendicular to $\vec{\omega}$ and so $\vec{\omega} \times \vec{v}$ is negligible.
  If instead motion is along on the equator, then $\vec{\omega} \times \vec{v}$ can be substantive, however, we see that it pushes particles vertically and is negligible compared to gravity so can still usually be ignored.
...
\begin{example}
  ...
  At the floor of the tower $\ell = \omega'R^2$ where $\omega'$ is the angular speed of the ball around the axis right before it hits the ground.
  Since angular momentum is conserved $\omega(R + h)^2 = \omega'R^2$ and so $\omega' > \omega$.
  This means that the angular speed of the ball must increase as it falls in order to conserve $\ell$.
  Thus the ball rotates around the Earth faster than the tower and so will fall in front of the tower.
...
\begin{tikzpicture}[>=stealth, decoration={
  ...
  \node[below right, scale=0.5, yshift=-5, xshift=12] at (A) {(Out of plane of page)};
...
\begin{align*}
  ...
  \vec{g} &= (0, 0, -g)
\end{align*}
For $\vec{\omega}$, we need to resolve it into two components:
\begin{center}
\begin{tikzpicture}[>=stealth]
