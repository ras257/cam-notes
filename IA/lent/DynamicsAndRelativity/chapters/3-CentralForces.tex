...
For example in \cref{gravity}, we saw that the force one one mass depends on its radial distance to another mass, so, if we treat the other mass as the origin, the force on the first particle depends only on its distance $r$ to that origin.
...
\section{Polar Coordinates in the Plane} \label{polarCoordinates}
...
\[
  m\ddot{\vec{x}} = m(\ddot{r} - r \dot{\theta}^2)\uvec{r} + m(r \ddot{\theta}  + 2\dot{r} \dot{\theta}) \uvec{\theta} = -\deriv{V}{r}\uvec{r}
\]
...
\begin{align*}
  ...
  \implies m\ddot{r} &= - \deriv{V}{r} + \frac{m\ell^2}{r^3} \\
...
Since angular momentum is also conserved, the impact parameters for a particle from the incoming direction and for a particle from the outgoing direction are equal.
...
\end{align*}
For paths where the non-interacting trajectory would pass very close to the target, the impact parameter is very small and so we would expect to get large scattering angles.
This allows us to use scattering of a particle with initial velocity $v$ to find subatomic distances by measuring relatively large $\phi$ and then finding $b$ using:
\[
  b = \frac{\kappa}{mv^2 \tan(\phi/2)}
\]
\end{document}
