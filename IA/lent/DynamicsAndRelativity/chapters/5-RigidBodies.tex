...
The magnitude of $\uvec{n} \times \vec{x}$ is the perpendicular distance between $\vec{x}$ and the axis of rotation:
\[
...
\end{align*}
where $d = |\uvec{n} \times \vec{x}|$ is the perpendicular distance of $\vec{x}$ to the axis of rotation.
...
If the torque is about the axis of rotation, then $\vec{G} = G\uvec{n}$ for some scalar $G$.
If we dot both sides of $\dot{\vec{L}} = \vec{G}$  to compare components along the axis, then we obtain:
\begin{equation}
...
\begin{remark}[Note]
  ...
  In IA Vector Calculus, we will define a more invariant \textit{inertia tensor} that packages up all the information needed to find the moment of inertia of a rigid body about \textbf{any axis}.
\end{remark}
...
\]
where $\rho$ is the \textit{density} of the rigid body at $\vec{x}$, although we typically consider situations with uniform density, i.e. $\rho(\vec{x}) = \rho_0\ \forall \vec{x} \in \R^3$.
...
\begin{example}[Moments of Inertia for Basic Shapes]
  ...
  \begin{enumerate}
    \item Consider a 1D ring $R$ of radius $a$:
      ...
      \[
        I = \int_{R} \rho a^2 \d{s}
      \]
      Parametrising $R$ as $\vec{x}(\theta) = (a \cos \theta, a \sin \theta)$ for $\theta \in [0, 2\pi)$ we have:
      \[
        I = \int_{0}^{2\pi} \rho a^2 \abs{\deriv{\vec{x}}{\theta}} \d{\theta} = \int_{0}^{2\pi} \rho a^2 \cdot a \d{\theta} = 2\pi \rho a^3
      \]
...
\begin{equation}
  T = \underbrace{\frac{1}{2}M|\dot{\vec{R}}|^2}_{\text{translational}} + \underbrace{\frac{1}{2} I_{\text{CoM}} \omega^2}_{\text{rotational}} \label{rigidKESplit}
\end{equation}
...
One nice case is constant uniform gravitational field.
From \cref{uniformGravitationalPot} we know that $V_i(\vec{x}_i) = m_i g z_i$ and so:
\begin{equation}
...
\begin{remark}[Note]
  Here we used an axis passing through the CoM, however, in some cases it may be easiest to consider an axis that does not pass through the CoM.
\end{remark}
...
\]
Since the axis through the pivot and the axis through the CoM are parallel, we can use the parallel axis theorem (\cref{parallelAxisTheorem}) and so:
\[
...
\begin{remark}[Intuition]
  The point of contact having 0 velocity the instant it contacts the ground can be hard to imagine.
  Instead, consider how an object like a spiky star would roll along the ground, when each vertex of the star is touching the ground it remains stationary.
  As we increase the number of vertices, this polygon becomes increasingly circular and so we can carry our intuition over to no-slip rolling.
\end{remark}
...
To check that energy is conserved when rolling when a frictional force $F_f$ acts at the contact point, we can differentiate the kinetic energy:
\[
...
Since the frictional force $F_{f}$ is acting at a perpendicular distance $a$, the torque $G = aF_{f}$ and so $I \ddot{\theta} = aF_{f}$.
Furthermore, using Newton's equation $M\ddot{x}$ is the force $F_{f}$ acting on the ball and so:
\begin{align*}
  \deriv{E}{t} &= \dot{x}(-F_{f}) + \dot{\theta}aF_{f} \\
               &= F_{f}(-\dot{x} + a \dot{\theta}) \text{ as rolling: $\dot{x} = v = a \dot{\theta}$} \\
               &= 0
\end{align*}
