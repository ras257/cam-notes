...
\begin{example}
  ...
  \[
    ...
    \end{pmatrix}
  \]
  On the bottom side $z = 0$, so $f = 0$, thus $\int_{\text{bottom}} f \d{\vec{S}} = \vec{0}$.
  Two of the other sides yield $\vec{0}$ and other other two yield $(8, 0, 0)$ and $(0, 8, 0)$ so the result is verified.
\end{example}
...
This means that a total magnetic charge cannot build up at a point in space, so we cannot have a north pole without an accompanying south pole, that is we cannot have \textit{magnetic monopoles}.
...
This continuity equation takes the same form as the continuity equation for charge conservation (\cref{maxwellContuinuity}), just with $\vec{J}$ replaced by $\rho \vec{u}$ so as above, we obtain:
\[
