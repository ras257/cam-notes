...
\begin{proof}
  ...
  \end{tikzpicture}
  \end{center}
  We can then divide $C$ into a right hand part $C^{+}$, which is parametrised as $(x^{+}(y), y)$ for each $y$ in the range $y_{\text{min}} \leq y \leq y_{\text{max}}$ and left hand part $C^{-}$, which is parametrised as $(x^{-}(y), y)$.
  ...
  \begin{align*}
    \int_{A} \pderiv{P}{y} \d{y} \d{x} &= \int_{x_{\text{min}}}^{x_{\text{max}}} \left(\int_{y^{-}(x)}^{y^{+}(x)} \pderiv{P}{y} \d{y}\right) \d{x} \\
                                       ...
                                       &= -\int_{x_{\text{min}}}^{x_{\text{max}}} P(x, y^{-}(x)) \d{x} - \int_{x_{\text{max}}}^{x_{\text{min}}} P(x, y^{+}(x)) \d{x} \\
...
\begin{example}[Area of an ellipse]
  ...
  We see that $\pderiv{Q}{x} - \pderiv{P}{y} = 1$ and so:
  \[
  ...
  We can parameterise $\partial A$ using $(a \cos t, b \sin t)$ for $t \in [0, 2\pi)$ and so:
  \[
...
\begin{theorem}[Stokes' Theorem]
  ...
  That is, the flux of $\nabla \times \vec{F}$ through the surface $S$ is equal to the line integral of $\vec{F}$ over the boundary of $S$, $C = \partial S$.
\end{theorem}
\begin{remark}
  Green's Theorem in the plane (\cref{greensTheorem}) is a special 2D case of Stokes' Theorem.
...
\begin{example}
  ...
  \[
    ...
    \end{pmatrix}
  \]
  with $\rho \in [0, 1]$ and $\phi \in [0, 2\pi]$.
  ...
  \[
    \oint_{C} \vec{F} \cdot \d{\vec{x}} = \int_{2\pi}^{0} \begin{pmatrix}
    x^2 y \\
    0 \\
    0 \\
    \end{pmatrix} \cdot \begin{pmatrix}
    ...
    \cos \phi \\
    0 \\
    \end{pmatrix} \d{\phi} = \int_{0}^{2\pi} \cos^2 \phi \sin^2 \phi \d{\phi} = \frac{\pi}{4}
  \]
  So they agree, as expected.
\end{example}
\subsection{Closed Surfaces}
Although we stated that Stokes' Theorem is for open surfaces, it also applies to closed surfaces $S$.
...
\[
  \vec{x}(t) = \left(\cos t, \frac{1}{2} \sin t, 0\right)
\]
...
\begin{proof}
  ...
  Carrying out a similar process on $S^{-}$, we have $\vec{e}_1 \cdot \d{\vec{S}} = - \d{y}\d{z}$.
  Thus:
  \[
    \int_{V} \pderiv{F_1}{x} \d{V} = \int_{S+} F_1 \vec{e}_1 \cdot \d{\vec{S}} + \int_{S-} F_1 \vec{e}_1 \cdot \d{\vec{S}} = \int_{S} F_1 \vec{e}_1 \cdot \d{\vec{S}}
  \]
...
\end{proof}
\begin{example}
  What is the flux of the vector field $\vec{F} = (y, x, z^6)$ through the surface of the sphere $V_r = \{\vec{x} \in \R^3: |\vec{x}| < R\}$?
...
\begin{remark}[Advice]
  ...
  \begin{align*}
    \int_{0}^{2\pi} \sin^{2m + 1} \phi \cos^n \phi \d{\phi} = \int_{0}^{2\pi} \sin^{m} \phi \cos^{2n + 1} \phi \d{\phi} &= 0 \\
    \text{ and } \int_{0}^{\pi} \sin^{m} \theta \cos^{2n + 1}\theta \d{\theta} &= 0
  \end{align*}
  For example, if presented with:
  \[
    \int_{\theta = 0}^{\pi} \int_{\rho = 0}^{1} \rho^3(1 - \rho^2)^3 e^{-\rho} \sin^2 \theta \cos \theta \d{\rho} \d{\theta}
  \]
...
That is, if $\nabla \times \vec{F} = \vec{0}$ everywhere in a simply connected domain $\mathscr{D}$, then there exists a single valued scalar potential $f$ such that $\vec{F} = \nabla f$.
We now have the tools to prove this assertion.
\begin{proof}
  We first need to show that if $\oint_C \vec{F} \cdot \d{\vec{x}} = 0$ for all closed curves $C$ then $\int_{\vec{a}}^{\vec{b}} \vec{F} \cdot  \d{\vec{x}}$ is path independent.
  ...
  By Stokes' Theorem $\oint_{C} \vec{F} \cdot \d{\vec{x}} = \int_{S} \nabla \times \vec{F} \cdot \d{\vec{S}} = 0$, and so by the above, the line integral of $\vec{F}$ is path independent.
...
\end{theorem}
\begin{proof}
  Let $\vec{F} = \phi \nabla \psi$, then using an identity from \cref{vectorIdentities}, we have:
  \begin{align*}
    \nabla \cdot \vec{F} = \nabla \cdot (\phi \nabla \psi) &= \phi (\nabla \cdot (\nabla \psi)) + \nabla \psi \cdot \nabla \phi \\
                                                           &= \phi \nabla^2 \psi + \nabla \phi \cdot \nabla \psi
  \end{align*}
...
\end{proof}
\begin{remark}
  There are many other identities like this, most of which are provide analogously by considering a vector field $\vec{F} = f\vec{a}$ for some fixed $\vec{a}$ and then applying an integral theorem, and then manipulating the result to remove $\vec{a}$.
\end{remark}
\begin{example}
  Integrate $\nabla f$ where $f(\vec{x}) = xyz$ in the cube $V: 0 \leq x, y, z \leq 2$ and confirm the result using the above identity (\cref{gradVolumeIdentity}).

  \textbf{Integrating Normally}\par
  Firstly, $\nabla f = (yz, xz, xy)$.
  ...
  \textbf{Using the Identity}\par
  To confirm the identity, we can calculate $\int_{\partial V}  f \d{\vec{S}}$ by considering six separate surfaces, one for each side of the cube.

  On the three sides with $x = 0$, $y = 0$, and $z = 0$ respectively, $f = 0$ and so on these sides $\int f \d{\vec{S}} = \vec{0}$.

  On the top side, $z = 2$, $f = 2xy$, $\vec{n} = (0, 0, 1)$, and $\d{S} = \d{x}\d{y}$ so:
  \[
    \int_{\text{top}} f \d{\vec{S}} = \int_{0}^{2} \int_{0}^{2} 2xy \begin{pmatrix}
    0 \\
    0 \\
    1 \\
    ...
    \end{pmatrix}
  \]
  and similarly the other two sides yield $(8, 0, 0)$ and $(0, 8, 0)$ so the result is verified.
\end{example}
\section{Maxwell's Equations and Continuity Equations}
\subsection{Maxwell's Equations}
\begin{definition}[Maxwell's Equations]
  \textit{Maxwell's equations} in a vacuum for an \textit{electric field} $\vec{E}(\vec{x}, t)$ and \textit{magnetic field} $\vec{B}(\vec{x}, t)$ in the presence of an \textit{electric charge density} $\rho(\vec{x}, t)$ (the \textit{charge per unit volume}) and the \textit{electric current density} $\vec{J}(\vec{x}, t)$ (the current per unit area) are:
  \begin{align}
    \nabla \cdot \vec{E} &= \frac{\rho}{\varepsilon_0} \label{maxwell1} \\
    \nabla \cdot \vec{B} &= 0 \label{maxwell2} \\
    \nabla \times \vec{E} &= -\pderiv{\vec{B}}{t} \label{maxwell3} \\
    \nabla \times \vec{B} &= \mu_0\left(\vec{J} + \varepsilon_0 \pderiv{\vec{E}}{t}\right) \label{maxwell4}
  \end{align}
  where $\varepsilon_0$ is the dielectric permittivity of free space and $\mu_0$ is the permeability of free space, which are related by $c^2 = \frac{1}{\varepsilon_0 \mu_0}$ where $c$ is the speed of light.
\end{definition}
\begin{remark}[Exams]
  You are not required to memorise Maxwell's equations for exams, they will be provided when required.
\end{remark}
\subsubsection{Gauss's Law}
Let $V$ be an arbitrary fixed volume containing total charge $Q_V$.
Since $\rho$ is the charge density, $Q_V$ is just the volume integral of $\rho$ over $V$.
So applying \cref{maxwell1}, the differential form of Gauss's Law, we have:
\[
  Q_V = \int_{V} \rho \d{V} = \int_{V} \varepsilon_0 \nabla \cdot \vec{E} \d{V}
\]
Then using the divergence theorem (\cref{divergenceTheorem}), we have:
\[
  \int_{V} \nabla \cdot \vec{E} \d{V} = \int_{\partial V} \vec{E} \cdot \d{\vec{S}} \implies \int_{\partial V} \vec{E} \cdot \d{\vec{S}} = \frac{1}{\varepsilon_0}Q_V
\]
This is the integral form of \textit{Gauss's Law} and tells us that the flux of $\vec{E}$ through a \textbf{closed surface} is proportional to the total charge within that surface, irrespective of how the charge is distributed.

\subsubsection{Gauss's Law for Magnetism}
Using \cref{maxwell2}, the differential form of Gauss's law for magnetism, and the divergence theorem, we have:
\[
  \int_{\partial V} \vec{B} \cdot \d{\vec{S}} = \int_{V} \nabla \cdot \vec{B} \d{V} = 0
\]
This is the integral form of \textit{Gauss's Law for magnetism} and tells us that the net magnetic flux through a closed surface is always zero.
This means that a total magnetic charge cannot build up at a point in space, so we cannot have a north pole without an accompanying south pole, i.e. we cannot have \textit{magnetic monopoles}.
\subsubsection{Faraday's Law of Induction}
\nonexaminable
Suppose $S$ is a fixed open surface.
The rate of change of the magnetic flux through $S$ is:
\[
  \deriv{}{t}\int_{S} \vec{B} \cdot \d{\vec{S}} = \int_{S} \pderiv{\vec{B}}{t} \cdot \d{\vec{S}}
\]
as we can bring the derivative under the integral sign.
We can then apply \cref{maxwell3}, the differential form of Faraday's Law of induction, and Stokes' Theorem (\cref{stokesTheorem}), to yield:
\[
  \deriv{}{t}\int_{S} \vec{B} \cdot \d{\vec{S}} = - \int_{S} \nabla \times \vec{E} \cdot \d{\vec{S}} = -\oint_{\partial S} \vec{E} \cdot \d{\vec{x}}
\]
This is the integral form of \textit{Faraday's law of induction}, which is sometimes known as the \textit{Maxwell-Faraday equation}.
It tells us that if we change flux of a magnetic field, then this causes circulation of the electric field around $\partial S$.
\subsubsection{Amp\`ere-Maxwell Law}
\nonexaminable
Similarly, also for a fixed open surface $S$, we have:
\[
  \deriv{}{t} \int_{S} \vec{E} \cdot \d{\vec{S}} = \int_{S} \pderiv{\vec{E}}{t} \cdot \d{\vec{S}}
\]
We can then apply \cref{maxwell4}, the differential form of the Amp\`ere-Maxwell Law, to yield:
\begin{align*}
  \deriv{}{t} \int_{S} \vec{E} \cdot \d{\vec{S}} &= \int_{S} \frac{1}{\varepsilon_0}\left(\frac{1}{\mu_0} \nabla \times \vec{B} - \vec{J}\right) \cdot \d{\vec{S}} \\
                                                 &= \frac{1}{\varepsilon_0\mu_0} \int_{S} \nabla \times \vec{B} \cdot \d{\vec{S}} - \frac{1}{\varepsilon_0} \int_{S} \vec{J} \cdot \d{\vec{S}} \\
                                                 &= \frac{1}{\varepsilon_0\mu_0} \oint_{\partial S} \vec{B} \cdot \d{\vec{x}}  - \frac{1}{\varepsilon_0} \int_{S} \vec{J} \cdot \d{\vec{S}} \text{ by Stokes' Theorem}
\end{align*}
This is the integral form of the \textit{Amp\`ere-Maxwell Law}.
We see that if $\vec{E}$ is constant in time, the left hand side vanishes and so we see that the electric current causes circulation of the magnetic field.
\subsection{Continuity Equations}
\subsubsection{In Maxwell's Equations}
If we take the divergence of \cref{maxwell4}, we see that:
\begin{align*}
  \nabla \cdot (\nabla \times \vec{B}) &= \mu_0\left(\nabla \cdot \vec{J} + \varepsilon_0 \nabla \cdot \pderiv{\vec{E}}{t}\right) \\
                                       &= \mu_0\left(\nabla \cdot \vec{J} + \varepsilon_0 \pderiv{}{t}(\nabla \cdot \vec{E})\right)
\end{align*}
where we can exchange $\pderiv{}{t}$ and $\nabla$ since $\pderiv{}{t}$ and $\pderiv{}{x_i}$ commute.
Recall from \cref{solenoidalFields} that for any vector field $\vec{F}$, we have $\nabla \cdot (\nabla \times \vec{F}) = 0$.
Thus:
\[
  \nabla \cdot \vec{J} + \varepsilon_0 \pderiv{}{t}(\nabla \cdot \vec{E}) = 0
\]
Finally, applying \cref{maxwell1}, we have:
\begin{equation}
  \pderiv{\rho}{t} + \nabla \cdot \vec{J} = 0 \label{maxwellContinuity}
\end{equation}
This is the \textit{continuity equation} for electric charge so represents the \textit{conservation of charge}.

If we integrate this over a fixed volume $V$, we see that:
\[
  \int_{V} \pderiv{\rho}{t} \d{V} = \deriv{}{t} \int_{V} \rho \d{V}  = \deriv{}{t}Q_V
\]
since $Q_V$ is the total charge in the volume.
Furthermore:
\[
  \int_{V} \nabla \cdot \vec{J} \d{V} = \int_{\partial V} \vec{J} \cdot \d{\vec{S}}
\]
Therefore, combining the above:
\[
  \deriv{}{t}Q_V = - \int_{\partial V} \vec{J} \cdot \vec{n} \d{S}
\]
So the rate at which the total charge in $V$ decreases is equal to the net flux of current out of $V$.
This represents the conservation of charge, that is, the charge in $V$ only changes because currents carry charge into or out of the volume.

We know this because $\vec{J}$ represents the charge moving across a unit area in a unit time and, since only the component of $\vec{J}$ perpendicular to the surface actually results in charge leaving $V$, $\vec{J} \cdot \vec{n}$ is the rate at at which charge leaves $V$ per unit area of $\partial V$.

In this context, $\vec{J}$ is called the flux density of charge, i.e. the rate at which charge is moving around.
\subsubsection{In Fluid Dynamics}
We see similar continuity equations in other areas of mathematical physics.
For example, one of the equations governing fluid mechanics is:
\begin{equation}
  \pderiv{\rho}{t} + \nabla\cdot(\rho \vec{u}) = 0 \label{fluidContinuity}
\end{equation}
which represents the \textit{conservation of mass}.
In this equation $\rho(\vec{x}, t)$ is the \textit{density of the fluid} and $\vec{u}(\vec{x}, t)$ is the \textit{velocity of the fluid} at a particular point.

This continuity equation takes the same form as the continuity equation for charge conservation (\cref{maxwellContinuity}), just with $\vec{J}$ replaced by $\rho \vec{u}$ so as above, we obtain:
\[
  \deriv{}{t}\int_{V} \rho \d{V} = - \int_{\partial V} \rho \vec{u} \cdot \d{\vec{S}}
\]
This tells us that the total mass within a volume $V$ only charges as a result of fluid particles being transported through the surface of $V$, which they are at a rate of $\vec{u} \cdot \vec{n}$ per unit area of $\nabla V$.
So $\rho \vec{u}$ is the flux density of mass.

In the case of a fluid with constant density, \cref{fluidContinuity} reduces to $\nabla \cdot \vec{u} = 0$.
This is why solenoidal vector fields (\cref{solenoidalFields}) are also referred to as incompressible vector fields.
\end{document}
