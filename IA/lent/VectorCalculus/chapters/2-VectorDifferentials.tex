...
\begin{definition}[Operator]
  An \textit{operator} is a map between vector spaces.
\end{definition}
\section{Del}
\begin{definition}
...
\end{align*}
We utilised that since $\partial/\partial x_i$ is a scalar operator, it can be moved ``through'' the dot product as:
\begin{equation}
  ...
  \pderiv{b_3}{x_3} \\
  \end{pmatrix} =
  \vec{a} \cdot \left(\pderiv{}{x_i} \vec{b}\right) \label{scalarOperatorMove}
\end{equation}
...
\begin{example}[Using Divergence]
  ...
  \begin{enumerate}
    ...
    \item If $A$ is a constant $3 \times 3$ matrix then:
      \[
...
or in suffix notation:
\[
  ((\vec{u} \cdot \nabla)\vec{F})_i = u_j \pderiv{F_i}{x_j}
\]
\end{definition}
...
\begin{example}
  ...
  $f(\vec{x}) = c \implies |\vec{x}| = \sqrt{c}$ so the level surfaces of $f$ are just spheres centered at the origin.
...
\begin{itemize}
  \item $D_{\vec{n}}f$ is maximised when $\vec{n}$ points in the same direction to $\nabla f$.
  \item $D_{\vec{n}}f = 0$ when $\vec{n}$ is orthogonal to $\nabla f$ (i.e. $\vec{n}$ is tangent to the level surface).
  \item $D_{\vec{n}}f$ is minimised when $\vec{n}$ points in the opposite direction to $\nabla f$.
\end{itemize}
\[
  \max_{\forall \vec{n}} D_{\vec{n}}f = |\nabla f| \text{ and } \min_{\forall \vec{n}} D_{\vec{n}}f = -|\nabla f|
\]
...
\begin{remark}[Defining level sufaces in two ways]
  ...
  In terms of $h$, $\nabla h \in \R^2$ points in the direction that is steepest uphill on the surface (as $D_{\vec{n}} h$ is maximised when $\vec{n}$ points in the same direction as $\nabla h$) and then projected down onto the $x$-$y$ plane.
...
\begin{remark}[Warning]
  The scalar triple product formula $\vec{a} \cdot (\vec{b} \times \vec{c}) = (\vec{a} \times \vec{b}) \cdot \vec{c}$ and the vector triple product formula $\vec{a} \times (\vec{b} \times \vec{c}) = (\vec{a} \cdot \vec{c}) \vec{b} - (\vec{a} \cdot \vec{b}) \vec{c}$ \textbf{do not always work} if one of the vectors is $\nabla$.
  ...
  One case where you are able to cycle the scalar triple product is in the identity:
  \[
    (\vec{F} \times \nabla) \cdot \vec{G} = \vec{F} \cdot (\nabla \times \vec{G})
  \]
  This can be checked with suffix notation but works as $\nabla$ is always acting on $\vec{G}$.
\end{remark}
By setting $f(x) = 1/x$ in \cref{gradChain}, we can derive that:
\[
  \nabla \left(\frac{1}{g}\right) = - \frac{1}{g^2} \nabla g
\]
which we can then use with the other three of the first four identities to derive quotient rule expressions for division by a scalar field.
...
\end{proof}
Intuitively this makes sense as if we have a function of the radius, then grad is just $f'(r)$ (i.e. rate of change of radius in the outward direction) multiplied by a unit vector in the outward direction so that it points in the direction of the rate of change.
\begin{example}
...
We are also able to differentiate the modulus of a vector field:
\[
...
\]
We can also derive a result without indies by using the vector differential identities introduced earlier.
We let $g(\vec{x}) = |\vec{F}|^2 = \vec{F} \cdot \vec{F}$ and $f(x) = \sqrt{x} \implies f'(x) = \frac{1}{2\sqrt{x}}$, applying the identity in \cref{gradChain} to yield:
\[
  \nabla |\vec{F}| = \nabla [f(g(\vec{x}))] = f'(g(\vec{x})) \nabla g = \frac{1}{2|\vec{F}|} \nabla (\vec{F} \cdot \vec{F})
\]
Then, to find $\nabla(\vec{F} \cdot \vec{F})$ we apply the identity in \cref{gradFG} with $\vec{G} = \vec{F}$ yielding:
\[
  \nabla |\vec{F}| = \frac{(\vec{F} \cdot \nabla) \vec{F} + \vec{F} \times (\nabla \times \vec{F})}{|\vec{F}|}
\]
\section{Laplacian}
\label{secLaplacian}
    ...
    \item If $f(\vec{x}) = (\vec{a} \cdot \vec{x})^2$, then $\nabla^2 f$ is:
      ...
      \begin{align*}
        \nabla^2 f &= \nabla^2(a^2_1 x^2 + a^2_2 y^2 + a^2_3 z^2 + 2a_1a_2xy + 2a_1a_3xz + 2a_2a_3yz) \\
                   ...
                   &= 2|\vec{a}|^2
      \end{align*}
      Using components is easy enough here but it is often preferable to use vector methods as they scale to arbitrary dimensions without the need to introduce fiddly suffix notation.
  \end{enumerate}
\end{example}
Functions that satisfy $\nabla^2 f = 0$ (\textit{Laplace's Equation}) are called \textit{harmonic}.
They are incredibly important in many fields so we will study these later in the course in \cref{chapter7}.
...
\end{definition}
\begin{proposition}
  The Laplacian of a vector field $\vec{F}$ has the following coordinate independent definition:
  \[
    \nabla^2 \vec{F} = \nabla(\nabla \cdot \vec{F}) - \nabla \times (\nabla \times \vec{F})
  \]
\end{proposition}
...
The converse is also true but only if $\mathscr{D}$ is \textit{simply connected} (this will be proved later in the course after we have covered line integrals, see \cref{scalarPotentials}).
...
\begin{example}
  ...
  In fact, it can be shown that no single valued scalar potential for $\vec{F}$ exists in a simply connected domain so it cannot be conservative, despite that it is irrotational.
...
\begin{remark}[Nomenclature]
  The name ``solenoidal'' arises from magnetic fields which are always solenoidal.
