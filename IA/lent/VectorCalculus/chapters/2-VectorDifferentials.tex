...
\]
Instead of using the above, the easiest way to find $\nabla |\vec{F}|$ is to let $g(\vec{x}) = |\vec{F}|^2 = \vec{F} \cdot \vec{F}$ and $f(x) = \sqrt{x} \implies f'(x) = \frac{1}{2\sqrt{x}}$.
We can then apply the identity in \cref{gradChain} to yield:
\[
