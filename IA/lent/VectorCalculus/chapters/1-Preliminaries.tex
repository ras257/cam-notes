...
Suffix notation and summation convention will be used heavily.
Further, we will use Newton's dot notation ($\dot{\vec{x}},\ \ddot{\vec{x}}, \ldots$) to denote differentiation with respect to $t$, even if $t$ does not represent time.

\subsection{Calculus on Vectors}
Many rules of calculus for scalars apply similarly to vectors:
\begin{align*}
...
\begin{equation}
  \int_{t_1}^{t_2} \vec{a}(t) \d{t} = \int_{t_1}^{t_2} (a_1, a_2, a_3) \d{t} = \left(\int_{t_1}^{t_2} a_1 \d{t}, \int_{t_1}^{t_2} a_2 \d{t}, \int_{t_1}^{t_2} a_3 \d{t}\right) \label{integralVector}
\end{equation}
...
For example, $\d{\vec{x}} = (\d{x}, \d{y}, \d{z})$, we then have:
\begin{equation}
