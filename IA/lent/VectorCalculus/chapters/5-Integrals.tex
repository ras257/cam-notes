...
\begin{example}[Revisiting \cref{lineIntegralExamples}]
  \begin{itemize}
    ...
    \item In \textbf{vi}, we integrated $F = z \vec{e}_\phi$.
      ...
      Suppose that $\nabla \times \vec{F}$ had actually been $\vec{0}$, we could not have concluded that $\vec{F}$ would be conservative as the domain of $\vec{F}$ is not simply connected.
      This is because the domain does not include the $z$-axis as $\vec{e}_\phi$ is not defined there.
  \end{itemize}
\end{example}
