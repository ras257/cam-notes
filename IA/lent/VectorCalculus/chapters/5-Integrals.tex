    ...
    \item What is the work done by a force field $\vec{F} = (y^2 e^{x}, e^{y} + 2ye^{x}, 0)$ moving a particle along $C_3$, which is the ellipse $x^2/a^2 + y^2/b^2 = 1$ in the $x$-$y$ plane starting at $(a, 0, 0)$ and going anticlockwise to $(0, b, 0)$.
      ...
      \begin{align*}
        \int_{C_3} \vec{F} \cdot \d{\vec{x}} &= \eval{e^{b \sin \theta}}{0}{\pi/2} + \int_{0}^{\pi/2} b^2 \sin \theta e^{a \cos \theta} (-a + 2\cos \theta + a \cos^2 \theta) \d{\theta} \\
                                             &= e^{b} - 1 + \int_{1}^{0} b^2e^{au}(1 - 2u - au^2) \d{u} \text{ using $u = \cos \theta$}\\
...
\[
\int_{C} \vec{F} \cdot \d{\vec{x}} = \int_{C} \nabla f \cdot \dot{\vec{x}} \d{t} = \int_{t_1}^{t_2} \deriv{f}{t} \d{t} = f(\vec{b}) - f(\vec{a})
\]
\end{proof}
...
\end{corollary}
\begin{proof}
  Since $\vec{F}$ is conservative, we can write $\vec{F} = \nabla f$ for a \textbf{single valued} $f$.
...
\begin{example}[Revisiting \cref{lineIntegralExamples}]
  \begin{itemize}
    ...
    \item In \textbf{vi}, we integrated $F = z \vec{e}_\phi$.
      ...
      So we could shrink a curve around the $z$ axis to a point by passing it through the ``gap'' at the origin, however, the derivatives of $\vec{F}$ are not defined at the origin and so $\nabla \times \vec{F}$ is also not defined there so we are not working in a simply connected domain.
  \end{itemize}
\end{example}
...
\[
  \int_{x_{\text{min}}}^{x_{\text{max}}} \left(\int_{Y(x)} f(x, y) \d{y}\right) \d{x}
\]
...
\begin{example}[Example Volume Integral]
  ...
  We could have seen that $\int_{V} yz \d{V} = 0$ directly from the symmetry of the cuboid as for each $y \in [0, b]$, $-y \in [-b, 0]$ which has an opposite contribution to the integrand $yz$ and so they cancel out.
\end{example}
...
A finite proportion of the $(x, y)$-plane is an open surface and is orientable and so we treat is as such before taking the limit so it becomes the full plane.
...
\begin{example}[Surface Area of Shell]
  Find the surface area of the hemispherical shell $H_R = \{\vec{x} \in \R^3: |\vec{x}| = R, z < 0\}$.
...
\begin{example}[Flux Through a Sphere]
  ...
  \]
  where $\alpha$ is a constant through the surface of the sphere $S_R = \{\vec{x} \in \R^3: |\vec{x}| = R\}$, choosing the normal that points away from the origin.
...
\begin{example}[Flux Through an Infinite Plane]
  Find the flux of $\vec{F}(\vec{x}) = (y^2 e^{-(y^2 + z^2)}, e^{-y^4}, -x^2 e^{-(y^2 + z^2)})$ through the infinite plane $\Pi = \{\vec{x} \in \R^3 : x - z = 0\}$ where the normal points in the direction of positive $x$.
...
\begin{remark}[Warnng]
  ...
  \]
  This however, is not correct, because the area of a surface element is $\abs{\pderiv{\vec{x}}{x} \times \pderiv{\vec{x}}{y}}\d{x}\d{y}$ not just $\d{x}\d{y}$.
\end{remark}
