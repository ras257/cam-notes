...
We can map constant lines of $u$ and $v$, which are straight in $\R^2$, onto the surface and they do not necessarily have to remain straight on the surface.
...
The part of $S$ bounded by these curves forms, to a lowest order approximation, a small planar parallelogram with one side given by the vector:
\[
  \vec{x}(u + \delta u, v) - \vec{x}(u, v) = \pderiv{\vec{x}}{u} \delta + O(\delta u^2) \text{ using Taylor's Theorem}
\]
and the other side being given by:
\[
  \vec{x}(u, v + \delta v) - \vec{x}(u, v) = \pderiv{\vec{x}}{v} \delta v + O(\delta v^2)
\]
As $\delta u$ and $\delta v$ get very small, this approaches an exact parallelogram so by \cref{areaParallelogram}, the area of the region for infinitesimal $\d{u}$, $\d{v}$ is:
\[
...
\begin{example}
  ...
  \[
    \vec{x}(\theta, \phi, r) = \begin{pmatrix}
    r\sin\theta\cos\phi \\
    r\sin\theta\sin\phi \\
    r\cos\theta \\
    \end{pmatrix}
...
\end{proof}
Therefore, for infinitesimal $\d{u}, \d{v}, \d{w}$, the volume of such a region is exactly the area of the parallelepiped so:
\[
  \d{V} = \abs{\pderiv{\vec{x}}{u} \cdot \left(\pderiv{\vec{x}}{v} \times \pderiv{\vec{x}}{w}\right)} \d{u}\d{v}\d{w}
\]
The infinitesimal volume $\d{V}$ is called a \textit{volume element}.
\end{document}
