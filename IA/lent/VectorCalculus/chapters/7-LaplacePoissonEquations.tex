...
\begin{align*}
  \int_{\mathscr{D}} f \d{V} &= \int_{\mathscr{D}} \nabla \cdot (\nabla \phi) \d{V} \\
                             ...
                             &= \int_{\partial \mathscr{D}} g \d{S} \text{ using the boundary condition}
\end{align*}
...
\begin{remark}[Note]
  To show that $\nabla \phi(\rho) = \phi'(\rho) \vec{e}_\rho$, we can use grad in cylindrical polar coordinates from \cref{cylindricalOperators}.
\end{remark}
...
\begin{align*}
  F_R &= \int_{\mathscr{C}_R} \nabla^2 \phi \d{V} \\
      ...
      &= \int_{\partial \mathscr{C}_R} \phi'(\rho)\vec{e}_\rho \cdot \d{\vec{S}}
\end{align*}
Since the divergence theorem (\cref{divergenceTheorem}) requires a closed surface, we need to consider the flux integrals over both the curved part and circular caps of $\partial \mathscr{C}_R$.
For the curved part:
\begin{align*}
  \int_{\text{curved}} \phi'(\rho)\vec{e}_\rho \cdot \d{\vec{S}} &= R\int_{\text{curved}} \phi'(\rho) \vec{e}_\rho \cdot \vec{e}_\rho \d{S} \text{ as $\vec{n} = R \vec{e}_\rho$} \\
                                                                 &= R\phi'(R) \int_{\text{curved}} \d{S} \\
                                                                 &= 2\pi R \phi'(R) \text{ as curved surface area is $2\pi R$}
\end{align*}
The flux integrals through the circular caps of $\partial \mathscr{C}_R$ disappear as $\vec{n} = \pm \vec{e}_z$ and $\vec{e}_\rho \cdot \vec{e}_z = 0$.
Therefore:
\[
...
\end{equation}
This is the scalar potential for a \textit{point source} of strength $F$ at the origin.
...
Since $\vec{F} = q\vec{E}$, the force per unit charge is $\vec{E} = -\nabla \phi$ so:
\[
...
Consider Poisson's equation $\nabla^2 \phi = f$ in $\R^3$ with a cylindrically symmetric source term that is non-zero only in the cylindrical region $\mathscr{C}_\varepsilon$ with total source strength $F = \int_{\mathscr{C}_\varepsilon} f \d{V}$, where $\mathscr{C}_R$ is a cylinder of radius $R$ and height 1 with its axis aligned along the $z$ axis.
...
\begin{proof}
  ...
  Suppose $\exists \vec{x} \in B_r(\vec{a})$ such that $\phi(\vec{x}) < \phi(\vec{a})$, then the average value over $B_r(\vec{a})$, that is, $\overline{\phi}_r(\vec{a})$, would also be strictly less than $\phi(\vec{a})$ which contradicts the result from \cref{sphereAverage} that $\overline{\phi}_r(\vec{a}) = \phi(\vec{a})$.
  Hence $\phi(\vec{x}) = \phi(\vec{a})\ \forall \vec{x} \in B_r(\vec{a})$.

  We now increase $r$ until it is large enough for $B_r(\vec{a})$ to intersect the boundary $\partial V$ at some point $\vec{b}$.
