...
\subsubsection{Constant Surfaces}
\begin{itemize}
  \item Surfaces of constant $\rho$ are cylinders aligned along the $z$-axis.
...
\]
Note that this matches the notation $\vec{e}_r = \frac{1}{r}\vec{r}$ that we used earlier in \cref{radialIdentities}.
\subsubsection{Infinitesimals}
Using \cref{vectorInfinitesimal}, we obtain:
\begin{align*}
...
\begin{remark}[Note]
  ...
  \]
  when looking at the laplacian of a $\R^3$ radially symmetric $f(r)$ in \cref{laplacianSphericalSymmetry}.

  We have also already seen the first two terms of $\nabla^2 f$ in cylindrical coordinates:
  \[
    \pderiv[2]{f}{\rho} + \frac{1}{r}\pderiv{f}{\rho}
  \]
  when looking at the laplacian of a $\R^2$ radially symmetric $f(r)$.
  This makes sense as if we ignore $z$, a cylindrically symmetric field is just a radially symmetric field in $\R^2$.
\end{remark}
\subsection{Examples}
...
\end{example}
\subsection{Summary of Examinable Content}
