...
\begin{enumerate}
  ...
  \item For $A, B \in \salg$, $\P(B \setminus A) = \P(A \cup B) - \P(A)$ as $\P(A \cup B) = \P((B \setminus A) \cup A) = \P(B \setminus A) + \P(A)$.
  \item If $A \subseteq B$, then $\P(B \setminus A) = \P(B) - \P(A)$ as $A \cup B = B$.
...
\begin{proof}
  ...
  \]
  These $B_n$ are the ``new'' elements of $A_n$ that do not occur in any of the previous $A_i$.
...
\begin{example}[6-Sided Die]
  ...
  We then choose $\P(\{\omega\}) = 1/6$ for $\omega \in \Omega$ so that each singleton event is equally likely as the dice is fair.
...
\begin{example}[Deck of Cards]
  ...
  \[
    \P(\text{top 2 cards are aces}) = \frac{4 \times 3 \times 50!}{52!} = \frac{1}{221}
  \]
...
\begin{example}[Largest Digit]
  ...
  \]
  We can then define the event $A_k$ to be:
  \[
...
\begin{proof}
  ...
  {$n - 1$ events}{}
  {$n$ events}{
    We start by applying the result for two events:
    \begin{align*}
...
Suppose $\Omega$ is a finite set with $|\Omega| = n$ which we want to partition $\Omega$ into $k$ disjoint subsets $\Omega_1, \ldots, \Omega_k$ with $|\Omega_i| = n_i$ and $n_1 + \cdots + n_k = n$.
...
\end{definition}
\begin{remark}
  In the case of $k = 2$, $n_1 + n_2 = n \implies n_2 = n - n_1$ and so we obtain the standard binomial coefficient as this counts the ways to pick a subset of size $n_1$ from $n$:
  \[
    \binom{n}{n_1, n - n_1} = \frac{n!}{n_1!(n - n_1)!} = \binom{n}{n_1}
  \]
\end{remark}
...
\begin{remark}[Nomenclature]
  Increasing functions are sometimes also referred to as \textit{non-decreasing}.
\end{remark}
\begin{proposition}
...
\end{proof}
\begin{proposition}
  For $k, n \in \N$, the number of \textbf{increasing} $f: \{1, \ldots, k\} \to \{1, \ldots, n\}$ is $\binom{n + k - 1}{k}$.
\end{proposition}
\begin{proof}
...
\begin{proof}[Stirling's Formula -- \Cref{stirling}]\par
  ...
  Let
  \[
    A = \exp\left[1 - \sum_{k = 1}^{\infty} a_k\right] < \infty
  \]
  So $n! = n^n e^{-n} \sqrt{n} \cdot A \exp\left[\sum_{k = n}^{\infty} a_k\right]$.
  As $n \to \infty$, since $\sum_{k = 1}^{\infty} a_k$ converges, $\sum_{k = n}^{\infty} a_k \to 0$ so $\exp\left[\sum_{k = n}^{\infty} a_k\right] \to 1$.
  Thus:
  \[
  ...
  Secondly, using a different method, we will show that $2^{-2n}\binom{2n}{n} \sim \frac{1}{\sqrt{\pi n}}$ as $n \to \infty$, which will then force $A = \sqrt{2\pi}$ by uniqueness of limits.

  \textbf{Showing $2^{-2n} \binom{2n}{n} \sim \frac{1}{\sqrt{\pi n}}$}\par
  Consider the integrals $I_n$ defined by:
  \[
  ...
  As discussed, this forces $A = \sqrt{2\pi}$ and so $n! \sim \sqrt{2\pi n} \left(\frac{n}{e}\right)^{n}$.
\end{proof}
...
If we know the probabilities of each of the parameters, $\P(B_i)$, and we have a model which gives us $\P(A \vert B_i)$ (the probability of the evidence occurring given each of the parameters), then with this formula we can find the \textit{posterior probabilities}.
These are the probabilities of a particular parameter $B_k$ occurring given the evidence $A$ occurs.
\subsection{Examples}
\begin{example}[False Positives]
  ...
  \]
  In this example, $\P(A^{\comp})$ and $\P(P \vert A)$ are very close to 1 so the relevant ratio governing the size of $\P(A \vert P)$ is $\frac{\P(P \vert A^{\comp})}{\P(A)}$.
...
\begin{example}
  ...
  \end{align*}
  At least one child is a boy is the event $BB \cup BG \cup GB$ and both children are boys is the event $BB$.
  ...
  \end{align*}
  This probability is less than $1/2$ as not knowing which ``position'' the boy is in means there are two ways they could have 1 boy and 1 girl but only one way they could have two boys.
  ...
  \]
  Here we get the intuitive $1/2$ result as we specified which ``position'' the boy was in.
  ...
  \textbf{Summary}\par
  To make sense of this, consider how it is easier to have a boy born on a Thursday if you have two boys rather than one boy.
  Therefore, if we have the information that one of them is a boy born on a Thursday, it is now less likely that there is only one boy, i.e. it is more likely that there are two boys, hence why the probability is greater than the $1/3$ we got originally.
\end{example}
