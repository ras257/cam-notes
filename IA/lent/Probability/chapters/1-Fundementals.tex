...
\begin{proof}
  ...
  \]
  Then $(B_n)$ is a disjoint collection of sets in $\salg$ and $\bigcup\limits_{n \in \N} B_n = \bigcup\limits_{n \in \N} A_n$.
  So
  \[
...
\begin{proof}
  ...
  \]
  Then $(B_n)$ is disjoint collection of events and $A_n = \bigcup_{k = 1}^{n} B_k$
  \[
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \implies \P(A_n) &\to \P\left(\left[\bigcup_{k = 1}^{\infty} A^{\comp}_k\right]^{\comp}\right) = \P\left(\bigcap_{k = 1}^{\infty} A_k\right)
  \end{align*}
  which is the desired result.
\end{proof}
