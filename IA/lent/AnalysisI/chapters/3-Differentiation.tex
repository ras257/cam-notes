...
\begin{theorem}[Inverse Function Theorem -- Version 2]
  If $f: [a, b] \to \R$ is continuous and differentiable on $(a, b)$ and $f'(x) > 0\ \forall x \in (a, b)$, then $f: [a, b] \to [f(a), f(b)]$ is bijective and $f^{-1} : [f(a), f(b)] \to [a, b]$ is continuous and differentiable in $(f(a), f(b))$ with derivative: \label{inverseFunction2}
  \[
...
\begin{remark}
  ...
  If we instead have $f'(x) < 0\ \forall x \in [a, b]$, then the result still holds by applying the theorem to $g(x) = -f(x)$.
\end{remark}
...
\begin{proposition}[Cauchy Mean Value Theorem]
  If $f, g: [a, b] \to \R$  are continuous and differentiable on $(a, b)$, then $\exists c \in (a, b)$ such that: \label{cauchyMVT}
  \[
