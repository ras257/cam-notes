...
\end{definition}
\begin{remark}[Note]
  Here both $x \in X$ and $a + h \in X$ as we cannot approach $a$ from outside of the domain.
\end{remark}
\begin{remark}
...
\begin{itemize}
  \item For interior points, we can approach in any direction.
    If the limits from different directions disagree, then the limit does not exist.
...
\begin{example}
  \begin{enumerate}
    ...
    \item Consider $f(z) = \overline{z}$.
      ...
      \[
        \lim_{\lambda \to 0} \frac{f(z + i\lambda) - f(z)}{i\lambda} = \lim_{\lambda \to 0} \frac{-i\lambda}{\lambda} = -i
      \]
...
\begin{lemma}[Differentiation Rules]
  Let $f, g: X \subseteq \C \to \C$ be differentiable at $a \in X$, then so are: \label{diffRules}
  \begin{enumerate}
...
\begin{remark}[Remarks]
  \begin{enumerate}
    ...
    \item In the proof we see that in both directions, $A = f'(a)$ and so this must always be the case as if we have such a function, then $A = f'(a)$.
  \end{enumerate}
\end{remark}
\begin{proof}
...
\begin{proposition}[Chain Rule]
  ...
  If $f$ is differentiable at $a \in U$ and $g$ is differentiable at $f(a) \in V$, then $g \circ f: U \to \C$ is differentiable at $a \in U$ and $(g \circ f)'(a) = g'(f(a))f'(a)$. \label{chainRule}
\end{proposition}
\begin{proof}
...
\begin{proof}
  ...
  \begin{align*}
    \lim_{x \to a} f(x) &= \lim_{h \to 0} f(a + h) \\
                        ...
                        &= f(a)
  \end{align*}
  and so $f$ is continuous at $a$ by the remark in \cref{continuityLimit}.
\end{proof}
...
\begin{proof}
  ...
  \]
  Because $f(a) = f(b)$, if the function is not constant, then the extreme values cannot both occur at the endpoints of $[a, b]$ and so there must be an extreme value attained at some value in $(a, b)$.
  \begin{proofcases}
    \begin{case}{$f(x_{\text{min}}) = f(x_{\text{max}})$}
      If $f(x_{\text{min}}) = f(x_{\text{max}})$, then the function must be fixed at $f(a)$ and so $f'(c) = 0\ \forall c \in (a, b)$.
    \end{case}
    \begin{case}{$f(x_{\text{min}}) < f(a)$}
      Since $f(x_{\text{min}}) < f(a) = f(b)$, $x_{\text{min}} \in (a, b)$.
      We wish to show that $f'(x_{\text{min}}) = 0$.
      ...
      Since $f$ is differentiable on $(a, b)$, as $h_n \to 0$ both of the above limit to $f'(x_{\text{min}})$.
      Thus $f'(x_{\text{min}})$ is trapped below and above by 0 and so $f'(x_{\text{min}}) = 0$.
    \end{case}
    \begin{case}{$f(x_{\text{max}}) > f(a)$}
      Since $f(x_{\text{max}}) > f(a) = f(b)$, $x_{\text{max}} \in (a, b)$.
      Similarly to before:
      \[
...
\end{proof}
\begin{remark}
  Instead of using sequences $h_n$ decreasing to 0 from above, we also directly use the \textit{left and right limits}, for example in \textbf{Case 2}, since $f'(x_{\text{min}})$ exists:
  \[
    f'(x_{\text{min}}) = \begin{cases}
     \lim_{h \to 0^{+}} \frac{f(x_{\text{min}} + h) - f(x_{\text{min}})}{h} \geq 0 \\
     \lim_{h \to 0^{-}} \frac{f(x_{\text{min}} + h) - f(x_{\text{min}})}{h} \leq 0
    \end{cases}
  \]
  where $h \to 0^{+}$ and $h \to 0^{-}$ mean the limit as $h$ approaches 0 from above and below respectively.
\end{remark}
...
\begin{proof}
  ...
  \]
  If we subtract this line from $f(x)$, then the values at $a$ and $b$ must both be 0, i.e let $\phi(x) = f(x) - \ell(x)$ so that $\phi(a) = \phi(b) = 0$.
...
\begin{corollary}
  ...
  \begin{enumerate}
    \item $f' \geq 0$ on $(a, b) \iff f$ is increasing.
    \item $f' > 0$ on $(a, b) \implies f$ is strictly increasing.
    \item $f' \leq 0$ on $(a, b) \iff f$ is decreasing.
    \item $f' < 0$ on $(a, b) \implies f$ is strictly decreasing.
    \item $f' = 0$ on $(a, b) \iff f$ is constant.
  \end{enumerate}
\end{corollary}
\begin{proof}
  See Example Sheet 2 Q1e and Q10.
\end{proof}
\begin{remark}[Warning]
  ...
  However, this function is clearly not constant.
\end{remark}
We can generalise the relation between $f'(z) = 0$ and constant functions into $\C$:
\begin{corollary}
  If $f: \C \to \C$ is differentiable in $\C$ and $f'(z) = 0\ \forall z \in \C$, then $f$ is constant. \label{constantF}
\end{corollary}
\begin{proof}
...
\begin{theorem}[Inverse Function Theorem -- Version 2]
  If $f: [a, b] \to \R$ is continuous on $[a, b]$ and differentiable on $(a, b)$ and $f'(x) > 0\ \forall x \in (a, b)$, then $f: [a, b] \to [f(a), f(b)]$ is bijective and $f^{-1} : [f(a), f(b)] \to [a, b]$ is continuous on $[f(a), f(b)]$ and differentiable in $(f(a), f(b))$ with derivative: \label{inverseFunction2}
  \[
...
\begin{proof}
  ...
  Given arbitrary $h$ such that $y + h \in (f(a), f(b))$, since $f$ is bijective we can define $k(h)$ such that $y + h = f(x + k)$.
...
\end{proof}
\begin{remark}
  In the above we required that $f'(x) > 0\ \forall x \in [a, b]$ so that $f$ was monotonically increasing, however if we instead have $f'(x) < 0\ \forall x \in [a, b]$, then the result still holds by instead applying the theorem to $g(x) = -f(x)$.
\end{remark}
\begin{proposition}[Local Inverse Function Theorem]
  If $f: [a, b] \to \R$ is continuous and differentiable on $(a, b)$ and $\exists x_0 \in (a, b)$ such that $f'(x_0) \neq 0$, then in some region around $x_0$ there is an inverse function $f^{-1}$ in that region that is differentiable.
\end{proposition}
\begin{proof}
  See Example Sheet 3.
\end{proof}
...
\end{remark}
\begin{example}
  \label{smoothRational}
  \begin{enumerate}
    \item Fix some $R > 0$ and define $f: [0, R] \to \R$, $x \mapsto x^{N}$ for some $N \in \N$ (we need $N \neq 0$ else $f$ is not strictly increasing).
...
\begin{proposition}[Cauchy Mean Value Theorem]
  If $f, g: [a, b] \to \R$ are continuous on $[a, b]$ and differentiable on $(a, b)$, then $\exists c \in (a, b)$ such that: \label{cauchyMVT}
  \[
...
\begin{proof}
  ...
  \]
  Like we did in the proof for \cref{MVT} we want to try to use Rolle's Theorem (\cref{rollesTheorem}) the LHS should be the derivative of some function $\phi$ evaluated at $c$ which satisfies $\phi(a) = \phi(b) = 0$.
  The function $\phi(x)$ defined by:
  \[
    \phi(x) = [g(x) - g(a)][f(b) - f(a)] - [g(b) - g(a)][f(x) - f(a)] \\
  \]
  satisfies this.
  It is also continuous and differentiable on the correct intervals as $f$ and $g$ are.

  So applying Rolle's theorem, $\exists c \in (a, b)$:
  \[
    \phi'(c) = 0 \implies g'(c)[f(b) - f(a)] - f'(c)[g(b) - g(a)] = 0
  \]
\end{proof}
\begin{proposition}[L'H\^opital's Rule]
  Let $-\infty \leq a < b \leq \infty$ and let $f, g: [a, b] \to \R$ be continuous on $[a, b]$ and differentiable on $(a, b)$.
  Suppose $g(x) \neq 0\ \forall x \in (a, b)$ and $\lim_{x \to a} \frac{f'(x)}{g'(x)} = A$ for some $-\infty \leq A \leq \infty$ (where the cases of $\pm \infty$ are suitably interpreted). \label{lHopitals}

  As $x \to a$, if either $f(x), g(x) \to 0$ or $f(x), g(x) \to \pm \infty$, then:
  \[
...
\begin{definition}[Continiously Differentiable]
  We say that $f: X \subseteq \C \to \C$ is \textit{$n$-times} continuously differentiable and write $f \in C^{n}(X)$ if $f$ is $n$-times differentiable and $f^{(n)}: X \to \R$ is continuous. \label{ctsDifferentiable}
\end{definition}
\begin{remark}
...
\begin{definition}[Taylor Polynomial and Series]
  For $f: X \to \C$ that are $n$-times differentiable and $x_0 \in X$, we define the \textit{degree $n$ Taylor polynomial for $f$ about $x_0$} as: \label{taylorSeries}
  \[
...
\begin{definition}[Taylor Remainder]
  For $f: X \to \C$ that are $(n - 1)$-times differentiable, $x_0 \in X$, and $h$ such that $x_0 + h \in X$, we define the \textit{Taylor remainder} $R_{n, f, x_0}(h)$ as: \label{taylorRemainder}
  \begin{align*}
...
\begin{theorem}[Taylor's Theorem -- Lagrange Remainder]
  If $f: X \to \R$ is continuous on $[a, a + h] \subset X$ and $n$ times differentiable on $(a, a + h)$, then $\exists \theta \in (0, 1)$ such that:
...
\begin{remark}[Remarks]
  \begin{itemize}
    \item All of the requirements above are satisfied if $f \in C^{n}((c, d))$ and $[a, a + h] \subset (c, d)$.
      This is usually true when we are using Taylor's theorem, however the above statement requires as little as possible about the function.
    ...
    \item We do not need $h > 0$, as if $h = -t < 0$ and $f$ has $n - 1$ derivatives that are continuous on $[a - t, a]$ and is $n$ times differentiable on $(a - t, a)$, then we can apply the above to $g(x) = f(-x)$:
      \begin{align*}
        f(a + h) = g(-a + t) &= \sum_{n=0}^{n - 1} \frac{g^{(k)}(-a)}{k!} t^{k} + \frac{t^{n}}{n!}g^{(n)}(-a + \theta t) \\
                             ...
                             &= \sum_{n=0}^{n - 1} \frac{f^{(k)}(a)}{k!} h^{k} + \frac{h^{n}}{n!}f^{(n)}(a + \theta h)
      \end{align*}
    \item If $f \in C^{n}((c, d))$, with $[a, a + h] \subset (c, d)$, then $f^{(n)}$ is also continuous and hence by EVT (\cref{EVT}) is bounded on $[a, a + h]$ and so $\exists M_n$ s.t. $M_n = \sup\limits_{x \in [a, a + h]} |f^{(n)}(x)|$.
...
\begin{proof}[1 -- Similarly to MVT (\cref{MVT})]\par
  ...
  Since $f$ is $n$ times differentiable on $(a, a + h)$, it first $n - 1$ derivatives must also be continuous in this interval.
...
\begin{proof}[2 -- Using a clever function]
  ...
  \end{align*}
  Now, we need to construct a function to apply Rolle's theorem (\cref{rollesTheorem}) on, so, for $p \in \{1, \ldots, n\}$ define:
  \[
  ...
  \begin{equation}
    f(h) = T_{n - 1, f, 0}(h) + \frac{h^{n}}{p(n - 1)!}(1 - \theta)^{n - p}f^{(n)}(\theta h) \label{pRemainder}
  \end{equation}
  so taking $p = n$ yields the desired result.
\end{proof}
\begin{remark}
  This form of the remainder where we can pick any $p \in \{1, \ldots, n\}$ is known as the \textit{Schl\"omilch Remainder}.
\end{remark}
\begin{theorem}[Taylors Theorem -- Cauchy Remainder]
  ...
  \label{cauchyRemainder}
  \[
    R_{n, f, a}(h) =  \frac{h^n}{(n - 1)!} (1 - \theta)^{n - 1}f^{(n)}(a + \theta h)
  \]
  or equivalently:
  \[
    f(a + h) = \sum_{k = 0}^{n - 1} \frac{f^{(k)}(a)}{k!}h^{k} + \frac{h^n}{(n - 1)!} (1 - \theta)^{n - 1}f^{(n)}(a + \theta h)
  \]
\end{theorem}
\begin{proof}
  In the 2nd proof of \cref{lagrangeRemainder} above, taking $p = 1$ yields:
  \[
    f(h) = T_{n - 1, f, 0}(h) + \frac{h^{n}}{(n - 1)!}(1 - \theta)^{n - 1}f^{(n)}(\theta h)
  \]
  which is the desired result.
\end{proof}
\begin{remark}
  If $f \in C^{n}([c, d])$ with $[a + h] \subset (c, d)$, then as we did for the Lagrange remainder, we can set:
  \[
    M_{n} = \sup_{x \in [a, a + h]} |f^{(n)}(x)|
  \]
  Then using the Cauchy remainder:
  \[
    |R_{n, f, a}(h)| \leq M_n \cdot \frac{|h|^{n}}{(n - 1)!}
  \]
  This doesn't look any better than the estimate that we get from the Lagrange remainder as we now have $(n - 1)!$ instead of $n!$, however, the Cauchy remainder is sometimes more useful in a different way as we will see below.
\end{remark}
\begin{example}[Lagrange vs Cauchy Remainder]
  Consider $f(x) = x^{q}$ for $q \in \Q$.
  From \cref{smoothRational}, we know that this is smooth on $(0, \infty)$ with $k$-th derivative:
  \begin{align*}
    f^{(k)}(x) &= q(q - 1)(q - 2) \cdots (q - k + 1) x^{q - k} \\
    \implies f^{(k)}(1) &= q(q - 1)(q - 2) \cdots (q - k + 1)
  \end{align*}
  Thus, the $n - 1$ degree Taylor polynomial of $f$ about $1$ is:
  \begin{align*}
    T_{n - 1, f, 1}(1 + x) &= \sum_{k = 0}^{n - 1} \frac{q(q - 1) \cdots (q - k + 1)}{k!} x^{k} \\
                           &= \sum_{k = 0}^{n - 1} \binom{q}{k} x^{k}
  \end{align*}
  where $\binom{q}{k} = \frac{1}{k!} \prod_{r = 0}^{k - 1} (q - r)$ is the \textit{generalised binomial coefficient}.

  We would like to bound the Taylor remainder $R_{n, f, 1}$ of this for $x \in (-1, 1)$.
  \[
    R_{n, f, 1}(x) = (1 + x)^{q} - \sum_{k = 0}^{n - 1} \binom{q}{k}x^{k}
  \]

  \textbf{Using Lagrange Remainder}\par
  If we use the Lagrange remainder (\cref{lagrangeRemainder}), $\exists \theta \in (0, 1)$ such that:
  \begin{align*}
    R_{n, f, 1}(x) &= \frac{x^{n}}{n!}f^{(n)}(1 + \theta x) \\
                   &= \frac{q(q - 1) \cdots (q - n + 1)}{n!} (1 + \theta x)^{q - n} x^{n} \\
                   &= \binom{q}{n} (1 + \theta x)^{q - n} x^{n}
  \end{align*}
  If $n \geq q$, then $(1 + \theta x)^{q - n} \leq 1$ for $x \in [0, 1)$ and so:
  \[
    |R_{n, f, 1}(x)| = \abs{\binom{q}{n}x^{n}}|(1 + \theta x)^{q-n}| \leq \abs{\binom{q}{n}x^{n}}
  \]
  If we let $a_n = \binom{q}{n}x^{n}$, we see that:
  \[
    \abs{\frac{a_{n + 1}}{a_{n}}} = \abs{\frac{r - n}{n + 1}}|x| \to |x|
  \]
  Since $x \in [0, 1)$, by the ratio test (\cref{ratioTest}), $\sum_{n = 0}^{\infty} |a_n|$ converges.
  Thus the $n$-th term test (\cref{nthTermTest}) tells us that $a_n \to 0$ as $n \to \infty$.
  So we get that $R_{n, f, 1}(x) \to 0$ as $n \to \infty$ for $x \in [0, 1)$.

  This argument fails for $x \in (-1, 0)$ because $(1 + \theta x)^{q - n} \geq 1$ for $n \geq 1$, so we cant bound the remainder in this range using the Lagrange remainder.

  \textbf{Using Cauchy Remainder}\par
  If we instead use the Cauchy remainder (\cref{cauchyRemainder}), $\exists \theta \in (0, 1)$ such that:
  \begin{align*}
    R_{n, f, 1}(x) &= \frac{x^{n}}{(n - 1)!}(1 - \theta)^{n - 1}f^{(n)}(1 + \theta x) \\
                   &= n \cdot \frac{x^{n}}{n!} \cdot (1 - \theta)^{n - 1} q(q - 1) \cdots (q - n + 1)(1 + \theta x)^{q - n} \\
                   &= (1 - \theta)^{n - 1}n\binom{q}{n}(1 + \theta x)^{q - n} x^{n} \\
                   &= (1 - \theta)^{n - 1}q \binom{q - 1}{n-1} (1 + \theta x)^{q - n}x^{n} \\
                   &= q \binom{q - 1}{n - 1} \left(\frac{1 - \theta}{1 + \theta x}\right)^{n - 1} (1 + \theta x)^{q - 1} x^{n}
  \end{align*}
  We see that $1 + \theta x - (1 - \theta) = \theta (x + 1) \geq 0$ as $x > -1$ so $1 - \theta \leq 1 + \theta x$.
  Therefore, provided that $n \geq 1$:
  \[
    \left(\frac{1 - \theta}{1 + \theta x}\right)^{n - 1}  \leq 1
  \]

  We would like also like a term similar to $\binom{q}{n}x^{n}$ from the previous case so we reorganise the terms as:
  \[
    R_{n, f, a}(x) = qx \cdot \binom{q - 1}{n - 1}x^{n - 1} \cdot (1 + \theta x)^{q - 1} \cdot \left(\frac{1 - \theta}{1 + \theta x}\right)^{n - 1}
  \]
  Hence:
  \begin{align*}
    |R_{n, f, 1}(x)| &= |qx| \abs{\binom{q - 1}{n - 1}x^{n - 1}} |1 + \theta x|^{q - 1}\abs{\frac{1 - \theta}{1 + \theta x}}^{n - 1} \\
                     &\leq |qx| \abs{\binom{q - 1}{n - 1} x^{n - 1}}|1 + \theta x|^{q - 1}
  \end{align*}
  This is similar to the previous case, however, here the term with $1 + \theta x$ does not have any $n$ dependence so we can define the constant $C = |qx||1 + \theta x|^{q - 1}$ for a given $x$.
  We then have the bound:
  \[
    |R_{n, f, 1}(x)| \leq C \abs{\binom{q - 1}{n - 1}x^{n - 1}} \to 0 \text{ as $n \to \infty$}
  \]
  and this is true for every $x \in (-1, 1)$.

  Thus, the Cauchy remainder gives us that the remainder is exponentially small as $n \to \infty$ for all $x \in (-1, 1)$, however, the Lagrange remainder could only guarantee this for $x \in [0, 1)$.
\end{example}
\begin{definition}[Analytic Function]
  We say that $f: (c, d) \to \R$ with $f \in C^{\infty}$ is \textit{analytic} if for every $a \in (c, d)$, there exists an $r > 0$ such that $\forall h \in \R \text{ with } |h| < r$:
  \[
    f(a + h) = \sum_{k = 0}^{\infty} \frac{f^{(k)}(a)}{k!}h^{k} = \lim_{n \to \infty} T_{n, f, a}(a + h)
  \]
  That is, for such $h$, $R_{n, f, a}(h) \to 0$ as $n \to \infty$.
\end{definition}
\begin{remark}[Remarks]
  \begin{itemize}
    \item There are lots of other definitions of what an analytic function is, none of which are ``canonical'' and so the above definition is \textbf{non-examinable}.
    \item $R_{n, f, a}(h) \centernot\to 0$ as $n \to \infty$ does not mean that the Taylor series does not converge, just that it does not equal $f(a + h)$.
  \end{itemize}
\end{remark}
\end{document}
