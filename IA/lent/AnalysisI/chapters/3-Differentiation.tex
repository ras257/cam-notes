...
\begin{example}[Lagrange vs Cauchy Remainder]
  ...
  We would like also like a term similar to $\binom{q}{n}x^{n}$ from the previous case so we rewrite:
  \[
...
\begin{remark}[Remarks]
  \begin{itemize}
    ...
    \item $R_{n, f, a}(h) \centernot\to 0$ as $n \to \infty$ does not mean that the Taylor series does not converge, just that it does not equal $f(h)$.
  \end{itemize}
\end{remark}
\end{document}
