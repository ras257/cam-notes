...
\begin{definition}[Bounded Sequence]
  We say that $(x_n)$ is bounded if $\exists M > 0$ such that $|x_n| \leq M$ or equivalently $\sup\limits_{n \geq0} |x_n| \leq M$.
\end{definition}
...
\begin{proof}
  ...
  \]
  Since there are finitely many times with index $< N$, we can construct the bound:
  \[
...
\end{remark}
\begin{remark}
  We see from $x_n = (-1)^{n}$ that there can be more than one convergent subsequence and their limits can disagree.
\end{remark}
\end{document}
