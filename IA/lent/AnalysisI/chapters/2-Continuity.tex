...
\begin{remark}
  ...
  \begin{itemize}
    ...
    \item If $a$ is an accumulation point (so attempting to $z \to a$ is valid), then $f$ is continuous if and only if $\lim\limits_{z \to a} f(z) = f(a)$.
    \item If $a$ is an isolated point, then, somewhat tautologically, $f$ is continuous at $a$ because $|z - a| < \delta \implies z = a \implies f(z) = f(a)$ for $\delta$ sufficiently small.
  \end{itemize}
\end{remark}
\begin{example}
  \begin{enumerate}
