...
\begin{definition}[Limit of a function]
  Let $f: X \subseteq \C \to \C$ and let $a \in \C$ be an accumulation point of $X$.

  We say that \textit{$f$ tends to $y \in \C$ as $z$ tends to $a$} if:
  \label{limitDef}
  \[
    \forall \varepsilon > 0\ \exists \delta = \delta(\varepsilon) > 0 \text{ s.t. } (0 < |z - a| < \delta \text{ for } z \in X \implies |f(z) - y| < \varepsilon)
  \]
  and write $f(z) \to y$ as $z \to a$.
\end{definition}
\begin{remark}[Note]
  We can also equivalently define $f(z) \to y$ as $z \to a$ as:
  \[
    \forall \varepsilon > 0\ \exists \delta = \delta(\varepsilon) > 0 \text{ s.t. } (|z - a| < \delta \text{ for } z \in X \setminus \{a\} \implies |f(z) - y| < \varepsilon)
  \]
  Note how here we restrict to $z \in X \setminus \{a\}$ so that $z = a$ is not considered, whereas above, we achieve the same thing by restricting to $0 < |z - a|$.

  The important intuition here is how the \textbf{limit does not care about what the function does at $a$}, hence why we can take the limit to a point outside the domain of the function.
\end{remark}
\begin{remark}
  If $a$ is an isolated point of $X$, then we can always find $\delta$ sufficiently small so that $|z - a| < \delta,\ z \in X \iff z = a$.
  This means that for such $\delta$:
  \[
    |z - a| < \delta \text{ for } z \in X \setminus \{a\} \implies |f(z) - y| < \varepsilon
  \]
  is vacuously true as there are no such $z \in X \setminus \{a\}$ satisfying $|z - a| < \delta$.

  This means that if we included isolated points in the definition, $f(z) \to y$ as $z \to a$ for any $y \in \C$ trivially.
  So, the definition would make sense for isolated points, however, is not very interesting and leads to non-unique limits, hence why we exclude isolated points from the definition.
\end{remark}
\begin{definition}[Divergence to infinity of a function]
  Let $a \in \C$ be an accumulation point for $X$.
  We say that $f$ diverges to $\infty$ if as $z \to a$ if $\lim\limits_{z \to a} \frac{f(z)}{|f(z)|}$ exists and:
  \[
    \forall L > 0,\ \exists \delta = \delta(L) \text{ s.t. } (0 < |z - a| < \delta \text{ for } z \in X \implies |f(z)| > L)
  \]
\end{definition}
\begin{remark}[Note]
  Again, we can choose to exclude $0$ by restricting to $0 < |z - a|$ as above, or by restricting to $z \in X \setminus \{a\}$.
  We need to do this as we could have a function that diverges to infinity as $x \to a$ but with $f(a) = 0$.
\end{remark}
\begin{example}
  \label{sinxExample}
  ...
  We can show geometrically using a unit circle that:
  \begin{equation}
  ...
  \]
  Choosing $\delta = \sqrt{2\varepsilon}$ satisfies this and so $f(x) \to 1$ as $x \to 0$.
\end{example}
\subsection{Limit Properties}
\begin{lemma}
  Let $f: X \subseteq \C \to \C$ and $a \in \C$ be an accumulation point of $X$.
  \begin{enumerate}
    \item $f(z) \to y$ as $z \to a$ if and only if $f(z_n) \to y$ for every sequence $(z_n)$ on $X \setminus \{a\}$ with $z_n \to a$.
    \item $f(z)$ diverges to infinity as $z \to a$ if and only if $(f(z_n))_n$ diverges to infinity for every $(z_n)_{n \in \N}$ on $X \setminus \{a\}$ with $z_n \to a$.
  \end{enumerate}
\end{lemma}
\begin{proof}[\textbf{i}]
  \begin{proofdirection}{Assume $f(z) \to y$ as $z \to a$}
    Suppose we have a sequence $(z_n)$ on $X \setminus \{a\}$ with $z_n \to a$.

    Since $f(z) \to y$, $\forall \varepsilon > 0\ \exists \delta > 0$ s.t. $0 < |z - a| < \delta \implies |f(z) - y| < \varepsilon$.
    Since $z_n \to a$, for all such $\delta$, $\exists N$ s.t. $|z_n - a| < \delta\ \forall n \geq N$ and so, since $z_n \neq a$, $|f(z_n) - y| < \varepsilon\ \forall n \geq N$.
  ...
  \begin{proofdirection}{Assume $f(z_n) \to y$ for every sequence with $z_n \to a$}
    ...
    Then $\exists \varepsilon > 0 \text{ s.t. } \forall \delta > 0\ \exists z = z(\delta) \text{ s.t. } 0 < |z - a| < \delta$ but $|f(z) - y| \geq \varepsilon$.

    For some such $\varepsilon$, consider $\delta = \frac{1}{n}$.
    By the above, we can always find some $z_n \in X \setminus \{a\}$ such that $|z_n - a| < \frac{1}{n} = \delta$ but $|f(z_n) - y| \geq \varepsilon$.
    We construct the sequence $(z_n)$ as a sequence of such $z_n$.
    Thus we have constructed a sequence with $z_n \to a$, however $\exists \varepsilon > 0 \text{ s.t. } |f(z_n) - y| \geq \varepsilon\ \forall n$ and so $f(z_n) \centernot\to y$.
    This is a contradiction so we must have $f(z) \to y$.
  \end{proofdirection}
\end{proof}
\begin{proof}[\textbf{ii}]
  \begin{proofdirection}{Assume $f(z)$ diverges to $\infty$ as $z \to a$}
    Suppose we have a sequence $(z_n)$ on $X \setminus \{a\}$ with $z_n \to a$.

    Since $f(z)$ diverges to $\infty$, $\forall L > 0\ \exists \delta > 0$ s.t. $0 < |z - a| < \delta \implies |f(z)| > L$.
    Since $z_n \to a$, for all such $\delta$, $\exists N$ s.t. $|z_n - a| < \delta\ \forall n \geq N$ and so, since $z_n \neq a$, $|f(z_n)| > L\ \forall n \geq N$.

    Thus given some $L > 0$, $\exists N \text{ s.t. } |f(z_n)| > L\ \forall n > N$.
    Finally, since $f(z)$ diverges to $\infty$, $\lim_{z \to a} \frac{f(z)}{|f(z)|}$ exists and so by \textbf{(i)}, $\lim_{n \to \infty} \frac{f(z_n)}{|f(z_n)|}$ also exists.
    Therefore $f(z_n)$ diverges to $\infty$.
  \end{proofdirection}
  \begin{proofdirection}{Assume $f(z_n) \to y$ for every sequence with $z_n \to a$}
    For the purposes of contradiction, assume that $f(z)$ does not diverge to $\infty$ as $z \to a$.

    If $\lim_{x \to a} \frac{f(z)}{|f(z)|}$ does not exist, then by \textbf{(i)}, neither does $\lim_{n \to \infty} \frac{f(z_n)}{|f(z_n)|}$ for any sequence $z_n$ on $X \setminus \{a\}$ with $z_n \to a$ and so $z_n$ does not diverge to $\infty$.

    Otherwise, $\exists L > 0 \text{ s.t. } \forall \delta > 0\ \exists z = z(\delta) \text{ s.t. } 0 < |z - a| < \delta$ but $|f(z)| \leq L$.
    For some such $L$, consider $\delta = \frac{1}{n}$.
    By the above, we can always find some $z_n \in X \setminus \{a\}$ such that $|z_n - a| < \frac{1}{n} = \delta$ but $|f(z_n)| \leq L$.
    We construct the sequence $(z_n)$ as a sequence of such $z_n$.
    Thus we have constructed a sequence with $z_n \to a$, however $\exists L > 0 \text{ s.t. } |f(z_n)| \leq L\ \forall n$ and so $f(z_n)$ does not diverge to $\infty$.

    In either case, we have a contradiction and so $f(z)$ must diverge to $\infty$ as $z \to a$.
  \end{proofdirection}
\end{proof}
\begin{lemma}[Uniqueness of Limits]
  If $f: X \subseteq \C \to \C$ has a limit as $z \to a$, then this limit is unique.
\end{lemma}
\begin{proof}
  Suppose $f(z) \to y$ as $z \to a$ and $f(z) \to x$ as $z \to a$.
  Then $\forall \varepsilon > 0$:
  \begin{align*}
    \exists \delta_1 = \delta_1(\varepsilon) > 0 \text{ s.t. } 0 < |z - a| < \delta_1 \implies |f(z) - y| < \varepsilon \\
    \exists \delta_2 = \delta_2(\varepsilon) > 0 \text{ s.t. } 0 < |z - a| < \delta_2 \implies |f(z) - x| < \varepsilon
  \end{align*}
  Hence $\forall z \text{ s.t. } 0 < |z - a| < \min\{\delta_1, \delta_2\}$ (there must be some such $z$ as $a$ is an accumulation point and $\delta_1, \delta_2 > 0$):
  \[
...
\begin{lemma}[Limit Laws]
  Let $f, g: X \subseteq \C \to \C$ and $a \in \C$ be an accumulation point of $X$.
  If $\lim_{z \to a} f(z) = y$ and $\lim_{z \to a} g(z) = x$, then:
  \label{limitLaws}
  \begin{enumerate}
    \item $\lim_{z \to a} (f(z) + g(z)) = y + x$
    \item $\lim_{z \to a} (f(z)g(z)) = yx$
    \item If $g(z) \neq 0$ in some region around $a$ (but not necessarily at $a$) and $x \neq 0$, then $\lim_{z \to a} \frac{f(z)}{g(z)} = \frac{y}{x}$.
  \end{enumerate}
\end{lemma}
\begin{proof}
  Suppose $f(z) \to y$ as $z \to a$ and $f(z) \to x$ as $z \to a$.
  Then $\forall \varepsilon > 0$:
  \begin{align*}
    \exists \delta_1 = \delta_1(\varepsilon) \text{ s.t. } 0 < |z - a| < \delta_1 \implies |f(z) - y| < \varepsilon \\
    \exists \delta_2 = \delta_2(\varepsilon) \text{ s.t. } 0 < |z - a| < \delta_2 \implies |g(z) - x| < \varepsilon
  \end{align*}
  \begin{enumerate}
    ...
    \item We can also pick $\varepsilon = 1$ so that:
      \[
        \exists \delta_3 \text{ s.t. } 0 < |z - a| < \delta_3 \implies |f(z) - y| < \varepsilon \implies |f(z)| < 1 + |y|
      \]
    ...
    \item To show this, we can show that $\lim_{z \to a} \frac{1}{g(z)} = \frac{1}{x}$ and then use \textbf{(ii)}.
      ...
      \]
      Since $g$ is non-zero in some region around $a$, $\exists \delta_4 > 0 \text{ s.t. } 0 < |z - a| < \delta_4 \implies g(z) \neq 0$.
      Then $\forall z \text{ s.t. } 0 < |z - a| < \min\{\delta_2, \delta_3, \delta_4\}$:
      \begin{align*}
...
\begin{remark}
  \begin{itemize}
    \item The strategies used here are similar to those used when proving \cref{limitLaws} and \cref{reciprocalConvergence}, however, instead of bounding the whole sequence or its tail, we just introduce a new threshold $\delta$ such that if $0 < |z - a| < \delta$, then $|f(z)|$ is bounded by some constant.
    \item The proof for \textbf{(ii)} demonstrated that if $f(z) \to y$ as $z \to a$, then $\exists M, \delta > 0$ such that $0 < |z - a| < \delta \implies |f(z)| \leq M$.

      Similarly, the proof for \textbf{(iii)} demonstrated that if $g(z) \to x \neq 0$, then $\exists B, \delta > 0$ such that $0 < |z - a| < \delta \implies |g(z)| \geq B$.

      This is similar to how we bounded convergent sequences, however it only holds for $z$ sufficiently close to $a$ but not at $a$.
  \end{itemize}
\end{remark}
...
\begin{remark}
  ...
  \begin{itemize}
    \item Unlike in the definition for a limit (\cref{limitDef}), we do not require that $0 < |z - a|$ or $z \in X \setminus \{a\}$, this is because at $z = a$, we clearly have $0 = |f(z) - f(a)| < \varepsilon$ as a function can take only a single value for each input.
      Thus, it makes no difference to the definition of continuity whether we choose to include $z = a$ for consideration.
    \item If $a \in X$ is an accumulation point (so attempting to take $z \to a$ is valid), then $f$ is continuous if and only if $\lim\limits_{z \to a} f(z) = f(a)$.

      If $a \in X$ is instead an isolated point, then, somewhat tautologically, $f$ is continuous at $a$ because $|z - a| < \delta \implies z = a \implies f(z) = f(a)$ for $\delta$ sufficiently small.
  \end{itemize}
\end{remark}
\begin{example}
  \begin{enumerate}
    \item The function $f: \C \to \C,\ z \mapsto z$, the identity function, is continuous at any $a \in \C$ as $\forall \varepsilon > 0$:
      \[
        |z - a| < \delta(\varepsilon) \implies |f(z) - f(a)| = |z - a| < \varepsilon
      \]
      by picking $\delta(\varepsilon) = \varepsilon$.
    \item The function $f: \C \to \R,\ z \mapsto |z|$ is continuous at any $a \in \C$ as $\forall \varepsilon > 0$:
      \[
...
\begin{proposition}[Equivalence of Sequential and Standard Continuity]
  ...
  \label{sequentialContinuityEquivalence}
\end{proposition}
\begin{proof}
  \begin{proofdirection}{Assume $f$ is continuous}
    By the continuity of $f$, $\forall \varepsilon > 0,\ \exists \delta = \delta(\varepsilon) \text{ s.t. } |z - a| < \delta \implies |f(z) - f(a)| < \varepsilon$.
    ...
    Thus for arbitrary $\varepsilon > 0,\ \exists N(\varepsilon) \text{ s.t. } |f(z_n) - f(a)| < \varepsilon \ \forall n \geq n$ and so $f(z_n) \to f(a)$.
  \end{proofdirection}
  \begin{proofdirection}{Assume $f$ is sequentially continuous}
    ...
    We define the sequence $(z_n)$ as a sequence of such $z_n$.
    Thus we have constructed a sequence with $z_n \to a$, however $|f(z_n) - f(a)| \geq \varepsilon\ \forall n$ so $f(z_n) \centernot\to f(a)$.
    Hence the sequence $(z_n)$ contradicts sequential continuity at $a$ and so $f$ must be continuous at $a$.
  \end{proofdirection}
\end{proof}
\begin{example}[Using Sequential Continuity]
  \label{dirichletFunction}
  \begin{enumerate}
    \item The function:
      \[
        f(x) = \begin{cases}
    ...
    \item The \textit{Dirichlet function}:
      ...
      \begin{itemize}
        \item For $a \in \Q$, the sequence $x_n = a - \frac{\sqrt{2}}{n}$ on $\R \setminus \Q$ has $x_n \to a$.
          But $f(x_n) = 0\ \forall n \implies f(x_n) \to 0$ but $0 \neq 1 = f(a)$ and so $f$ is not continuous at $a$.
        \item For $a \in \R \setminus \Q$, the sequence $x_n$ on $\Q$ which is the truncated decimal expansion of $a$ has $x_n \to a$.
      ...
      Note that in future we can just assume such sequences on $\Q$ or $\R \setminus \Q$ exist because $\Q$ is dense in $\R$.

      This can also be shown using the non sequential definition of continuity, utilising again that both irrational and rational numbers are dense in the reals.
  \end{enumerate}
\end{example}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \exists \delta_2 = \delta_2(\varepsilon) \text{ s.t. } |z - a| < \delta_2 \implies |g(z) - g(a)| < \varepsilon
  \end{align*}
  \begin{enumerate}
    \item $\forall z \text{ s.t. } |z - a| < \min\{\delta_1, \delta_2\}$, we have:
      \begin{align*}
    ...
    \item We can pick $\varepsilon = \abs{\frac{f(a)}{2}} > 0$ so that:
      ...
      \]
      Then $\forall z \text{ s.t. } |z - a| < \min\{\delta_1, \delta_3\}$:
      \begin{align*}
        \abs{\frac{1}{f(z)} - \frac{1}{f(a)}} &= \frac{1}{|f(a)||f(z)|}|f(z) - f(a)| \\
                                       &< \frac{\varepsilon}{|f(a)|\abs{\frac{f(a)}{2}}}
      \end{align*}
...
\begin{proof}
  ...
  \[
    \exists \delta_2(\delta_1) \text{ s.t. } |z - a| < \delta_2 \text{ for } z \in U \implies |f(z) - f(a)| < \delta_1
  \]
...
\begin{theorem}[Extreme Value Theorem]
  Let $X \subset \R$ be a bounded closed set.
...
\begin{theorem}[Intermediate Value Theorem]
  If $f: [a, b] \to \R$ is continuous, then given any $y \in [\min\{f(a), f(b)\}, \max\{f(a), f(b)\}]$ there exists $c \in [a, b]$ such that $f(c) = y$.
  \label{IVT}
...
\begin{proof}
  ...
  \]
  So $\forall x \in [a, b] \text{ s.t. } |x - d| < \delta \implies x \notin S$, therefore $S \cap (d - \delta, d] = \emptyset$ which contradicts that $d$ is the supremum of $S$ since $d - \delta < d$ is an upper bound for $S$.
  ...
  \]
  However, if we consider $x = d + \frac{\delta}{2}$, $|d + \frac{\delta}{2} - d| < \delta \implies f(d + \frac{\delta}{2}) < y$ and so $d + \frac{\delta}{2} \in S$ but $d + \frac{\delta}{2} > d$ which contradicts the fact that $d$ is an upper bound for $S$.
...
\begin{remark}
  ...
  \]
\end{remark}
\begin{proposition}[Inverse Function Theorem -- Version 1]
...
\begin{proof}\par
  ...
  \textbf{Strictly Monotone --} Suppose $f^{-1}: [c, d] \to [a, b]$ is not strictly increasing, then $\exists y_1, y_2 \in [c, d]$ with $y_1 < y_2$ but $f^{-1}(y_1) \geq f^{-1}(y_2)$.
  ...
  \begin{proofcases}
    \begin{case}{$x_0 \in (a, b)$}
      ...
      We can use this inequality to construct two positive quantities that we can try to use as a $\delta$:
      \[
    ...
    \begin{case}{$x_0 = a$}
      ...
      \begin{align*}
        |y - y_0| < \delta,\ y \geq y_0 &\implies y_0 \leq y < y_0 + \delta \\
                                        &\implies f(a) \leq y < f(a) + \delta = f(a + \eta) \\
