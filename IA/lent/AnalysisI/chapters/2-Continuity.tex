...
\end{remark}
\begin{proof}
  If $f$ is constant, then the result follows trivially, if not, WLOG, $f(a) \leq f(b)$.
  Consider the set $S = \{x \in [a, b]: f(x) \leq y\}$.
  Then $a \in S \implies$ S is non empty.
  ...
  Thus $f(d) \leq y$ and $f(d) \geq y$ so $f(d) = y$.

  \textbf{Intervals}\par
  For the more general statement, set $J = f([a, b])$ and take any $y_1, y_2 \in J$.
  Since $y_1, y_2 \in f([a, b])$, there exists $x_1, x_2 \in I$ with $f(x_1) = y_1$ and $f(x_2) = y_2$.

  Since $[a, b]$ is an interval, $x_1, x_2 \in [a,b] \implies [x_1, x_2] \subseteq [a, b]$.
