...
Suppose that we wanted to find the shortest path between 2 points:
\begin{enumerate}
...
\]
and can do a similar thing in other cases.

A \textit{functional} is a map from a function $y(x)$ to $\R$.
This means our original problem reduces to wanting know which function $y(x)$ minimises this functional.
...
We say that $\vec{a} \in \R^{n}$ is a \textit{stationary point} of $f$ if and only if $\nabla f(\vec{a}) = \vec{0}$.
...
\]
where $H_{i j}(\vec{a}) = \frac{\partial^2 f}{\partial x_i \partial x_j}(\vec{a})$ is the \textit{Hessian} which is symmetric (i.e. $H_{i j} = H_{j i}$) since partial derivatives commute.
...
\]
From IA Vectors and Matrices, since it is symmetric, we can carry out an orthogonal diagonalisation.
Further, we can pick our diagonalisation matrix so that it is a rotation as we can simply reorder its coluns to ensure $\det R = 1$.
The components are then transformed by $x_i = R_{i j}x_j'$ and $H' = R^{\trans} H R$ is the diagonal matrix:
\[
...
\begin{itemize}
  ...
  \item If there exists a positive eigenvalue and a negative eigenvalue, then we have a saddle point as the function increases in one direction but decreases in another for sufficiently small $|\vec{x}|$.
  \item The remaining cases are \textit{degenerate stationary points}.
