...
\[
  \int_{\alpha}^{\beta} \delta y' \pderiv{f}{y'} \d{x} = \eval{\delta y \pderiv{f}{y'}}{\alpha}{\beta} - \int_{\alpha}^{\beta} \delta y \deriv{}{x} \pderiv{f}{y'} \d{x}
\]
...
\end{example}
\begin{example}[Brachistochrone Curve] \label{exBrachistochrone}
  ...
  \end{align*}
  If we now regard $x$ as a function of $\theta$, i.e. $x(\theta)$, then we need to solve:
  \[
...
\begin{example}[Further Examples]
  ...
  \end{itemize}
\end{example}
\subsection{Sturm-Liouville Problem} \label{SLPSec}
